...
Public Class FrmInStorge
    ...
    Private Sub FrmInStorge_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub FrmInStorge_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region "新建單據編號"
    Private Sub ToolStripButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TsBnInBill.Click
        ...
        'If StrMaxInBill = "" Then
        '    MessageBox.Show("單據生成失敗，請重建！", "信息提示！", MessageBoxButtons.OK, MessageBoxIcon.Information)
        '    Exit Sub
        'End If
       

        'Catch ex As Exception
        'SysMessageClass.WriteErrLog(ex, "MesWarehouseManage.FrmInStorge", "BnInBill_Click", "sys")
        'End Try
    End Sub
#End Region
...
#Region "入庫掃描"
        ...
        If e.KeyChar = Chr(13) Then
            Try
                ...
                If TxtMoid.Text = "" Then ''掃描第一筆箱號
                    ...
                    If DrPubUse.HasRows Then
                        ...
                        ElseIf DrPubUse.GetString(2) = "Y" Then
                            ...
                            If ClassInf.ChoseY = True Then
                                ...
                                IntQty = DrPubUse("Cartonqty").ToString
                                DrPubUse.Close()
                                '生成最大入庫單號
                                StrMaxInBill = GetInWhId()
                    ...
                    End If
                Else ''核對工單
                    If Trim(CobInBill.Text) = "" Then
                        MessageBox.Show("請選擇右上方下拉框中的[入庫單號]!", "信息提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        TxtCartonid.Clear()
                        CobInBill.Focus()
                        Exit Sub
                    End If
                    ...
                    If DrPubUse.HasRows Then
                        ...
                        If DrPubUse.GetString(1) = "I" Then
                            PlaySimpleSound(1)
                            MessageBox.Show("該箱已入庫,不能重復入庫!", "信息提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            TxtCartonid.Clear()
                            DrPubUse.Close()
                            Exit Sub
                        ElseIf DrPubUse.GetString(1) = "S" Then
                            PlaySimpleSound(1)
                            MessageBox.Show("該箱為拆散箱,不能重復入庫!", "信息提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            TxtCartonid.Clear()
                            DrPubUse.Close()
                            Exit Sub
                        ElseIf DrPubUse.GetString(1) = "E" Then
                            PlaySimpleSound(1)
                            MessageBox.Show("該箱為空箱,不能入庫!", "信息提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            TxtCartonid.Clear()
                            DrPubUse.Close()
                            Exit Sub
                        ElseIf DrPubUse.GetString(1) = "N" Then
                            PlaySimpleSound(1)
                            MessageBox.Show("該箱未裝滿,不能入庫!", "信息提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            TxtCartonid.Clear()
                            DrPubUse.Close()
                            Exit Sub
                        ElseIf DrPubUse.GetString(1) = "O" Then
                            PlaySimpleSound(1)
                            MessageBox.Show("該箱已正常出庫,不能再入庫!", "信息提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            TxtCartonid.Clear()
                            DrPubUse.Close()
                            Exit Sub
                        ElseIf DrPubUse.GetString(1) = "R" Then
                            PlaySimpleSound(1)
                            MessageBox.Show("該箱已出庫重工,不能再入庫!", "信息提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            TxtCartonid.Clear()
                            DrPubUse.Close()
                            Exit Sub
                        ElseIf DrPubUse.GetString(1) = "Q" Then
                            PlaySimpleSound(1)
                            MessageBox.Show("該箱已FQC抽驗判退,不能入庫!", "信息提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            TxtCartonid.Clear()
                            DrPubUse.Close()
                            Exit Sub
                        ElseIf DrPubUse.GetString(1) = "Y" Then
                ...
                End If
                '2009/10/28 -----
                Me.LabSumQty.Text = CInt(IIf(Me.LabSumQty.Text.Trim = "", 0, Me.LabSumQty.Text.Trim)) + IntQty
                TxtCartonid.Clear()
                PlaySimpleSound(0)
    ...
    Private Sub BtPScanEnd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim DrPCscan As SqlDataReader
...
#Region "確定入庫數據處理"
    Sub ConfirmInStorge()
        ...
        If MessageBox.Show("你確定將入庫單號:" & Me.CobInBill.Text & "入庫嗎？", "信息提示！", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = Windows.Forms.DialogResult.OK Then
            'StrInCof.Append(" begin tran Whupdate  ")
            ...
            StrInCof.Append(Environment.NewLine)
            'StrInCof.Append(" if @@error<>0 ")
            'StrInCof.Append(Environment.NewLine)
            'StrInCof.Append("commit tran  Whupdate ")
            'StrInCof.Append(Environment.NewLine)
            'StrInCof.Append("else")
            'StrInCof.Append(Environment.NewLine)
            'StrInCof.Append("rollback tran Whupdate ")
...
#Region "廢除掃描記錄"
    Private Sub TsBtDrop_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TsBtDrop.Click
        ...
        If MessageBox.Show("你確定捨棄入庫單號:" & Me.CobInBill.Text & "？", "信息提示！", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = Windows.Forms.DialogResult.OK Then
            ...
            'Reason :          M_Carton_t表增加 updatetime 和updateuser
            '**********************************************************
            'StrInDrop.Append("update M_Carton_t set cartonstatus='Y' where cartonid in (select cartonid from m_WhInD_t where inwhid='" & Me.CobInBill.Text & "' )")
                    ...
                    Case "Y"
                        ...
                        If DrPubUse.Read() Then
                            ...
                            If DrPubUse.Read() Then
                                ...
                                StrScan.Append(" begin tran TwhinD ")
                                ...
                                StrScan.Append(" if @@error<>0  commit tran  TwhinD  else   rollback tran TwhinD ")
                                PubClass.ExecSql(StrScan.ToString)
                                StrScan.Remove(0, StrScan.ToString.Length)
