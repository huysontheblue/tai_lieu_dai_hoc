...
#End Region
    ...
    Private Sub BtMultiPack_Click(sender As Object, e As EventArgs) Handles BtMultiPack.Click
        Dim Moid As String = "二层箱号替换"

        If String.IsNullOrEmpty(Me.rtxtMultiRemark.Text.Trim) Then
            MessageUtils.ShowInformation("请填写替换说明！")
            Exit Sub
        End If
        If Me.TxtMultiCarton.Text = "" Or Me.TxtNewMultiCarton.Text = "" Then
            MessageUtils.ShowInformation("旧箱号或新箱号不能为空！")
            Exit Sub
        End If
        If Len(Me.TxtMultiCarton.Text.Trim) <> Len(Me.TxtNewMultiCarton.Text.Trim) Then
            MessageUtils.ShowInformation("替换新旧箱号长度不一致！")
            Return
        End If
        Dim dt1 As DataTable = DbOperateUtils.GetDataTable("select 1 from m_cartonlevel_t  where SN ='" & Me.TxtNewMultiCarton.Text.Trim & "' AND SnLevel='1'")
        If dt1.Rows.Count > 0 Then
            MessageUtils.ShowInformation("该替换的新箱号已扫描，不允许被替换！")
            Return
        End If

        Dim strSQL As String
        Try
            strSQL = "update m_cartonlevel_t set PSN='" & Me.TxtNewMultiCarton.Text & "' where PSN='" & Me.TxtMultiCarton.Text & "'AND SnLevel='0'"
            strSQL = strSQL & vbNewLine & " update m_cartonlevel_t set SN='" & Me.TxtNewMultiCarton.Text & "' where SN='" & Me.TxtMultiCarton.Text & "'AND SnLevel='1'"
            strSQL = strSQL & vbNewLine & "insert into m_BarcodeExch_t(Oldbarcode,Newbarcode,moid,Userid,Intime,Remark)values('" & TxtMultiCarton.Text & "','" & TxtNewMultiCarton.Text & "',N'" & Moid & "','" & SysMessageClass.UseId & "',getdate(),N'" & Me.rtxtRemark.Text.Trim & "'" & ")"
            DbOperateUtils.ExecSQL(strSQL)
            MessageUtils.ShowInformation("二层外箱替换成功！")
        Catch ex As Exception
            MessageUtils.ShowInformation(ex.Message)
        End Try
        Me.TxtMultiCarton.Text = ""
        Me.TxtNewMultiCarton.Text = ""
        Me.rtxtMultiRemark.Text = ""

    End Sub

    Private Sub TextHoldSn_keyPress(sender As Object, e As KeyPressEventArgs) Handles TextHoldSn.KeyPress
        If e.KeyChar = Chr(13) Then
            Dim holdSn As String = TextHoldSn.Text
            Dim dt1 As DataTable = DbOperateUtils.GetDataTable("SELECT * FROM m_TestResult_t WHERE IsHold='Y' and ppid='" + holdSn + "'")
            If dt1.Rows.Count <= 0 Then
                MessageUtils.ShowInformation("该条码未锁定不需要解锁！")
                Return
            End If
            Dim strSQL As String
            Try
                strSQL = "update m_TestResult_t set IsHold='N' where ppid='" + holdSn + "'"
                strSQL = strSQL & vbNewLine & "insert into m_BarcodeExch_t(Oldbarcode,Newbarcode,moid,Userid,Intime,Remark)values('" & holdSn & "','" & holdSn & "',N'','" & SysMessageClass.UseId & "',getdate(),N'解锁'" & ")"
                DbOperateUtils.ExecSQL(strSQL)
                Label21.Text = "解锁成功！"
            Catch ex As Exception
    ...
    Private Sub TextLinkSn_keyPress(sender As Object, e As KeyPressEventArgs) Handles TextLinkSn.KeyPress
        If e.KeyChar = Chr(13) Then
            Dim linkSn As String = TextLinkSn.Text
            Dim csn As String
            Dim dt1 As DataTable = DbOperateUtils.GetDataTable("SELECT  * FROM m_AnkerLink_t WHERE SN='" + linkSn + "'")
            If dt1.Rows.Count <= 0 Then
                MessageUtils.ShowInformation("该条码绑定不需要解锁！")
                Return
            End If
            Dim moid As String = ""
            Dim partId As String = ""
            Dim DT As DataTable = DbOperateUtils.GetDataTable("SELECT b.moid ,b.PartID FROM m_Assysn_t a , m_Mainmo_t b where a.moid=b.moid and ppid='" & linkSn & "'")
            If DT.Rows.Count > 0 Then
                moid = DT.Rows(0)("moid")
                partId = DT.Rows(0)("PartID")
            End If
            csn = dt1.Rows(0)("csn")
            Dim strSQL As String
            Try
                strSQL = "DECLARE @Falg int;DECLARE @Msg nvarchar(150);   EXEC Eexc_ReScanJumpStationHandleNew '" & linkSn & "','" & moid & "','" & partId & "','T00329','" & VbCommClass.VbCommClass.UseId & "', @Falg OUTPUT,@Msg OUTPUT "
                ' strSQL = "INSERT INTO [m_BarcodeExch_t]([Oldbarcode], [Newbarcode], [Moid], [Userid], [Intime]) VALUES ('" & linkSn & "','" & csn & "','拆关联','" & SysMessageClass.UseId & "',getdate())"
                ' strSQL = strSQL & vbNewLine & "DELETE m_AnkerLink_t WHERE  SN='" + linkSn + "'"
                '删除CSN测试记录
                Dim sql As String = "delete MESDataCenter.dbo.m_TestResult_t where ppid='" & csn & "'"
                Dim sql1 As String = "delete [MESDataCenter].dbo.m_AnkerFocusing_t where CSN='" & csn & "'"
                DbOperateUtils.ExecSQL(sql)
                DbOperateUtils.ExecSQL(sql1)
                DbOperateUtils.ExecSQL(strSQL)
                Label30.Text = "解绑成功！"
            Catch ex As Exception
