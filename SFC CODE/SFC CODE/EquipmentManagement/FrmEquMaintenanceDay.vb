...
Public Class FrmEquMaintenanceDay

    Private m_strSelectDay As String = ""
#Region "属性"
    '当日是否已保养
    Private _MaintenanceIsExist As Boolean
    Public Property MaintenanceIsExist() As Boolean
        Get
            Return _MaintenanceIsExist
        End Get

        Set(ByVal Value As Boolean)
            _MaintenanceIsExist = Value
        End Set
    End Property
...
#Region "窗体事件"
    ...
    ''' 初始加载窗体
    ''' </summary>
    Private Sub FrmEquMaintenanceDay_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub tcMaintenanceDay_SelectedIndexChanged(sender As Object, e As EventArgs) Handles tcMaintenanceDay.SelectedIndexChanged
        If tcMaintenanceDay.SelectedIndex = 1 Then
            If m_strSelectDay = "" Then
                dtPickerCurrentDay.Text = Now.ToString("yyyy-MM-dd")
            Else
                dtPickerCurrentDay.Text = Now.ToString("yyyy-MM") + "-" + m_strSelectDay
            End If
            '加载当日保养数据
            LoadMaintenToDay()
        Else
...
#Region "自定义方法"
    ...
    Private Sub LoadMaintenEveryDay()
        Try
            ...
            If String.IsNullOrEmpty(Me.lbMonths.Text) Then
                Exit Sub
            End If
            dt = DbOperateUtils.GetDataTable(String.Format("exec GetEquMaintenanceByDay N'{0}',N'{1}','{2}'", Me.txtAssetNo.Text, Me.txtAssetName.Text, Me.lbMonths.Text))
    ...
    Private Sub LoadMaintenToDay()
        Try
            ...
            o_strSql.Append(String.Format("  FROM m_EquMaintenanceType_t WHERE AssetName =N'{0}' and IsMpDay=1   ORDER BY id ASC", Me.txtAssetNameToDay.Text.Trim))
            ...
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmEquMaintenanceDay", "BindData()", "sys")
        End Try
    End Sub
    ...
    Private Function CheckData() As Boolean
        '检验当日是否有保养
        If Me.MaintenanceIsExist = True Then
    ...
    Private Function CheckDataSpec() As Boolean
        If Me.dtPickerCurrentDay.Value = Now.ToString("yy-MM-dd") Then
            '检验当日是否有保养
            If Me.MaintenanceIsExist = True Then
                Me.lbMsg.Text = "当日已保养,请勿重复保养!"
                Return False
    ...
    Private Function InputCheckGridView() As Boolean
        ...
        For Each Row As DataGridViewRow In Me.dgvMaintenToDay.Rows
            ...
            IsNotUsed = CType(Row.Cells(5), DataGridViewCheckBoxCell).EditingCellFormattedValue
    ...
    Private Function SaveData() As Boolean
        Try
            ...
            Next
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmEquMaintenanceDay", "SaveData", "Equ")
        End Try
        Return True
    End Function
...
#End Region
    ...
    Private Sub dgvMaintenanceEveryDay_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvMaintenanceEveryDay.CellClick
        If dgvMaintenanceEveryDay.RowCount < 1 Then Exit Sub

        m_strSelectDay = CStr(Me.dgvMaintenanceEveryDay.CurrentCell.ColumnIndex)
    End Sub
