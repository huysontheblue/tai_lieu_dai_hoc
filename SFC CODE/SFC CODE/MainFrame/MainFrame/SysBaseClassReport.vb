...
#Region "數據庫數據處理-报表服务器"

Namespace SysDataHandle

    Public Class SysDataBaseClassReport
...
#Region "連接數據字符串"
        ...
        Private Function GetConnString() As String
            ...
            m_SqlserverName = SysDataBaseClass.SqlserverName
            Dim IsReadOlny As Boolean = True ''默认开启只读
            If Environment.Version.Major = 2 And Environment.Version.Build = 50727 And Environment.Version.Revision < 8762 Then
                IsReadOlny = False '当系统.net2.0版本太低的时候，不支持AlwaysOn，不开启只读 --by hs ke 20180411
            End If
            m_SqlserverIP = m_SqlserverName.Split(";")(0).Split("=")(1).ToString
            GetConnString = m_SqlserverName
            GetConnString = tool.GetOriginalConnString(GetConnString) + ";min pool size=10;max pool size=500;Connection Timeout=9999;pooling=false"
            '’dg-mesdb.luxshare.com.cn数据库指定SFC报表查询只读数据库为172.20.22.91--by hs ke 20180410
            If IsReadOlny Then
                If GetConnString.ToLower.IndexOf("dg-mesdb.luxshare.com.cn") > 0 Or
                                GetConnString.ToLower.IndexOf("172.20.23.53") > 0 Or
                                GetConnString.ToLower.IndexOf("172.20.23.59") > 0 Then
                    GetConnString = GetConnString.ToLower.Replace("dg-mesdb.luxshare.com.cn", "172.20.23.54")
                    GetConnString = GetConnString.ToLower.Replace("172.20.22.91", "172.20.23.54")
                    GetConnString = GetConnString.ToLower.Replace("172.20.23.59", "172.20.23.54")
                    GetConnString = GetConnString + ";ApplicationIntent=ReadOnly"
                    Exit Function
                End If
            End If
...
#Region "取得dataset數據集"
        ...
        Public Function GetDataSet(ByVal SqlStr As String, ByVal TableName As String) As DataSet
            ...
            SysDataSet = Nothing
        End Function
        Public Function GetDataSet(ByVal sql As String, Optional ByVal parameters As IDataParameter() = Nothing, Optional ByVal TableName As String = "") As DataSet
...
#Region "取得datatable"
        Public Function GetDataTable(ByVal SqlStr As String) As System.Data.DataTable
            SysCommand = New SqlCommand(SqlStr, PubConnection)
            SysCommand.CommandTimeout = 999
        ...
        Public Function GetDataTable(ByVal SqlStr As String, ByVal ParamArray cmdParms As SqlParameter()) As System.Data.DataTable
            ...
            PrepareCommand(SysCommand, Nothing, cmdParms)
            Dim SysAdapter As New SqlDataAdapter(SysCommand)
            SysAdapter.SelectCommand.CommandTimeout = 30000
            Dim SysDataTable As New System.Data.DataTable
            SysAdapter.Fill(SysDataTable)
            SysCommand.Parameters.Clear()
            GetDataTable = SysDataTable
