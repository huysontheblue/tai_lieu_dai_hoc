...
Public Class FrmSampleUploadOld
#Region "变量定义"
#Region "EquFileName"
...
#End Region
    ...
    Private Sub btnImportEqu_Click(sender As Object, e As EventArgs) Handles btnImportEqu.Click
        ...
        If Not String.IsNullOrEmpty(Me.EquFileName) Then
            ...
            Dim strPartNO As String = String.Empty, strFormatDes As String = ""
            strPartNO = SampleCom.GetPNInfo(Me.txtSampleNo.Text, strFormatDes)
            ' Me.lblPartNO.Text = "产品料号:[" & strPartNO & "]"
    ...
    Private Sub UpdateSheetUpload()
        ...
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "SampleManage.FrmSampleUpload", "UpdateSheetUpload", "sys")
        End Try
    End Sub
    ...
    Private Sub LoadData(ByVal Sql As String, ByVal type As SheetType)
        ...
        Select Case type
            ...
            Case SheetType.Document    '"TabDocument"
                'Sql = " SELECT SampleNO 样品单号,  BluePath 蓝图, CutPath 裁线卡, RCardPath 流程卡 " & _
    ...
    Private Sub ToolRefresh_Click(sender As Object, e As EventArgs) Handles ToolRefresh.Click
        Dim strPartNO As String = "", strFormatDes As String = "", SqlStr As String = ""
        Try
        ...
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "SampleManage.FrmSampleUpload", "ToolRefresh_Click", "sys")
        End Try
    End Sub
    ...
    Public Function GetSheetInfo(Sqlstring As String) As DataTable
        ...
        Select Case TabSheet.SelectedTab.Name
            ...
            Case "TabZEqu"
                ...
                StrSql = StrSql & " ORDER BY no"
            Case "TabModel"
                StrSql = "SELECT a.SampleNO,NO,ModelName, a.Qty, IsShare ,SharePN ,RDUserName,PlanDueDate, RealDueDate" & _
                " FROM m_SampleModel_t a LEFT JOIN m_Sample_t b ON a.SampleNO = b.SampleNO " & _
                " WHERE 1=1 " & Sqlstring
                StrSql = StrSql & " ORDER BY no"
            Case "TabDocument"
...
#End Region
    ...
    Private Sub UpdatePathToDB(ByVal iDocumentType As Integer, ByVal strPath As String)
        ...
        Try
            ...
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmSampleUpload", "UpdatePathToDB", "WIP")
        End Try
    End Sub
    ...
    Private Sub ToolEquConfirm_Click(sender As Object, e As EventArgs) Handles ToolEquConfirm.Click
        ...
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmSampleUpload", "ToolEquConfirm_Click", "WIP")
        End Try
    End Sub
    ...
    Private Sub dgvZEqu_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvZEqu.CellDoubleClick
        ...
        Try
            ...
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "SampleManage.FrmSampleUpload", "dgvZEqu_CellClick", "sys")
        End Try
    End Sub
...
#End Region
    ...
    'Private Sub dgvModel_CellBeginEdit(sender As Object, e As DataGridViewCellCancelEventArgs) Handles dgvModel.CellBeginEdit
    '    If e.RowIndex = -1 OrElse Me.dgvModel.Rows.Count <= 0 Then Exit Sub
    '    Dim strIsShare As String = "", o_tempstrPlanDueDate As String = ""
    '    Try
    '        For rowIndex As Integer = 0 To Me.dgvModel.Rows.Count - 1
    '            strIsShare = DbOperateUtils.DBNullToStr(dgvModel.Rows(rowIndex).Cells(enumdgvModel.IsShare).EditedFormattedValue)
    '            If Not String.IsNullOrEmpty(strIsShare) Then
    '                Select Case strIsShare
    '                    Case "否"
    '                        Me.dgvModel.Rows(rowIndex).Cells(enumdgvModel.PlanDueDate).ReadOnly = False
    '                        Me.dgvModel.Rows(rowIndex).Cells(enumdgvModel.RealDueDate).ReadOnly = False
    '                    Case "是"
    '                        Me.dgvModel.Rows(rowIndex).Cells(enumdgvModel.PlanDueDate).ReadOnly = True
    '                        Me.dgvModel.Rows(rowIndex).Cells(enumdgvModel.RealDueDate).ReadOnly = True
    '                    Case Else
    ...
    Private Sub ToolComfrm_Click(sender As Object, e As EventArgs) Handles ToolComfrm.Click
        Select Case TabSheet.SelectedTab.Name
            Case "TabBOM"
                If Not SampleCom.CheckIsBOMDateFinish(Me.txtSampleNo.Text) Then
                    MessageUtils.ShowError("BOM的交期维护未完成,请检查！")
                    Exit Sub
                Else
                    Call BOMConfirm()

                    MessageUtils.ShowInformation("确定成功")
                End If
            Case Else
                Exit Sub
        End Select
    ...
    '批量维护交期
    Private Sub ToolBlock_Click(sender As Object, e As EventArgs) Handles ToolBlock.Click
        Dim o_strColumnName As String = "", strPlanDate As String = "", strRealDate As String = ""
        Dim o_strSQL As New StringBuilder
        Select Case TabSheet.SelectedTab.Name
            Case "TabEqu"
            ...
            Case "TabBOM"
                ...
                '            o_strSQL.Append(" WHERE  SampleNO ='" & Me.txtSampleNo.Text.Trim & "' AND isnull(IsExist,'0') ='0'  AND ISNULL(PlanDueDate,'') = '' ")
                '            ' MAX( cast(nullif(PlanDuedate,'') as date))
                '            o_strSQL.Append(" Declare @MaxPlanDate datetime")
                '            o_strSQL.Append(" SELECT @MaxPlanDate= MAX( cast(nullif(PlanDuedate,'') as date)) FROM m_SampleBOM_t WHERE sampleno ='" & Me.txtSampleNo.Text.Trim & "' AND IsExist ='0' and isnull(RealDuedate,'') = ''")
                '            o_strSQL.Append(" Update M_SampleD_t set PlanDueDate = @MaxPlanDate WHERE sampleno = '" & Me.txtSampleNo.Text.Trim & "' AND levelid ='" & SampleCom.enumLevelID.MaterialGive & "'")
        ...
        If Not String.IsNullOrEmpty(o_strSQL.ToString) Then
            DbOperateUtils.ExecSQL(o_strSQL.ToString)
            MessageUtils.ShowInformation("批量更新日期成功!")
        End If
    ...
    Private Sub dgvEqu_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvEqu.CellDoubleClick
        ...
        Try
            ...
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "SampleManage.FrmSampleUpload", "dgvEqu_CellDoubleClick", "sys")
        End Try
    End Sub
