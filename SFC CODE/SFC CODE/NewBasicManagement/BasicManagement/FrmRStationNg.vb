...
#Region "初始化树"
    ...
    Private Sub AdvTreeLoad()
        Try
            Dim dt As DataTable = conn.GetDataTable("select distinct ECodeID EsortName,ECodeName  CsortName  from m_StationCode_t where StationID ='" + station + "'  ")
        ...
        Try
            ...
            If addOrDel = True Then
                ...
                If level = 2 Then
                    ...
                    sqlCheckStr.Append("INSERT INTO [dbo].[m_StationCode_t] ([StationID],[StationName],[ECodeID],[ECodeName],[CodeID],[CodeName],[Factory],[Profit],[UserID],[Intime]) " &
                        "VALUES('" + station + "',N'" + stationName + "','" + Split(node.Parent.Text.ToString, "-")(0) + "',N'" + Split(node.Parent.Text.ToString, "-")(1) + "','" + Split(node.Text.ToString, "-")(0) + "',N'" + codeName + "',N'" + factory + "',N'" + profit + "','" + userID + "',GETDATE ())")
            ...
            Else
                ...
                '为false选定
                If level = 2 Then
                    '  sqlCheckStr.Append(" delete from [dbo].[m_StationCode_t] where StationID='" + station + "' and ECodeID='" + Split(node.Parent.Text.ToString, "-")(0) + "' and CodeID='" + Split(node.Text.ToString, "-")(0) + "' and isnull(partid,'')='' ")
                    sqlCheckStr.Append(" delete from [dbo].[m_StationCode_t] where StationID='" + station + "' and ECodeID='" + Split(node.Parent.Text.ToString, "-")(0) + "' and CodeID='" + Split(node.Text.ToString, "-")(0) + "' and isnull(partid,'')='' ")
                    sqlCheckStr.Append(Environment.NewLine)
    ...
    Private Sub CheckAllChildNodes(ByVal treeNode As TreeNode, ByVal nodeChecked As Boolean, ByVal nodeChecky As Boolean)
        Try
            ...
            For Each node In treeNode.Nodes
                ...
                Else
                    If node.Level = 2 Then
                        sqlCheckStr.Append("delete from [dbo].[m_StationCode_t] where StationID='" + station + "' AND ")
                        sqlCheckStr.Append(" ECodeID='" + Split(node.Parent.Text.ToString, "-")(0) + "' and CodeID='" + Split(node.Text.ToString, "-")(0) + "' and isnull(partid,'')='' ")
                        sqlCheckStr.Append(Environment.NewLine)
                        ' else
                    End If
                End If
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                For i As Integer = 0 To dt.Rows.Count - 1
                    Dim barcodeInfo As String = "SFCSNG#" & dt.Rows(i)("CodeID").ToString

                    sheet.Cells.SetRowHeight(2 * Fix(i / 5) + 2, 75)
                    sheet.Cells.SetRowHeight(2 * Fix(i / 5) + 3, 49)
                    sheet.Cells.SetColumnWidth(i Mod 5, 16)

                    Dim index As Integer = sheet.Pictures.Add(2 * Fix(i / 5) + 2, i Mod 5, Print_Qrcode(barcodeInfo))
