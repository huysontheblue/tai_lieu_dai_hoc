...
Public Class FrmLineSeriesType
    ...
    Sub New(ByVal deptId As String, ByVal lineId As String, ByVal isAdd As Boolean, ByVal isDelete As Boolean)
        _deptId = deptId
        _lineId = lineId
        _isAdd = isAdd
        _isDelete = isDelete
    ...
    Private Sub LoadRootNotes()
        ...
        Try
            'sql = "SELECT DISTINCT N'产品类型' as typename FROM m_PartSeriesType_t WHERE usey='Y'"


            sql = "SELECT DISTINCT N'系列别' as typename FROM m_PartSeriesType_t WHERE usey='Y'"
    ...
    Private Sub LoadSubNotes()
        ...
        Try
            'sql = " SELECT  PartSeriesTypeCode ,PartSeriesTypeName " & _
            '      " FROM m_PartSeriesType_t WHERE usey='Y' ORDER BY PartSeriesTypeCode"


            sql = "select SeriesID,SeriesName from m_Series_t where Usey='Y' ORDER BY SeriesID "
            Using dt As DataTable = sConn.GetDataTable(sql)
    ...
    Private Sub LoadLeftNotes()
        ...
        Try
            ...
            'o_strSQL = " SELECT  a.PartSeriesTypeCode, a.PartSeriesTypeName" & _
            '                    " FROM m_LineSericesType_t a " & _
            '                    " LEFT JOIN m_PartSeriesType_t b ON a.PartSeriesTypeCode = b.PartSeriesTypeCode " & _
            '                    " WHERE a.lineid='" & _lineId & "'" & _
            '                    " ORDER BY b.PartSeriesTypeCode"



            o_strSQL = " SELECT  a.PartSeriesTypeCode, a.PartSeriesTypeName" & _
                             " FROM m_LineSericesType_t a " & _
                             " LEFT JOIN m_Series_t b ON B.SeriesID = a.PartSeriesTypeCode " & _
                             " WHERE a.lineid='" & _lineId & "'" & _
                             " ORDER BY b.SeriesID"
            ...
            End Using
        Catch ex As Exception
            Throw New Exception("获取该线别的系列别失败" + ex.Message)
        End Try
...
#End Region
    ...
    Private Function WriteToDB(ByRef node As TreeNode) As Boolean
        ...
        strSql = " SELECT PartSeriesTypeCode FROM m_LineSericesType_t " & _
                 " WHERE Lineid='" & _lineId & "' AND PartSeriesTypeCode='" & node.Tag & "'"
        If sConn.GetRowsCount(strSql) > 0 Then
            MessageBox.Show(String.Format("线别:{0},产品类型{1}已经存在，请勿重复添加", _stationName, node.Tag))
            Return False
        End If

        o_sbSQL.Append("IF ISNULL((SELECT 1 FROM m_LineSericesType_t WHERE Lineid='" & _lineId & "' AND Deptid='" & _deptId & "' AND PartSeriesTypeCode='" & node.Tag & "'),0)<>1")
        o_sbSQL.Append(" BEGIN ")
        o_sbSQL.Append(" INSERT INTO m_LineSericesType_t(Deptid,Lineid,PartSeriesTypeCode,PartSeriesTypeName,INTIME,USERID) VALUES")
        o_sbSQL.Append(" ('" & _deptId & "','" & _lineId & "','" & node.Tag & "',N'" & node.Text & "',GETDATE(),'" & VbCommClass.VbCommClass.UseId & "')")
        o_sbSQL.Append(" END ELSE BEGIN ")
        o_sbSQL.Append(" UPDATE m_LineSericesType_t SET INTIME=GETDATE(),USERID='" & VbCommClass.VbCommClass.UseId & "'")
        o_sbSQL.Append("  WHERE Lineid='" & _lineId & "' AND PartSeriesTypeCode='" & node.Tag & "' AND Deptid='" & _deptId & "'")
        o_sbSQL.Append("  END")

        '       update deptlineD_t set typeid = ISNULL(typeid,'') + IIF(ISNULL(TYPEID,'')='','',',') +  'E04'
        'WHERE LINEID ='XT001'

        'o_sbSQL.Append(" UPDATE deptline_t SET PartSeriesTypeCode = ISNULL(PartSeriesTypeCode,'') + IIF(ISNULL(PartSeriesTypeCode,'')='','',',')+ '" & node.Tag & "', ")
        'o_sbSQL.Append(" PartSeriesTypeName = isnull(PartSeriesTypeName,'') + iif(isnull(PartSeriesTypeName,'')='','',',') + N'" & node.Text & "'")
    ...
    Private Function RemoveFromDB(ByRef node As TreeNode) As Boolean
        ...
        strSQL = " SELECT PartSeriesTypeCode FROM deptline_t " & _
        ...
        o_sbSQL.Append(" DELETE FROM m_LineSericesType_t  WHERE LineID='" & _lineId & "' AND PartSeriesTypeCode='" & node.Tag & "' AND Deptid='" & _deptId & "'")
        'o_sbSQL.Append(" UPDATE deptline_t set PartSeriesTypeCode = '" & o_strFinshTypeid & "', PartSeriesTypeName = N'" & & "' ")
        'o_sbSQL.Append(" WHERE LINEID ='" & _lineId & "'")
