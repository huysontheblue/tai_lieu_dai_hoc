...
Public Class FrmRPartStationCopy
...
#Region "事件"
    ...
    Private Sub btnCopy_Click(sender As Object, e As EventArgs) Handles btnCopy.Click
        ...
        Try
            ...
            Dim str As String = String.Format("你确定要复制旧料号{0}料件版本到新料号{1}吗?", oldPartId, newPartId)
            ...
            strSQL = " declare @strmsgid varchar(1),@strmsgText nvarchar(500) " &
                     " exec [Exec_CopyPartId] '{0}','{1}','{2}','{3}',@strmsgid output,@strmsgText output" &
            ...
            Dim fra As FrmRPartStation = Me.Owner

            If dt.Rows(0)(0).ToString = "1" Then
                fra.result = dt.Rows(0)(0).ToString
                fra.newPartIdm = txtPartId.Text.Trim
                Me.lblMessage.Text = dt.Rows(0)(0).ToString
                Me.lblMessage.Visible = True
                Me.Close()
            ElseIf dt.Rows(0)(0).ToString = "0" Then
                Me.lblMessage.Text = dt.Rows(0)(1).ToString
                Me.lblMessage.Visible = True
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "BasicManagement.FrmRPartStationCopy", "btnCopy_Click", "sys")
            Exit Sub
        End Try
    End Sub
    '回车键查询料号
    ...
    Private Sub dgvPartId_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvPartId.CellClick
        Try
            ...
            txtPartId.Text = Me.dgvPartId.Rows(Me.dgvPartId.CurrentRow.Index).Cells("TAvcPart").Value.ToString()
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "BasicManagement.FrmRPartStationCopy", "dgvPartId_CellClick", "sys")
            Exit Sub
        End Try
    End Sub
#End Region
...
#Region "方法"
    ...
    Private Sub LoadPartId()
        Dim strSQL As String
        Try
            strSQL = "select DISTINCT TAvcPart,PAvcPart,CusID from dbo.m_PartContrast_t where TavcPart=PavcPart and usey='Y'"

            If Me.txtPartId.Text.Trim <> "" Then
                strSQL = strSQL & " and TavcPart like '" & Me.txtPartId.Text.Trim & "%' "
            End If

            Dim dt As DataTable = DbOperateUtils.GetDataTable(strSQL)
            Me.dgvPartId.DataSource = dt
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "BasicManagement.FrmRPartStationCopy", " LoadPartId", "sys")
            Exit Sub
        End Try
    End Sub
#End Region
