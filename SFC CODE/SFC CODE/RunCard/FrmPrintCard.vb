...
'        'Dim sql As String = "SELECT DISTINCT A.PARTNUMBER FROM M_PARTNUMBER_T A,M_RUNCARD_T B,M_RUNCARDDETAIL_T C" & vbNewLine & _
'        '" WHERE A.ID=B.PARTID AND B.ID=C.RUNCARDID AND B.STATUS=1 AND C.STATUS=1  AND A.PARTNUMBER IN("
'        Dim sql As String = "SELECT DISTINCT A.PARTNUMBER FROM M_PARTNUMBER_T A,M_RUNCARD_T B,M_MAINMO_T C" & vbNewLine & _
...
'        Dim sqlConn As New MainFrame.SysDataHandle.SysDataBaseClass
'        Dim index As Integer = 0
'        Dim sql As String = "SELECT DISTINCT B.PARTNUMBER FROM M_PARTNUMBER_T A,M_PARTNUMBER_T B,M_BOM_T C " & vbNewLine & _
...
'    '有以下几种情况
'    '1.当MO为成套工单时，查看其下面的每个单根的料件是否存在M_PARTNUMBER_T,不存在,重新download（一般维护了流程卡就会有）主要防止先download工单在维护流程卡
...
'        Dim sql As String = "SELECT A.PARTID FROM M_MAINMO_T A LEFT JOIN M_PARTNUMBER_T B ON A.PARTID=B.PARTNUMBER WHERE A.PARENTMO='" & txtMo.Text & "' AND B.PARTNUMBER IS NULL "
...
'                    sql.Append(" DELETE FROM M_BOM_T WHERE PARENTPARTID IN(SELECT A.PARTID FROM M_BOM_T A,M_PARTNUMBER_T B WHERE B.PARTNUMBER='" & rpn & "' AND B.ID=A.PARENTPARTID)").Append(vbNewLine)
'                    sql.Append(" DELETE FROM M_BOM_T WHERE PARENTPARTID IN(SELECT ID FROM M_PARTNUMBER_T WHERE PARTNUMBER='" & rpn & "')").Append(vbNewLine)
'                    For i As Integer = 0 To dt.Rows.Count - 1
'                        sql.Append(" IF NOT EXISTS(SELECT 1 FROM M_PARTNUMBER_T WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ParentPartId).ToString & "') begin").Append(vbNewLine)
'                        sql.Append(" INSERT INTO M_PARTNUMBER_T(PARTNUMBER,DESCRIPTION,STATUS,USERID,INTIME,DESCRIPTION1,TYPE) VALUES('" & dt.Rows(i)(BomInfo.ParentPartId).ToString & "',").Append(vbNewLine)
...
'                        sql.Append(" ELSE BEGIN UPDATE M_PARTNUMBER_T SET DESCRIPTION=N'" & dt.Rows(i)(BomInfo.ParentDescription).ToString.Replace("'", "''") & "',DESCRIPTION1=N'" & dt.Rows(i)(BomInfo.Format).ToString.Replace("'", "''") & "' WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ParentPartId).ToString & "' END").Append(vbNewLine)
'                        sql.Append(" IF NOT EXISTS(SELECT 1 FROM M_PARTNUMBER_T WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ChildPartId).ToString & "') begin").Append(vbNewLine)
'                        sql.Append(" INSERT INTO M_PARTNUMBER_T(PARTNUMBER,DESCRIPTION,STATUS,USERID,INTIME,DESCRIPTION1,TYPE) VALUES('" & dt.Rows(i)(BomInfo.ChildPartId).ToString & "',").Append(vbNewLine)
...
'                        sql.Append(" ELSE BEGIN UPDATE M_PARTNUMBER_T SET DESCRIPTION=N'" & dt.Rows(i)(BomInfo.ChildDescription).ToString.Replace("'", "''") & "',DESCRIPTION1=N'" & dt.Rows(i)(BomInfo.CFormat).ToString.Replace("'", "''") & "' WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ChildPartId).ToString & "' END").Append(vbNewLine)
'                        sql.Append(" IF NOT EXISTS(SELECT 1 FROM M_BOM_T WHERE PARENTPARTID=(SELECT TOP 1 ID FROM M_PARTNUMBER_T WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ParentPartId).ToString & "') AND PARTID=(SELECT TOP 1 ID FROM M_PARTNUMBER_T WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ChildPartId).ToString & "')) begin").Append(vbNewLine)
...
'                        sql.Append(" VALUES((SELECT TOP 1 ID FROM M_PARTNUMBER_T WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ParentPartId).ToString & "'),").Append(vbNewLine)
'                        sql.Append("(SELECT TOP 1 ID FROM M_PARTNUMBER_T WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ChildPartId).ToString & "'),'1','" & VbCommClass.VbCommClass.UseId & "',getdate(),'" & dt.Rows(i)(BomInfo.Extensible).ToString & "','" & dt.Rows(i)(BomInfo.Qty).ToString & "'").Append(vbNewLine)
...
'                        sql.Append(" ELSE BEGIN UPDATE M_BOM_T SET QTY='" & dt.Rows(i)(BomInfo.Qty).ToString & "',EXTENSIBLE='" & dt.Rows(i)(BomInfo.Extensible).ToString & "'").Append(vbNewLine)
...
'                        sql.Append(" WHERE PARENTPARTID=(SELECT TOP 1 ID FROM M_PARTNUMBER_T WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ParentPartId).ToString & "') ").Append(vbNewLine)
'                        sql.Append(" AND PARTID=(SELECT TOP 1 ID FROM M_PARTNUMBER_T WHERE PARTNUMBER='" & dt.Rows(i)(BomInfo.ChildPartId).ToString & "') end").Append(vbNewLine)
'                    Next
'                End If
'            End Using
...
'        Try
'            oConn = New OracleDb("")
'            Using dt As DataTable = oConn.ExecuteQuery(GetSql(rpn)).Tables(0)
'                Return dt
'            End Using
'        Catch ex As Exception
'            Throw ex
...
'        "START WITH A.BMB01='" & rpn & "') A,IMA_FILE B,IMA_FILE C " & _
...
Public Class FrmPrintCard
    ...
    Private Function IsInputMoEqualParentMo() As Boolean
        Dim sqlConn As New MainFrame.SysDataHandle.SysDataBaseClass
        Dim sqlString As String = "SELECT MOID FROM M_RUNCARDWORKORDER_T WHERE MOID='" & txtMo.Text.Trim & "' AND MOID=PARENTMO"
        If sqlConn.GetRowsCount(sqlString) = 1 Then
            Return True
        End If
        Return False
    ...
    Private Function CheckNeedReloadBom() As Boolean
        Dim sConn As SysDataBaseClass = New SysDataBaseClass
        Dim sql As String = "SELECT A.PARTID FROM M_RUNCARDWORKORDER_T A LEFT JOIN M_RUNCARDPARTNUMBER_T B ON A.PARTID=B.PARTNUMBER WHERE A.PARENTMO='" & txtMo.Text & "' AND B.PARTNUMBER IS NULL "
        Return sConn.GetRowsCount(sql) > 0
    End Function
