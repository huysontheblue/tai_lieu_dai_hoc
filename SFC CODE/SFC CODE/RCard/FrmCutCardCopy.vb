...
#Region "事件"
    Private Sub FrmCutCardCopy_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '初始化
        Inite()
    End Sub
    ...
    Private Sub btnOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOK.Click
        ...
        'cq 20160722
        '当复制裁线卡时，若里面存在同一个料号流程卡时，点击确定，提示“里面已有此版本的料号，是否替换”，
        ...
        If PnAlreadyExists() Then
            If MessageUtils.ShowConfirm("里面已有此版本的料号，是否替换?", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                'do next
                lsSQL = CutCardBusiness.DeleteCutHeader(False, Me.txtNewPart.Text.Trim)

                RCardComBusiness.ExecSQL(lsSQL)
            Else
                Me.lblMsg.Text = "新料号裁线卡已经存在"
                Exit Sub
            End If
        End If
        ...
        Try
            ...
            CutCardBusiness.udpCopyCutCard(arrayList)

            MessageUtils.ShowInformation("复制成功！")
            Me.DialogResult = Windows.Forms.DialogResult.OK
            Me.Close()
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmRunCardCutCopy", "btnOK_Click", "RCard")
        End Try
    End Sub
...
#Region "方法"
    ...
    Private Function PnAlreadyExists() As Boolean
        Dim sql As String = "SELECT PartID FROM m_RCardCutM_t WHERE PartID='" & Me.txtNewPart.Text.Trim & "'" &
    ...
    Private Function CheckVersionAndDownBOM(ByVal parOldPN As String) As Boolean
        ...
        Try
            ...
            Return True
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmRunCardCutCopy", "CheckVersionAndDownBOM", "RCard")
            Throw ex
        End Try
    End Function
    ...
    Private Function CheckExistOldVersion(ByVal parNewPN As String) As Boolean
        ...
        Try
            ...
            If CutCardBusiness.CutCardExistInOldData(parNewPN, Me.txtVersion.Text) Then
                If MessageUtils.ShowConfirm("旧裁线卡里面已有此版本的料号，是否替换?", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                    'do next
                    lsSQL = RunCardBusiness.DeleteOldRCard(parNewPN, Me.txtVersion.Text)
                    RCardComBusiness.ExecSQL(lsSQL)
                Else
                    Me.lblMsg.Text = "旧裁线卡里面已经存在这个版本！！"
                    Return False
                End If
            End If
            ...
            Return True
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmRunCardCutCopy", "CheckExistOldVersion", "RCard")
            Throw ex
        End Try
    End Function
...
#Region "下载BOM"
    Private Function DownloadBom(ByVal pn As String, ByRef msg As String, ByVal parOldPN As String) As Boolean
        ...
        Try
            ...
            Else
                strSQL = SapCommon.GetErpFilterSqlUnion(pn)  'cq 20170206
                dt = RCardComBusiness.GetDataTableOracle(strSQL)
            End If
