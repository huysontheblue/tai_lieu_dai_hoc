...
namespace DBUtility
{
    ...
    public abstract class DbOracleHelperSQL
    {
        //数据库连接字符串(web.config来配置)，也可以动态更改connectionString支持多数据库.	;	
        //public static string connectionString = "Host=topscan.luxshare.com.cn;Port=1521;Service Name=TOPPROD;User ID=MESDG;Password=DGMES;";
        public static string connectionString ;

        public DbOracleHelperSQL() { }

        #region 公用方法

        public static bool Exists(string strSql)
        {
            object obj = DbOracleHelperSQL.GetSingle(strSql);
            int cmdresult;
        ...
        {
            object obj = DbOracleHelperSQL.GetSingle(strSql, cmdParms);
            int cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
                cmdresult = 0;
        ...
        /// <param name="trans">OracleTransaction事件</param>
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        ...
        public static int ExecuteSqlTran(List<OracleCommandInfo> list, List<OracleCommandInfo> oracleCmdSqlList)
        {
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                ...
                try
                {
                    foreach (OracleCommandInfo myDE in list)
                    {
        ...
        /// </summary>
        /// <param name="connection">OracleConnection对象</param>
        /// <param name="trans">OracleTransaction事务</param>
        /// <param name="SQLString">SQL语句</param>
        /// <returns>DataSet</returns>
        ...
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        public static int ExecuteSql(OracleConnection connection, OracleTransaction trans, string SQLString, params OracleParameter[] cmdParms)
        {
            using (OracleCommand cmd = new OracleCommand())
            {
                try
        ...
        //</summary>
        //<param name="SQLStringList">SQL语句的哈希表（key为sql语句，value是该语句的OracleParameter[]）</param>
        public static int ExecuteSqlTran(System.Collections.Generic.List<OracleCommandInfo> cmdList)
        {
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                conn.Open();
                using (OracleTransaction trans = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        int count = 0;
                        //循环
                        foreach (OracleCommandInfo myDE in cmdList)
                        {
                            string cmdText = myDE.CommandText;
                            OracleParameter[] cmdParms = (OracleParameter[])myDE.Parameters;
                            PrepareCommand(cmd, conn, trans, cmdText, cmdParms);
        ...
        /// </summary>
        /// <param name="SQLStringList">SQL语句的哈希表（key为sql语句，value是该语句的OracleParameter[]）</param>
        public static void ExecuteSqlTranWithIndentity(System.Collections.Generic.List<OracleCommandInfo> SQLStringList)
        {
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                conn.Open();
                using (OracleTransaction trans = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        int indentity = 0;
                        //循环
                        foreach (OracleCommandInfo myDE in SQLStringList)
                        {
                            string cmdText = myDE.CommandText;
                            OracleParameter[] cmdParms = (OracleParameter[])myDE.Parameters;
        ...
        /// <param name="SQLString">计算查询结果语句</param>
        /// <returns>查询结果（object）</returns>
        public static object GetSingle(OracleConnection connection, OracleTransaction trans, string SQLString, params OracleParameter[] cmdParms)
        {
            using (OracleCommand cmd = new OracleCommand())
            {
                try
        ...
        /// <param name="SQLString">查询语句</param>
        /// <returns>DataSet</returns>
        public static DataSet Query(OracleConnection connection, OracleTransaction trans, string SQLString, params OracleParameter[] cmdParms)
        {
            OracleCommand cmd = new OracleCommand();
            PrepareCommand(cmd, connection, trans, SQLString, cmdParms);
            using (OracleDataAdapter da = new OracleDataAdapter(cmd))
        ...
        private static OracleCommand BuildIntCommand(OracleConnection connection, string storedProcName, IDataParameter[] parameters)
        {
            ...
            command.Parameters.Add(new OracleParameter("ReturnValue",
                OracleDbType.Int16, 4, ParameterDirection.ReturnValue,
