...
Public Class FrmRunCardImportStation
    ...
    Public filePath As String = _sopFilePath.Substring(0, _sopFilePath.Length - 8) + "RunCard\RunCard_MO_Template.xlsm"  'Environment.CurrentDirectory & "\Templates" & "\RunCardTemplate.xlsx"
...
#Region "导出"
    ...
    Public Function GetExportSql(Optional ByVal runCardPn As String = "") As String
        Dim sql As String = Nothing
        'Modify by cq 20160104,modify audio username
        ' AND PART.TAvcPart = PART.PAvcPart, cq 20160510
        ' add AND  PARTD.PAvcPart = '904092886-02' ,AND PARTD.PAvcPart='" & runCardPn & "'" ,cq 20160627 
        ' remove  "  AND PARTD.Userid <> 'SYSTEM'" , cq 20160804, //  DETAIL.PROCESSSTANDARD + IIf(ISNULL(DETAIL.REMARK,'')='', '', '('+ DETAIL.REMARK + ')') STANDARD
        ' IIf(isnull(DETAIL.PROCESSSTANDARDPrint,'') ='', DETAIL.PROCESSSTANDARD,DETAIL.PROCESSSTANDARDPrint)
        sql = "SELECT PartID," &
        "       DRAWINGVER," &
        "       SHAPE," &
        "       ID AS SEQ," &
        ...
        "       IIf(isnull(DETAIL.PROCESSSTANDARD,'') ='',DETAIL.PROCESSSTANDARDPrint,DETAIL.PROCESSSTANDARD)+ IIf(ISNULL(DETAIL.REMARK,'')='', '', '('+ DETAIL.REMARK + ')') STANDARD," & _
        ...
        "      FROM m_PartContrast_t PART,m_RCardM_t HEADER,m_RCardD_t DETAIL, m_Rstation_t STATION" &
    ...
    Public Function GetVariables(ByVal dt As DataTable, Optional ByVal o_stcMO As MOComBusiness.stcMO = Nothing) As System.Collections.Generic.Dictionary(Of String, String)
        ...
        If dt.Rows.Count > 0 Then
            ...
            If VbCommClass.VbCommClass.IsConSap = "Y" Then
                strCustVer = SapCommon.GetVerFromTiptopSap(strPN)
            Else
                strCustVer = SapCommon.GetVerFromTiptop(strPN)
            End If
            'cq 20160921
            'dics.Add("VERSION", dt.Rows(0)("DRAWINGVER").ToString)
            dics.Add("VERSION", strCustVer)
            ...
            If dt.Rows(0)("DESCRIPTION").ToString.IndexOf("日本市场") >= 0 Or
               dt.Rows(0)("TypeDest").ToString.IndexOf("日本市场") >= 0 Then
                strArea.Append("(日本市场)")
            End If
            If dt.Rows(0)("DESCRIPTION").ToString.IndexOf("贴UL标签") >= 0 Or
               dt.Rows(0)("TypeDest").ToString.IndexOf("贴UL标签") >= 0 Then
