...
Public Class FrmShowRunCard
    ...
    Public m_iDirection As String = Nothing
...
#Region "Main Function "
    ...
    Private Sub FrmShowRunCard_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
        InitPrinter()
        Me.lblMsg.Text = String.Empty  'Add by CQ 20151204
    End Sub
...
#Region "Show Run Card"
    ...
    Private Function GetRunCardSql() As String
        Dim sql =
            ...
            "   WHERE PART.TAvcPart='" & pn & "'  " &
            MOComBusiness.GetFatoryProfitcenter("HEADER") &
            "   AND HEADER.PARTID =PART.TAvcPart   " &
            "   AND HEADER.PARTID=DETAIL.PARTID   " &
            "   AND DETAIL.STATIONID =STATION.Stationid   " &
            "   )A)A ORDER BY A.ID "
        Return sql
    End Function
...
#Region "Show Run Card Detail List"
    ...
    Public Function GetRunCardDetailListDataTable() As DataTable
        Using dt As DataTable = DbOperateUtils.GetDataTable(GetRunCardDetailListSql())
            'for test
    ...
    Private Function GetRunCardDetailListSql() As String
        Dim sql =
            ...
            "  Convert(decimal(18,2), SUM(A.PREASSEMBLYHOURPRECHILD+A.ASSEMBLYHOURPRECHILD+A.MADEHOURPRECHILD)) TOTALHOURPRECHILD,  " &
            "  Convert(decimal(18,2),SUM(A.PREASSEMBLYHOURPREMO+A.ASSEMBLYHOURPREMO+A.MADEHOURPREMO)) TOTALHOURPREMO,  " &
            " Convert(decimal(18,2),SUM(A.PREASSEMBLYHOURPRECHILD)) PREASSEMBLYHOURPRECHILD,  " &
            " Convert(decimal(18,2),SUM(A.ASSEMBLYHOURPRECHILD)) ASSEMBLYHOURPRECHILD,  " &
            " SUM(A.MADEHOURPRECHILD) MADEHOURPRECHILD,  " &
            " Convert(decimal(18,2),SUM(A.PREASSEMBLYHOURPREMO)) PREASSEMBLYHOURPREMO,  " &
            " SUM(A.ASSEMBLYHOURPREMO) ASSEMBLYHOURPREMO,  " &
            " SUM(A.MADEHOURPREMO)  MADEHOURPREMO  " &
            " FROM (  " &
            " SELECT DISTINCT D.TAvcPart PN,B.StationSQ,A.DRAWINGVER VERSION,ISNULL(D.AmountQty,0) QTY ,  " &
            "            CASE F.ID  " &
            " WHEN 1 THEN ISNULL(B.WorkingHours,0)*ISNULL(D.AmountQty,0)  " &
            " ELSE 0 END PREASSEMBLYHOURPRECHILD,  " &
            "            CASE F.ID  " &
            " WHEN 2 THEN ISNULL(B.WorkingHours,0)*ISNULL(D.AmountQty,0)  " &
            " ELSE 0 END ASSEMBLYHOURPRECHILD,  " &
            "             CASE F.ID  " &
            " WHEN 3 THEN ISNULL(B.WorkingHours,0)*ISNULL(D.AmountQty,0)  " &
            " ELSE 0 END MADEHOURPRECHILD,  " &
            "             CASE F.ID  " &
            " WHEN 1 THEN ISNULL(B.WorkingHours,0)*ISNULL(D.AmountQty,0)    " &
            " ELSE 0 END PREASSEMBLYHOURPREMO,  " &
            "             CASE F.ID  " &
            " WHEN 2 THEN ISNULL(B.WorkingHours,0)*ISNULL(D.AmountQty,0)    " &
            " ELSE 0 END ASSEMBLYHOURPREMO,  " &
            "             CASE F.ID  " &
            " WHEN 3 THEN ISNULL(B.WorkingHours,0)*ISNULL(D.AmountQty,0)    " &
            " ELSE 0 END MADEHOURPREMO  " &
            " FROM m_RCardM_t A,m_RCardD_t B,m_PartContrast_t M,m_PartContrast_t D,  " &
...
#Region "Show Process Card"
    ...
    Private Function GetProcessCardSql() As String
        ...
        sql =
            ...
            MOComBusiness.GetFatoryProfitcenter("B") &
...
#Region "Print"
    ...
    Private Sub Print(ByVal o_filePath As String)
        ...
        Try
            ...
            xlWorkSheet = xlWorkBook.Sheets(1) '1 
            xlApp.Visible = False
            xlWorkSheet.PageSetup.PaperSize = Microsoft.Office.Interop.Excel.XlPaperSize.xlPaperA4 'xlPaperA4
            xlWorkSheet.PageSetup.LeftMargin = 27 '左边距
