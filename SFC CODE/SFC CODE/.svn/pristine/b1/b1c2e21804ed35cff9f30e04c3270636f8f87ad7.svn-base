...
#Region "事件"
    ...
    Private Sub toolScanSet_Click(sender As Object, e As EventArgs) Handles toolScanSet.Click
        ...
        If GetIsScanFinish(ScanOption.SelMOID) = True Then
            ...
            SetMessage("FAIL", "该工单全部扫描完成 Công đơn này đã quét hoàn thành！")
            Exit Sub
        End If
        txt_tmpbarcode = ""
        ...
        If errDt.Rows.Count > 0 Then
            ...
            SetMessage("PASS", "已解锁，请继续进行作业 Đã mở khóa, tiếp tục thao tác")
    ...
    Private Sub ScanInternalPack(ByVal level As String)
        ...
        If level = "3" Then
            If CheckStyle(txtThird.Text.Trim, "内包装") = False Then
                WorkStantOption.ErrorStr = "三层包装样式不匹配 Quy cách đóng gói 3 tầng không tương thích！"
        ...
        If level = "2" Then
            If CheckStyle(txtSecond.Text.Trim, "内包装") = False Then
                WorkStantOption.ErrorStr = "二层包装样式不匹配 Quy cách đóng gói 2 tầng không tương thích！"
                SetMessage("FAIL", WorkStantOption.ErrorStr)
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                Select Case dt.Rows(0)(0).ToString()
                    ...
                    Case "4"
                        ...
                        If level = 2 Then
                            txtSecond.Text = dt.Rows(0)("NewCartonid").ToString
                            SetMessage("PASS", "二层外箱条码序号，扫描成功 Quét thành công thứ tự mã thùng 2 tầng ...")
                        Else
                            txtThird.Text = dt.Rows(0)("NewCartonid").ToString
                            SetMessage("PASS", "三层外箱条码序号，扫描成功 Quét thành công thứ tự mã thùng 3 tầng...")
                        End If
            ...
            Else
                ...
                'Me.TxtCartonID.Text = ""
                'PlaySimpleSound(1)
                WorkStantOption.ErrorStr = "系统无法识别此外箱标签序号 Hệ thống không thể nhận biết mã của thùng ngoài này  "
                SetMessage("Fail", "系统无法识别此外箱标签序号 Hệ thống không thể nhận biết mã của nhãn thùng này")
        ...
        Catch ex As Exception
            MessageUtils.ShowInformation("有异常发生，请联系管理员!异常信息 Phát sinh lỗi, Vui lòng liên hệ quản trị viên：" + ex.ToString)
    ...
    Private Sub txtPpid_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtPpid.KeyPress
        If e.KeyChar = Chr(13) Then
            ...
            If Me.txtPpid.Text.Length <> LTrim(Me.txtPpid.Text).Length Then
                ...
                WorkStantOption.ErrorStr = "ERROR,左端有空格 Bên trái có dấu cách trống！"
                SetMessage("Fail", "ERROR,左端有空格 Bên trái có dấu cách trống")
            ...
            If Me.txtPpid.Text.Length <> RTrim(Me.txtPpid.Text).Length Then
                ...
                WorkStantOption.ErrorStr = "ERROR,右端有空格 Bên phải có dấu cách trống！"
                SetMessage("Fail", "ERROR,右端有空格 Bên phải có dấu cách trống")
                WorkStantOption.BarCodeStr = txtPpid.Text
                WorkStantOption.vMainBarCode = Me.txtPpid.Text
                ShowFrmScanErrPrompt()
                SetMessage("PASS", "已解锁，请继续进行作业 Đã mở khóa, tiếp tục thao tác")
                ...
                If dt.Rows.Count > 0 Then
                    Select Case dt.Rows(0)(0).ToString()
                        Case "0", "1", "2", "3", "5"
                            ...
                            Exit Sub
                        Case 6
                            SetMessage("PASS", "产品条码 Quét thành công Mã sản phẩm:  " + Trim(ppid) + "扫描成功 ！")
                        ...
                        Case 8 '最外层大外箱装箱完成
                            ...
                            SetMessage("PASS", " Quét thành công mã sản phẩm 产品条码: " + Trim(ppid) + "扫描成功！装箱完毕，请扫描下一个外箱条码 Đóng gói hoàn thành, Tiếp tục quét mã thùng sau")
                            If ScanOption.IsOnelinePrintCarton Then
                                ...
                                If str <> "" Then
                                    MessageBox.Show("在线打印失败,请重新打印 In trực tiếp thất bại, Vui lòng In lại !")
                                    Exit Sub
                                End If
                            End If
                    End Select
    ...
    Private Sub toolRePrint_Click(sender As Object, e As EventArgs) Handles toolRePrint.Click
        ...
        If lblPrint.Text = "" Then
            SetMessage("FAIL", "找不到默认打印机 Không tìm thấy máy in ")
            Return
        End If
        If lblmoid.Text.Trim = "" Then
            SetMessage("FAIL", "请先扫描设置 Vui lòng cài đặt quét mã ！")
            Return
        End If
...
#Region "方法"
    Private Sub ScanCarton()
        ...
        If CheckCartonIndex = 0 Then
            ...
            If Me.chkIsHWASN.Checked = True Then
                ...
                ElseIf dtHWASN.Rows(0)(2).ToString() = "2" Then '非S19二维码条码
                    m_strQRCode = txtCarton.Text.Trim
                    SetMessage("PASS", "请继续刷入19条码 Tiếp tục nhập vào 19 mã")
                    Me.txtCarton.Text = ""
                    Me.txtCarton.Focus()
                    Exit Sub
            ...
            If CheckStyle(txtCarton.Text.Trim, "外箱条码") = False Then
                WorkStantOption.ErrorStr = "外箱条码样式不匹配 QUy cách mã thùng ngoài không tương thích！"
                SetMessage("FAIL", WorkStantOption.ErrorStr)
                'PlaySimpleSound(1)
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                Select Case dt.Rows(0)(0).ToString()
                    ...
                    Case "4"
                        ...
                        SetMessage("PASS", "外箱条码序号，扫描成功 Quét thành công mã thùng ngoài...")
                        LoadPackingData()
                        If ScanOption.IsScanThirdCode Then
            ...
            Else
                ...
                'PlaySimpleSound(1)
                WorkStantOption.ErrorStr = "系统无法识别此外箱标签序号 Hệ thống không thể nhận biết mã của thùng ngoài này"
                SetMessage("Fail", "系统无法识别此外箱标签序号 Hệ thống không thể nhận biết mã của thùng ngoài này")
                WorkStantOption.BarCodeStr = txtCarton.Text
                WorkStantOption.vMainBarCode = Me.txtCarton.Text
                ShowFrmScanErrPrompt()
                SetMessage("PASS", "已解锁，请继续进行作业 Đã mở khóa, tiếp tục thao tác")
            ...
            End If
        Catch ex As Exception
            MessageUtils.ShowInformation("有异常发生，请联系管理员!异常信息 Phát sinh lỗi hệ thống, liên hệ người phụ trách MES：" + ex.ToString)
            Dim errMsg As Exception = New Exception(String.Format("MOID:{0}#BCode:{1}#{2}", Me.lblmoid.Text.Trim, txtCarton.Text.Trim, ex.ToString))
            SysMessageClass.WriteErrLog(errMsg, "BarCodeScan.FrmInCarton", "CartonIDScanHandle", "sys")
            Exit Sub
        End Try
    ...
    Private Sub ControlCartonState(ByVal ScanIndex As String)
        If ScanIndex = "0" Then
            ...
            lblResult.Text = "请扫描最外层外箱条码 Vui lòng quét mã vạch thùng carton ngoài cùng"
            lblMessage.Text = "开始扫描 bắt đầu quét mã ..."
            lblResult.ForeColor = Color.DarkGreen
            lblMessage.ForeColor = Color.Green
            'yen them 
            'ScanCarton()
            'txtCarton.Focus()
            txtPpid.Focus()    'yen them 
            ...
            If Outer_box.Checked = True Then
                ...
                If txtStyle.Text.Trim() <> "" Then
                    ...
                    ' Me.txtCarton.Text = OnlineCartonID() yen sua 
                    'ScanCarton()
                    If txtCarton.Text.Trim = "" Then
                        Exit Sub
                    End If
        ...
        ElseIf ScanIndex = "2" Then
            ...
            txtPpid.Enabled = False
            txtPpid.ReadOnly = True
            lblResult.Text = "请扫描二层外箱条码 Vui lòng quét mã vạch trên hộp bên ngoài tầng 2"
            lblMessage.Text = "开始扫描 Bắt đầu quét..."
            If Tray.Checked = True Then
                If txtStyle.Text.Trim() <> "" And txtSecond.Text = "" Then
                    txtSecond.Text = txtStyle.Text.Trim()
        ...
        ElseIf ScanIndex = "3" Then
            ...
            txtPpid.Enabled = False
            txtPpid.ReadOnly = True
            lblResult.Text = "请扫描三层外箱条码 Vui lòng quét mã vạch trên hộp bên ngoài tầng 3"
            lblMessage.Text = "开始扫描 Bát đầu quét..."
        ElseIf ScanIndex = "4" Then '产品条码
            ...
            txtPpid.ReadOnly = False
            Me.txtPpid.Focus()
            lblResult.Text = "请扫描产品条码 Vui lòng quét mã sản phẩm"
            lblMessage.Text = "开始扫描 Bắt đầu quét..."
        End If
    End Sub
    ...
    Private Function CheckStyle(ByRef BarCode As String, Optional ByVal BarcodeType As String = "条码") As Boolean
        ...
        If txtStyle.Text.Trim.Length = 0 Then
            SetMessage("Fail", "扫描的 Mã Quét:  " + BarcodeType + "样式不能为空 quy cách không được để trống！")
            Return False
        Else
            If ScanOption.IsOnlineGenCartonID = False Then
                If BarCode.Trim.Length <> txtStyle.Text.Length Then
                    SetMessage("Fail", "扫描的 Mã Quét: " + BarcodeType + "长度不符合標準樣式长度 Độ dài không phù hợp độ dài tiêu chuẩn ")
                    Return False
                End If
                If TextHandle.verfyBarCodeStyle(txtStyle.Text, BarCode, txtStyle.Text) = False Then
                    SetMessage("FAIL", "扫描的 Mã Quét:" + BarcodeType + "不符合標準樣式 không phù hợp quy cách tiêu chuẩn")
                    Return False
                End If
            End If
            End If
    ...
    Private Function CheckStyleC(ByRef BarCode As String) As Boolean
        ...
        If ScanOption.vCartonRepeatPara = "Y" And Me.txtStyle.Text.Trim.Length <> 0 Then
            If BarCode.Trim.Length <> txtStyle.Text.Length Then
                WorkStantOption.ErrorStr = "扫描箱条码长度不对 Độ dài mã thùng không đúng "
                SetMessage("Fail", "扫描箱条码长度不对 Độ dài mã thùng không đúng")
            ...
            If TextHandle.verfyBarCodeStyle(txtStyle.Text, BarCode, txtStyle.Text) = False Then
                WorkStantOption.ErrorStr = "條碼不符合標準樣式 Mã quét không phù hợp quy cách mã tiêu chuẩn "
                SetMessage("FAIL", "條碼不符合標準樣式 Mã quét không phù hợp quy cách mã tiêu chuẩn")
                'PlaySimpleSound(1)
                WorkStantOption.BarCodeStr = BarCode
                WorkStantOption.vMainBarCode = BarCode
                ShowFrmScanErrPrompt()
        ...
        If ScanOption.vCartonRepeatPara = "Y" Then
            If txt_tmpbarcode = "" Then
                ...
                SetMessage("PASS", BarCode & Space(3) & "扫描成品箱条码成功 ，请继续扫描箱校验条码！ Quét mã thùng thành phẩm thành công, Vui lòng tiếp tục quét mã vạch xác minh hộp")
                txtCarton.Clear()
                txtCarton.Focus()
                Return False
            Else
                If txtCarton.Text <> txt_tmpbarcode Then
                    SetMessage("FAIL", "校验码 Mã xác minh" & "{" & BarCode & "}校验不成功...Fail,请重新验证 Xác minh không thành công, Tiếp tục xác minh lại ")
                    Return False
                End If
            End If
            txt_tmpbarcode = ""
        ...
        If ScanOption.vCartonRepeatStyle = "Y" Then
            If txt_tmpbarcode = "" Then
                ...
                SetMessage("PASS", BarCode & Space(3) & "扫描箱条码成功，请继续扫描校验条码 Quét mã thành công, Vui lòng tiếp tục quét mã xác minh！")
            ...
            Else
                ...
                If BarCode.Length = str.Length AndAlso TextHandle.verfyBarCodeStyle(str, txtCarton.Text, str) Then
                    If styles.Length > 1 Then
                        ...
                        If styles(1) = "10" Or styles(1) = "11" Then
                            If iCartonStartindex + iCartonStartLength <> txt_tmpbarcode Then
                                SetFailContent(BarCode, String.Format("请检查料件打印参数料号[{0}]设置参数代码对应的参数值的正确性 Vui lòng kiểm tra tính chính xác của giá trị tham số tương ứng với mã tham số của vật liệu tham số in mã liệu [{0}] ", lblPartID.Text))
                                Return False
                            End If
                            ...
                            If ppidssss <> checkssss Then
                                SetFailContent(BarCode, String.Format("校验码{0}[{1}]流水[{2}]校验不成功...Fail,请重新验证 gửi xác minh không thành công, Gửi lại thông tin xác minh", CheckCartonCurIndex + 1, BarCode, checkssss))
                                Return False
                            End If
                        End If
                        '要求验证唯一性 保存到数据库中
                        If styles(1) = "01" Or styles(1) = "11" Then
                            If CheckppidIsRepeat(BarCode) = False Then
                                SetFailContent(BarCode, String.Format("校验码{0}[{1}]流水码重复扫描...Fail,请重新验证 Trùng lặp mã, Vui lòng xác minh lại ", CheckCartonCurIndex + 1, BarCode))
                                Return False
                            End If
                            '成功验证插入到表中
                        ...
                        End If
                    End If
                    SetMessage("PASS", "校验码" & CheckCartonCurIndex + 1 & "{" & BarCode & "}校验成功 Xác minh thành công，请继续扫描下一个校验条码 Tiếp tục quét xác minh mã tiếp theo！")
                    txtCarton.Clear()
                    txtCarton.Focus()
                    ...
                    End If
                Else
                    SetFailContent(BarCode, "校验码" & CheckCartonCurIndex + 1 & "{" & BarCode & "}校验不成功 Xác minh không thành công...Fail,请重新验证 vui lòng xác minh lại ")
                    Return False
                End If
            End If
            CheckCartonCurIndex = CheckCartonCurIndex + 1
    ...
    Private Function CheckStyle(ByRef BarCode As String) As Boolean
        ...
        '扫描条码样式不能为空
        If txtStyle.Text.Trim.Length = 0 Then
            WorkStantOption.ErrorStr = "扫描条码样式不能为空 Không được để trống kiểu quét mã vạch！"
            SetMessage("Fail", "扫描条码样式不能为空 Không được để trống kiểu quét mã vạch！")
            WorkStantOption.BarCodeStr = BarCode
            WorkStantOption.vMainBarCode = BarCode
            ShowFrmScanErrPrompt()
            txtPpid.Text = ""
        ...
        If txtStyle.Text.Trim.Length <> 0 Then
            If BarCode.Trim.Length <> txtStyle.Text.Length Then
                WorkStantOption.ErrorStr = "扫描条码长度不对 Độ dài mã quét không đúng "
                SetMessage("Fail", "扫描条码长度不对 Độ dài mã quét không đúng")
                WorkStantOption.BarCodeStr = BarCode
                WorkStantOption.vMainBarCode = BarCode
                ShowFrmScanErrPrompt()
                txtPpid.Text = ""
        ...
        If ScanOption.vRepeatStyle = "Y" Then
            If txt_tmpbarcode = "" Then
                ...
                SetMessage("PASS", BarCode & Space(3) & "扫描成品条码成功，请继续扫描校验条码 Quét mã sản phẩm thành công, tiếp tục quét mã xác minh！")
                ...
                If BarCode.Length = str.Length AndAlso TextHandle.verfyBarCodeStyle(str, txtPpid.Text, str) Then
                    If styles.Length > 1 Then
                        ...
                        If styles(1) = "10" Or styles(1) = "11" Then
                            ...
                            '流水不一致
                            If ppidssss <> checkssss Then
                                SetFailContent(BarCode, String.Format("校验码{0}[{1}]流水[{2}]校验不成功...Fail,请重新验证 Gửi xác minh không thành công, Gửi lại thông tin xác min", CheckCurIndex + 1, BarCode, checkssss))
                                Return False
                            End If
                        End If
                        '要求验证唯一性 保存到数据库中
                        If styles(1) = "01" Or styles(1) = "11" Then
                            If CheckppidIsRepeat(BarCode) = False Then
                                SetFailContent(BarCode, String.Format("校验码{0}[{1}]流水码重复扫描...Fail,请重新验证 Trùng lặp mã, Vui lòng xác minh lại", CheckCurIndex + 1, BarCode))
                                Return False
                            End If
                            '成功验证插入到表中
                            InsertAssysnCheck(BarCode, txt_tmpbarcode)
                        End If
                    End If
                    SetMessage("PASS", "校验码" & CheckCurIndex + 1 & "{" & BarCode & "}校验成功，请继续扫描下一个校验条码 Xác minh thành công, Tiếp tục quét xác minh mã tiếp theo！")
                    txtPpid.Clear()
                    txtPpid.Focus()
                    ...
                    End If
                Else
                    SetFailContent(BarCode, "校验码" & CheckCurIndex + 1 & "{" & BarCode & "}校验不成功 Xác minh không thành công...Fail,请重新验证 Vui lòng xác minh lại")
                    Return False
                End If
            End If
            CheckCurIndex = CheckCurIndex + 1
    ...
    Private Function OnlineCartonID() As String
        ...
        Else
            If PackBarCode.m_CartonID = "00000000" Then
                ...
                SetMessage("PASS", "该工单已扫描完成 Công đơn này đã quét hoàn thành！")
            End If
        End If
    End Function
