...
Public Class FrmCutCardImport

    Public runCardPartId As String
    Public operation As String
    Private _isQueryOldVersion As Boolean
    Public m_RuncardList As String
    Private _sopFilePath As String = VbCommClass.VbCommClass.SopFilePath
    Public filePath As String = _sopFilePath.Substring(0, _sopFilePath.Length - 8) + "RunCard\CutCard_MO_Template.xlsx"
    ...
    Public Enum RunCardUpload
        ...
        PARTNUMBER
    End Enum
    Private _getVariables As Dictionary(Of String, String)
...
#Region "事件"
    ...
    Private Sub FrmRunCardImportStation_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        lsSQL = " SELECT TOP 100 cusid,cusname from m_Customer_t WHERE usey='Y' " & _
...
#Region "导入"
        ...
        Try
            If InsertIntoTempTable(dt, file) Then
                ...
                If Not CheckStationStand(errorMsg) Then
                    ...
                    errorMsgs.Append(file & "--> " & errorMsg.Trim(",") + "不是标准工站,或是已经作废!")
                    Return False
                End If
    ...
    Private Function PnExists() As Boolean
        Dim sql As String = "SELECT PartID FROM m_RCardM_t WHERE PartID = '" & pn & "' " &
                             MOComBusiness.GetFatoryProfitcenter()

        Using dt As DataTable = MoComDB.GetDataTable(sql)
    ...
    Private Function PnDownLoadBom(ByVal file As String) As Boolean
        ...
        If VbCommClass.VbCommClass.IsConSap = "N" Then
            'Get CustID from TT first, if not get, then get from UI
            strCustID = SapCommon.GetCustIDFromTT(pn)
        End If
        ...
        If VbCommClass.VbCommClass.IsConSap = "N" Then
            ...
            strSeriesID = SapCommon.GetSeriesIDFromTT(pn)
        End If
        ...
        If Not MOComBusiness.SaveErpData(o_dt) Then
        ...
        Using dt As DataTable = MoComDB.GetDataTable(sql)
            If dt.Rows.Count > 0 Then
                ...
                If pn.Trim.StartsWith("9") Then
                    If version <> GetVersion(dt.Rows(0)(0)) Then
    ...
    Private Function CheckStationIsOk() As Boolean
        Dim sql As String = "SELECT STATIONNAME FROM m_RCardUpload_t WHERE STATIONNAME IS NULL AND ID='" & tempId & "' AND WORKSTATION='" & workStation & "'" &
            MOComBusiness.GetFatoryProfitcenter() & " UNION " &
            "SELECT STATIONNAME FROM m_RCardUpload_t WHERE ID='" & tempId & "' AND WORKSTATION='" & workStation & "'" &
            MOComBusiness.GetFatoryProfitcenter() & " GROUP BY STATIONNAME HAVING COUNT(STATIONNAME)>1"
    ...
    Private Function CheckStationStand(ByRef errorMsg As String) As Boolean
        ...
        sql = " SELECT STATIONNAME FROM m_RCardUpload_t A WHERE A.ID='" & tempId & "' AND A.WORKSTATION='" & workStation &
    ...
    Private Function SaveData(dtt As DataTable, file As String) As Boolean
        ...
        Try
            ...
            o_strExecSQLResult = MoComDB.ExecuteNonQureyByProc("udpSaveImportRCard", arrayList)
...
#Region "导出"
    ...
    Public Sub SelectPath(ByVal pn As String)
    ...
    Private Sub DoExport()
        ...
        Try
            For Each o_TempRuncardPN As String In m_RuncardList.Split(",")
                ...
                Using dt As DataTable = MoComDB.GetDataTable(GetExportSql(o_TempRuncardPN))
                    If dt.Rows.Count > 0 Then
                        dt.TableName = "RunCard"
                        If SysDataBaseClass.Import2ExcelByAs(filePath, o_TempoutputFile, dt, GetVariables(dt, Nothing), errorMsg) = False Then
...
#Region "Not use"
    ...
    '    "       CREATEUSERID ," &
    '    "       DID," &
    '    "        (CASE  WHEN LEN(ISNULL(rawinfo,''))>=3 THEN LEFT (RAWINFO, LEN (RAWINFO) - 3) ELSE '' END)RAWINFO, MODIFYTIME,AMOUNTQTY, RAWINFO as RawInfoHide FROM" &
    '    "  (SELECT A.*," &
    ...
    '    "        AND PARTD.PAvcPart='" & runCardPn & "'" & _
    '    "        AND PARTD.TAvcPart <> PARTD.PAvcPart" & _
    ...
    '    "       IIf(isnull(DETAIL.PROCESSSTANDARDPrint,'') ='', DETAIL.PROCESSSTANDARD,DETAIL.PROCESSSTANDARDPrint)+ IIf(ISNULL(DETAIL.REMARK,'')='', '', '('+ DETAIL.REMARK + ')') STANDARD," & _
    '    "       DETAIL.REMARK REMARK,SECTION.ID SID,PART.TypeDest, " &
    '    "       PART.DESCRIPTION, IIF(ISNULL(Part.AMOUNTQTY,1)=0, 1, ISNULL(Part.AMOUNTQTY,1))AMOUNTQTY," & _
    ...
    '    "      FROM m_PartContrast_t PART,m_RCardCutM_t HEADER,m_RCardCutD_t DETAIL, m_Rstation_t STATION" &
    ...
    '    MOComBusiness.GetFatoryProfitcenterOne("HEADER") &
    '    "  ) A)B " &
    '    "ORDER BY B.ID"
    ...
    '        "       AUDITUSERID," &
    '        "       CREATEUSERID ," &
    '        "       DID, MODIFYTIME , LeftWirePN1, WireDes, LeftProcessStandard, RightProcessStandard, LeftTVPN, RightTVPN,LCLValue " & _
    '        "  FROM " & _
    '        "  (SELECT A.*,  ('') AS RAWINFO" & _
    '        "   FROM" &
    '        "     (SELECT " &
    '        "        HEADER.PartID ,HEADER.DRAWINGVER,HEADER.SHAPE, " &
    '        "       CAST(ROW_NUMBER() OVER(ORDER BY DETAIL.STATIONSQ) AS INT) ID,STATION.Stationid,STATION.STATIONNAME STATIONNAME,ISNULL(DETAIL.WORKINGHOURS,0) LABORHOUR," &
    '        "       DETAIL.EQUIPMENT EQUIPMENT, StdLabors," & _
    '        "      IIf(DETAIL.PROCESSSTANDARDPrint is null, DETAIL.PROCESSSTANDARD,DETAIL.PROCESSSTANDARDPrint) + IIf(ISNULL(DETAIL.REMARK,'')='', '', '('+ DETAIL.REMARK + ')') STANDARD," & _
    '        "       DETAIL.REMARK REMARK,SECTION.ID SID,PART.TypeDest, " &
    '        "       PART.DESCRIPTION," & _
    '        "         (SELECT TOP 1 username FROM m_Users_t AA, m_RCardMAuditLog_t BB WHERE AA.userid = BB.AuditUser AND BB.partid ='" & runCardPn & "'  ORDER BY AuditTime DESC) AUDITUSERID," & _
    '        "       HEADER.INTIME,Header.modifyTime,DETAIL.StationID  DID, " & _
    ...
    '        "  , item.LeftWirePN1 , item.LeftTVPN, item.RightTVPN, item.ResultValue as LCLValue," & _
    ...
    '        "      LEFT JOIN m_RstationSection_t SECTION ON STATION.SECTIONID=SECTION.ID" &
    '        "      LEFT JOIN dbo.m_RCardDCheckItem_t  item ON   item.STATIONID = STATION.Stationid AND ITEM.PARTID ='" & runCardPn & "' AND item.CheckItemID ='LCL'" & _
    '        "      AND SECTION.USEY=1" &
    '        "      WHERE PART.TAvcPart ='" & runCardPn & "'" &
    '        "        AND HEADER.PARTID =PART.TAvcPart" &
    '        "        AND HEADER.PARTID=DETAIL.PARTID" &
    ...
    '        "  ) A)B " &
    '        "ORDER BY B.ID"
    '    sql = String.Format(sql, "m_RCardCutM_t", "m_RCardCutD_t", "m_RCardDPart_t", "m_PartContrast_t")
    'Else
    '    sql = "SELECT PartID,DRAWINGVER, SHAPE," & _
    '     "       ID AS SEQ," &
    '     "       STATIONNAME," &
...
#End Region
    ...
    ''' 打印裁线卡主SQL
    ''' </summary>
    ''' <param name="runCardPn"></param>
    ''' <param name="isQueryOldVersion"></param>
    ''' <param name="strRCardVersion"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function GetExportSql(Optional ByVal runCardPn As String = "", Optional ByVal isQueryOldVersion As Boolean = False, Optional ByVal strRCardVersion As String = "") As String
        ...
        If isQueryOldVersion = False Then
            ...
            sql = "SELECT PartID, DRAWINGVER, SHAPE," & _
                ...
                "  (SELECT A.*,  (SELECT DISTINCT PARTD.TAvcPart+' || '" &
                 "      FROM {2} PARTDETAIL,{3} PARTD" & _
                    "      WHERE PARTDETAIL.EWPartNumber=PARTD.TAvcPart" & _
                     ...
                     "        AND ISNULL(PARTD.TYPE,'R')='R'" &
                      "        AND  PARTD.PAvcPart = '" & runCardPn & "' " & _
                      "       AND ( PARTDETAIL.EWPartNumber LIKE '00%' OR PARTDETAIL.EWPartNumber LIKE '01%' OR  PARTDETAIL.EWPartNumber LIKE '1_____-%'  )  " & _
                      "        AND PARTD.TAvcPart <> PARTD.PAvcPart" &
                ...
                "       DETAIL.REMARK REMARK,SECTION.ID SID,PART.TypeDest, " &
                "       PART.DESCRIPTION," & _
                "         (SELECT TOP 1 username FROM m_Users_t AA, m_RCardCutMAuditLog_t BB WHERE AA.userid = BB.AuditUser AND BB.partid ='" & runCardPn & "'  ORDER BY AuditTime DESC) AUDITUSERID," & _
                "       HEADER.INTIME,Header.modifyTime,DETAIL.StationID  DID, " & _
                "     (SELECT username from m_Users_t WHERE userid = HEADER.userid) CREATEUSERID," & _
                "       CASE HEADER.STATUS WHEN 1 THEN N'已完成' WHEN 2 THEN N'待审核' ELSE N'制作中' END STATUS" & _
                "  , item.LeftWirePN1 , ISNULL(item.LeftTVPNShow,item.LeftTVPN)LeftTVPN, ISNULL(item.RightTVPNShow,item.RightTVPN)RightTVPN, item.ResultValue as LCLValue," & _
                " (SELECT TOP 1 DESCRIPTION FROM dbo.m_PartContrast_t m1 WHERE m1.TAvcPart=item.LeftWirePN1 AND DESCRIPTION <> '') WireDes, " & _
                "  DETAIL.LeftProcessStandard, DETAIL.RightProcessStandard" & _
                "      FROM m_PartContrast_t PART,{0} HEADER,{1} DETAIL, m_Rstation_t STATION" &
                "      LEFT JOIN m_RstationSection_t SECTION ON STATION.SECTIONID=SECTION.ID" &
                "      LEFT JOIN dbo.m_CutCardDCheckItem_t  item  ON   item.STATIONID = STATION.Stationid AND ITEM.PARTID ='" & runCardPn & "' AND item.CheckItemID ='LCL'" & _
                "      AND SECTION.USEY=1" &
                "      WHERE PART.TAvcPart ='" & runCardPn & "'" &
                "        AND HEADER.PARTID =PART.TAvcPart" &
                "        AND HEADER.PARTID=DETAIL.PARTID" &
        ...
        Else
            sql = "SELECT PartID,DRAWINGVER, SHAPE," & _
             ...
             "       CREATEUSERID ," &
             "       DID," &
             "        (CASE  WHEN LEN(ISNULL(rawinfo,''))>=3 THEN LEFT (RAWINFO, LEN (RAWINFO) - 3) ELSE '' END)RAWINFO, MODIFYTIME" & _
             " FROM " & _
             "  (SELECT A.*," &
    ...
    Public Function GetVariables(ByVal dt As DataTable, Optional ByVal o_stcMO As MOComBusiness.stcMO = Nothing) As System.Collections.Generic.Dictionary(Of String, String)
        Dim dics As New System.Collections.Generic.Dictionary(Of String, String)
        Dim o_strsufHours As String = "", autoHours As String = "", ProPreHours As String = "", trimHours As String = ""
        Dim o_blnChildPn As Boolean = False
        Dim strPN As String = String.Empty
        Dim o_iAmountQty As Integer = 1
        ...
        If dt.Rows.Count > 0 Then
            ...
            If VbCommClass.VbCommClass.IsConSap = "Y" Then
                version = SapCommon.GetVerFromTiptopSap(strPN)
            Else
                version = SapCommon.GetVerFromTiptop(strPN)
            End If
            ...
            '--TotalHours
            dics.Add("TotalHours", dt.Compute("sum(HOURS)", ""))
            dics.Add("preHours", MoComDB.DBNullToStr(dt.Compute("sum(HOURS)", "SECTIONID = '1' OR SECTIONID = '01'")))
            dics.Add("proHours", MoComDB.DBNullToStr(dt.Compute("sum(HOURS)", "SECTIONID = '2' OR SECTIONID = '02'")))

            o_strsufHours = MoComDB.DBNullToStr(dt.Compute("sum(HOURS)", "SECTIONID = '3' OR SECTIONID = '03'"))
            dics.Add("sufHours", IIf(o_strsufHours.Equals(String.Empty), "0", o_strsufHours))

            trimHours = MoComDB.DBNullToStr(dt.Compute("sum(HOURS)", "SECTIONID = '4' OR SECTIONID = '04'"))
            dics.Add("trimHours", IIf(trimHours.Equals(String.Empty), "0", trimHours))

            ProPreHours = MoComDB.DBNullToStr(dt.Compute("sum(HOURS)", "SECTIONID = '5' OR SECTIONID = '05'"))
            dics.Add("ProPreHours", IIf(ProPreHours.Equals(String.Empty), "0", ProPreHours))

            autoHours = MoComDB.DBNullToStr(dt.Compute("sum(HOURS)", "SECTIONID = 'A05'"))
            dics.Add("autoHours", IIf(autoHours.Equals(String.Empty), "0", autoHours))

            dics.Add("MOID", o_stcMO.MOID)
            dics.Add("MOQty", o_stcMO.MOQty)
            dics.Add("LINEID", o_stcMO.LineID)

            'Add by cq 20160510
            If strPN.IndexOf("-") > 0 Then
                If VbCommClass.VbCommClass.IsConSap = "N" Then
                    o_iAmountQty = SapCommon.GetAmountQtyFromTT(strPN.Substring(0, strPN.IndexOf("-")), strPN)
                End If
            Else
                o_iAmountQty = MOComBusiness.GetAmountQtyFromMES(strPN, o_stcMO.MOID)
            End If
            ...
            'round((iif(sum(d.WorkingHours)=0,0,round((3600/sum(d.WorkingHours)),1))*StdLabors),1) StdUPH
