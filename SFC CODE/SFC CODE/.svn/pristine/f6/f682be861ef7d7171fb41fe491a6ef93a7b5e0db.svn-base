
'--库位选择
'--Create by :　马锋
'--Create date :　2015/08/07
'--Ver : V01
'--Update date :  
'--

#Region "Imports"

Imports System.Windows.Forms
Imports MainFrame.SysDataHandle
Imports MainFrame.SysCheckData
Imports System.Drawing
Imports System.Data.SqlClient
Imports Microsoft.VisualBasic
Imports System.Text
Imports System.IO
Imports System.Drawing.Printing
Imports DevComponents.DotNetBar
Imports DevComponents.WinForms
Imports DevComponents.DotNetBar.Controls
#End Region

Public Class FrmWarehouseLocationQuery

#Region "变量声明"

    Dim _WarehouseId As String
    Dim _txtWarehouseLocationContral As DataGridViewMaskedTextBoxAdvEditingControl

#End Region

#Region "构造函数"

    Public Sub New()
        MyBase.New()
        '该调用是 Windows 窗体设计器所必需的。
        InitializeComponent()
    End Sub

    Public Sub New(ByVal WarehouseId As String, ByVal txtWarehouseLoactionContral As DataGridViewMaskedTextBoxAdvEditingControl)
        MyBase.New()
        '该调用是 Windows 窗体设计器所必需的。
        InitializeComponent()
        _WarehouseId = WarehouseId
        _txtWarehouseLocationContral = txtWarehouseLoactionContral
    End Sub
#End Region

#Region "加载事件"

    Private Sub FrmWarehouseLoactionQuery_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            LoadContralData()
            LoadQuery()
        Catch ex As Exception
            GetMesData.SetMessage(Me.lblMessage, "加载异常", False)
        End Try
    End Sub
#End Region

#Region "控件事件"

    Private Sub btnQuery_Click(sender As Object, e As EventArgs) Handles btnQuery.Click
        LoadQuery()
    End Sub

    Private Sub btnDetermine_Click(sender As Object, e As EventArgs) Handles btnDetermine.Click
        Try
            If Me.dgvQuery.Rows.Count = 0 OrElse Me.dgvQuery.CurrentRow Is Nothing Then
                GetMesData.SetMessage(Me.lblMessage, "请选择库位", False)
                Exit Sub
            End If
            _txtWarehouseLocationContral.Text = Me.dgvQuery.Rows(Me.dgvQuery.CurrentRow.Index).Cells("WarehouseLocationCode").Value.ToString()
            Me.Close()
        Catch ex As Exception
            GetMesData.SetMessage(Me.lblMessage, "选择异常", False)
        End Try
    End Sub

    Private Sub btnCancel_Click(sender As Object, e As EventArgs) Handles btnCancel.Click
        Me.Close()
    End Sub

    Private Sub dgvQuery_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvQuery.CellDoubleClick
        Try
            If Me.dgvQuery.Rows.Count = 0 OrElse Me.dgvQuery.CurrentRow Is Nothing Then
                Exit Sub
            End If
            _txtWarehouseLocationContral.Text = Me.dgvQuery.Rows(Me.dgvQuery.CurrentRow.Index).Cells("WarehouseLocationCode").Value.ToString()

            Me.Close()
        Catch ex As Exception
            GetMesData.SetMessage(Me.lblMessage, "选择异常", False)
        End Try
    End Sub

#End Region

#Region "函数"

    Private Sub LoadContralData()
        Try
            Me.dgvQuery.AutoGenerateColumns = False
            Me.cboWarehouse.DisplayMember = "WarehouseName"
            Me.cboWarehouse.ValueMember = "WarehouseCode"
            Me.cboWarehouse.DataSource = GetMesData.GetWarehouse(VbCommClass.VbCommClass.Factory, VbCommClass.VbCommClass.profitcenter, "")

            If Not (String.IsNullOrEmpty(_WarehouseId)) Then
                Me.cboWarehouse.SelectedIndex = Me.cboWarehouse.FindString(_WarehouseId)
                Me.cboWarehouse.Enabled = True
            Else
                Me.cboWarehouse.Enabled = True
            End If
        Catch ex As Exception
            GetMesData.SetMessage(Me.lblMessage, "加载异常", False)
        End Try
    End Sub

    Private Sub LoadQuery()
        Try
            Me.dgvQuery.DataSource = GetMesData.GetWarehouseLocation(Me.cboWarehouse.SelectedValue, Me.txtWarehouseLocationCode.Text.Trim)
        Catch ex As Exception
            GetMesData.SetMessage(Me.lblMessage, "查询异常", False)
        End Try

    End Sub
#End Region

End Class