...
Public Class FrmNewLabel_DG
    ...
    Private Sub FrmNewLabel_DG_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ''快捷建處理
    ...
    Private Sub FrmNewLabel_DG_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#End Region
    ...
    Private Sub CartonDcScan()
        ...
        Try
            ...
            '****************************
            'DrScan = PubClass.GetDataReader("select a.cartonstatus,a.moid,b.partid,a.cartonqty,b.deptid,a.teamid,e.cusname,i.floorid,i.whid,i.areaid from m_Carton_t a join m_Mainmo_t b on a.moid=b.moid JOIN m_WhInd_t h on a.cartonid=h.cartonid join m_WhInm_t i on h.inwhid=i.inwhid left join m_Customer_t e on b.cusid=e.cusid where a.cartonid='" & Trim(TxtCartonid.Text) & "'")
            DrScan = PubClass.GetDataReader("select a.cartonstatus,a.moid,b.partid,a.cartonqty,b.deptid,a.teamid,e.cusname,a.floorid,a.whid,a.areaid,a.Packlink,g.dcno,i.states from m_Carton_t a join m_cartonlot_t g on a.cartonid=g.cartonid join m_Mainmo_t b on a.moid=b.moid JOIN m_WhInd_t h on a.cartonid=h.cartonid join m_WhInm_t i on h.inwhid=i.inwhid left join m_Customer_t e on b.cusid=e.cusid where a.cartonid='" & Trim(TxtCartonid.Text) & "'")
            If DrScan.Read Then
                '加入“未入庫的箱”不能拆合--2009/6/24--by tangxiang
                If DrScan("states").ToString = "1" Then
                    MessageBox.Show("該箱還未確認入庫,不能拆和!", "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information)
...
#Region "選擇未確認的新箱"
    Private Sub CobNCartonid_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CobNCartonid.SelectedIndexChanged
        ...
        Try
            SqlStr.Append("select a.moid,b.partid,b.deptid,b.lineid,e.cusname,d.djc,a.cartonqty,f.CustPart from m_Carton_t a join m_Mainmo_t b on a.moid=b.moid left join m_Dept_t d on b.deptid=d.deptid  left join m_Customer_t e on b.cusid=e.cusid left join m_PartContrast_t f on b.partid=f.TAvcPart where a.cartonid='" & Trim(CobNCartonid.Text) & "'")
