...
Public Class FrmCutCardLog
    ...
    Private Sub FrmCutCardLog_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        lssql = " SELECT TOP 200 ID, a.PartID ,b.StationName,type ,c.UserName ,a.OldModifyTime ," & _
                ...
                " FROM m_CutCardChangeLog_t a left join m_RStation_t b on a.stationid = b.stationid" & _
        ...
        lssql = lssql + " SELECT COUNT(1)  FROM m_CutCardChangeLog_t a   left join m_RStation_t b on a.stationid = b.stationid  " & _
           " LEFT JOIN m_Users_t c ON a.OldUserID = c.UserID" & _
            " LEFT JOIN m_Users_t d ON a.NewUserID = d.UserID" & _
          " WHERE 1=1  " & sqlWhere
        ...
        Using o_ds As DataSet = DbOperateUtils.GetDataSet(lssql)
            If Not IsNothing(o_ds.Tables(0)) AndAlso o_ds.Tables(0).Rows.Count > 0 Then
                dgvCutCardChangeLog.DataSource = o_ds.Tables(0)
                dtOutLog = o_ds.Tables(0)

                toolStripStatusLabel3.Text = CStr(Me.dgvCutCardChangeLog.Rows.Count) + "/总笔数:" + IIf(DbOperateUtils.DBNullToStr(o_ds.Tables(1).Rows(0).Item(0)) = "0", "0", CStr(o_ds.Tables(1).Rows(0).Item(0)))

            End If
        End Using
        '   dgvRCardChangeLog.AutoSizeColumnsMode = DataGridViewAutoSizeColumnMode.AllCells
        dgvCutCardChangeLog.AllowUserToResizeColumns = True
    End Sub
    ...
    Private Sub LoadDataToDatagridview(ByVal SqlWhere As String)
        ...
        SqlStr = SqlStr + " SELECT COUNT(1)  FROM m_CutCardChangeLog_t a   left join m_RStation_t b on a.stationid = b.stationid  " & _
                " LEFT JOIN m_Users_t c ON a.OldUserID = c.UserID" & _
                 " LEFT JOIN m_Users_t d ON a.NewUserID = d.UserID" & _
               " WHERE 1=1  " & SqlWhere
        ...
        If ds.Tables.Count > 0 AndAlso (Not IsNothing(ds.Tables(0))) Then 'dt.Rows.Count > 0 Then
            dgvCutCardChangeLog.DataSource = ds.Tables(0) 'dt
            _dtOutLog = ds.Tables(0)
            toolStripStatusLabel3.Text = CStr(Me.dgvCutCardChangeLog.Rows.Count) + "/总笔数:" + IIf(DbOperateUtils.DBNullToStr(ds.Tables(1).Rows(0).Item(0)) = "0", "0", CStr(ds.Tables(1).Rows(0).Item(0)))
            If dgvCutCardChangeLog.Rows.Count > 0 Then
        ...
        Else
            dgvCutCardChangeLog.DataSource = Nothing
        End If
    End Sub
    ...
    Private Sub DoExportLog(ByVal strPath As String)
        ...
        Try
            ...
            If IsNothing(_dtOutLog) OrElse _dtOutLog.Rows.Count <= 0 Then
                MessageUtils.ShowError("找不到对应的裁线卡修改日志信息！")
                Exit Sub
            End If
    ...
    Private Function GetLogTable() As DataTable
        ...
        Try
            ...
            DtGrid = CType(dgvCutCardChangeLog.DataSource, DataTable).Copy()
            Return GetLogTable
        Catch ex As Exception
            Return Nothing
        End Try
