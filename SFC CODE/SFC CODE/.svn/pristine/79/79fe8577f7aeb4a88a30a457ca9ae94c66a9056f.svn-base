...
Public Class FrmOutWH2_DG
    ...
    Private Sub FrmOutWH2_DG_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Init()   '初始化變量
        DoRights() '權限處理
    End Sub
    ...
    Private Sub LoadInvoiceNo()
        ...
        StrSql = " select distinct a.invoice  as invoice from m_shipm_t a, m_shipd_t b " _
               ...
               & " where states='1' and intime>getdate()-3 and factoryid in (" & g_Rights & ") and usey='Y' " _
               & "  order by invoice "
    ...
    Private Sub LoadShipInfo()
        ...
        If Rs.HasRows Then
            ''Invoice 版
            'While Rs.Read
            '    TxtCust.Text = Rs(0).ToString
            '    TxtAddress.Text = Rs(1).ToString
            '    TxtTranType.Text = Rs(2).ToString
        ...
        Catch ex As Exception
            SysCheckData.SysMessageClass.WriteErrLog(ex, "MesWarehouseManage.FrmOutWH2_DG", "PartStateChangedErr", "sys")
    ...
    '點擊單筆出貨記錄時，顯示掃描明細
    Private Sub DGShipList_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
        Dim StrStatus As String = "" '狀態
        ...
        '    '點擊單選框時，反顯
        '    If StrStatus = "待出庫" Or StrStatus = "掃描中" Then
        '        DGShipList.Item(0, DGShipList.CurrentCell.RowIndex).Value = 1
        '        PlScan.Enabled = True
        ...
        '    ElseIf StrStatus = "需調整" Then

        '        MessageBox.Show("此單據需先駁回！", "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information)
        '        DGShipList.Item(0, DGShipList.CurrentCell.RowIndex).Value = 0
        '        PlScan.Enabled = False
        '        TxtCartonid.BackColor = Color.White
        '        TxtPalletID.BackColor = Color.White
        '    ElseIf StrStatus = "掃描完成" Then

        '        MessageBox.Show("該料號已確認出庫", "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information)
        '        DGShipList.Item(0, DGShipList.CurrentCell.RowIndex).Value = 0
        '        PlScan.Enabled = False
        '        TxtCartonid.BackColor = Color.White
        '        TxtPalletID.BackColor = Color.White
        ...
        '    MessageBox.Show("該料號已掃描完成", "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information)
        '    DGShipList.Item(0, DGShipList.CurrentCell.RowIndex).Value = 0
        '    CobInvoice.Enabled = True
        'Else
    ...
    Private Sub TxtCartonid_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TxtCartonid.KeyPress
        ...
        If Asc(e.KeyChar) = 13 And Trim(TxtCartonid.Text) <> "" Then
            '2009/11/09 取消控制-----tangxiang and zap.zheng 
            ' ''加入控制----2秒內輸入完成
            ...
            If chkReuse.Checked = False Then
                ...
                If StrWHid = "" Then
                    '先進先出判斷
                    If HaveOldProduct(DGShipList.Item(1, intSelrow).Value.ToString) Then
                        PlaySimpleSound(2)
                ...
                '調用存儲過程
                StrReturn = CheckScanData(intSelrow, StrWHid, DGShipList.Item(1, intSelrow).Value.ToString, DGShipList.Item(2, intSelrow).Value, Trim(TxtCartonid.Text), 1)
            ...
            End If
            '判斷是否已達到掃描數量
            'If DGShipList.Item(2, intSelrow).Value = DGShipList.Item(3, intSelrow).Value Then

            '    chkReuse.Checked = False
            '    TxtCartonid.Text = ""
            '    PlScan.Enabled = False
            '    DGShipList.Item(0, intSelrow).Value = 0
            '    MessageBox.Show("此料號的出貨數量已達要求！", "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information)
            'End If
    ...
    Function UpdtCusAdd(ByVal strInvoice As String) As Boolean
        ...
        Try
            ...
            Return True
        Catch ex As Exception
            SysCheckData.SysMessageClass.WriteErrLog(ex, "MesWarehouseManage.FrmOutWH2_DG", "CheckScanData", "sys")
            Return False
        End Try
    ...
    Private Function GetNewWHID(ByVal strPart As String, ByVal strInvoice As String) As String
        ...
        Try
            ...
            ''Invoice 版
            'If LabShipType.Text.Substring(0, 1) = "O" Then
            '    strSql = strSql & " update m_InvoiceS_t set outwhid='" & str & "', Stateid='1' where partid='" & strPart & "' " & _
            '                      "and InvoiceJob in(select InvoiceJob from m_InvoiceM_t where invoice='" & strInvoice & "'and Stateid='3') "
            'ElseIf LabShipType.Text.Substring(0, 1) = "T" Then
            '    strSql = strSql & " update m_InvoiceS_t set outwhid='" & str & "', Stateid='1' where partid='" & strPart & "' " & _
            '                      "and InvoiceJob='" & strInvoice & "' "
            'End If
            '當前版
        ...
        Catch ex As Exception
            str = ""
            SysCheckData.SysMessageClass.WriteErrLog(ex, "MesWarehouseManage.FrmOutWH2_DG", "GetNewWHID", "sys")
        End Try
    ...
    '檢查並處理掃描數據
    Private Function CheckScanData(ByVal IntSelrow As Integer, ByVal outwhid As String, ByVal partid As String, ByVal shipqty As Integer, ByVal CaPavalue As String, ByVal flag As String) As String
        ...
        Try
            ...
            Strsql = "declare @strmsgText varchar(50),@stateid varchar(1),@scanqty int,@scancarton int,@initcarton int " & _
                     ...
                     "'" & shipqty & "','" & StrOuttype & "','" & StrUserid & "','" & CaPavalue & "','" & flag & "'," & _
    ...
    Private Function DelScanData(ByVal IntSelrow As Integer, ByVal outwhid As String, ByVal shipqty As Integer, ByVal CaPavalue As String, ByVal flag As String) As String
        ...
        Catch ex As Exception
            MainFrame.SysCheckData.SysMessageClass.WriteErrLog(ex, "MesWarehouseManage.FrmOutWH2_DG", "DelScanData", "sys")
    ...
    Private Sub DropScanData(ByVal StrWhid As String)
        ...
        Try
            ...
            Pubclass.ExecSql(Strsql)
        Catch ex As Exception
            SysCheckData.SysMessageClass.WriteErrLog(ex, "MesWarehouseManage.FrmOutWH2_DG", "BtDrop_Click", "sys")
        End Try
    End Sub
    ...
    '快捷鍵設定
    Private Sub FrmOutWH2_DG_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Function HaveOldProduct(ByVal strPart As String) As Boolean
        ...
        '有多於3批貨（同一天入庫的，算一批）在庫時，系統提示“請遵守先進先出”信息------馮澤軍 and 唐祥
        strSql = "select count (*) from (select distinct substring(convert(varchar,c.intime,120),1,10) a " & _
                 "from m_Carton_t a join m_Mainmo_t b on a.moid=b.moid " & _
                 "join m_whind_t c on c.cartonid=a.cartonid " & _
                 "where b.partid='" & strPart & "' and a.cartonstatus='I' " & _
                 " group by c.intime) a "
        Dr = Pubclass.GetDataReader(strSql)
        While Dr.Read
            If Dr(0).ToString > 3 Then
                BlnTemp = True
            Else
                strReturn = False
            End If
        ...
        If BlnTemp = True Then
            strSql = " select top 1 a.floorid,a.whid,a.areaid,c.intime " _
                   & " from m_Carton_t a join m_Mainmo_t b on a.moid=b.moid " _
                   & " join m_whind_t c on c.cartonid=a.cartonid " _
                   & " where b.partid='" & strPart & "' and a.cartonstatus='I' " _
                   & "  order by c.intime "
...
#Region "聲音播放"
    Sub PlaySimpleSound(ByVal PassOrNg As Integer)
        For IntCount As Integer = 0 To 3
            Beep()
        Next
        Select Case PassOrNg
            ...
            Case 2
                My.Computer.Audio.Play(Application.StartupPath + "\FIFO.wav", AudioPlayMode.Background)
                System.Threading.Thread.Sleep(1000)
        End Select
    End Sub
#End Region
