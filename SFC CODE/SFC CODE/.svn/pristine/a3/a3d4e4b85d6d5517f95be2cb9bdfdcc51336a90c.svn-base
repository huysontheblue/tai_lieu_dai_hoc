...
'條碼編碼原則設置程序
'UPDATE:
'2015-07-07       马锋     修正lable大于10排序BUG
'--Update :  田玉琳 2018/10/19
'--更新内容：删除多余代码,删除旧连接方式，换成有LOG生成的
...
#Region "窗體初始化工作"
    Private Sub FrmCodeSet_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
    ...
    End Sub
    '回車轉Tab鍵
    Private Sub FrmCodeSet_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles Me.KeyDown
    ...
    '界面加載時
    Private Sub FrmCodeSet_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '檢視用戶權限
        ...
        LoadDataToDgrid(" select a.CodeRuleID, a.Fstyle, a.Fdest, " &
                        " case a.LabelType when 'S' then 'S-產品條碼' when 'C' then 'C-包裝條碼' when 'P' then 'P-棧板條碼' " &
                        " when 'N' then N'N-無流水碼標簽' when 'A' then N'A-附属條碼' when 'K' then N'K-制程条码'  when 'L' then N'L-批次条码'  end as LabelType,  " &
                        " a.Flen, a.Gflen, a.BarCodeQty, a.TextQty, isnull(a.IsMultCode,'N') IsMultCode,isnull(a.IsMultCheck,'N') IsMultCheck," &
                        " case a.Usey when 'Y' then 'Y-有效' when 'N' then 'N-無效' end as Usey, ISNULL(a.PartLengthSettingFlag,'N') AS PartLengthSettingFlag, " &
                        " a.Remark, b.username as Userid, a.Intime, c.username as Canceluser, a.Canceltime " &
    ...
    Private Sub ClearRuleD()
        ...
        Me.ChkPackQty.Checked = False
        If dgM_SnRuleD_t.Rows.Count > 0 Then
            Me.txtF_codestart.Text = CInt(dgM_SnRuleD_t.Rows(dgM_SnRuleD_t.Rows.Count - 1).Cells("F_codestart").Value) +
                                     CInt(dgM_SnRuleD_t.Rows(dgM_SnRuleD_t.Rows.Count - 1).Cells("F_codelen").Value)
        Else
        ...
        Try
            ...
            For rowIndex As Integer = 0 To DReader.Rows.Count - 1
                If DGrid Is Me.dgM_SnRuleM_t Then
                    DGrid.Rows.Add(DReader.Rows(rowIndex).Item("CodeRuleID").ToString, DReader.Rows(rowIndex).Item("LabelType").ToString,
                                   DReader.Rows(rowIndex).Item("Flen").ToString, DReader.Rows(rowIndex).Item("Gflen").ToString,
                                   DReader.Rows(rowIndex).Item("BarCodeQty").ToString, DReader.Rows(rowIndex).Item("TextQty").ToString,
                                   DReader.Rows(rowIndex).Item("Remark").ToString, DReader.Rows(rowIndex).Item("Fstyle").ToString,
                                   DReader.Rows(rowIndex).Item("Fdest").ToString, DReader.Rows(rowIndex)!IsMultCode.ToString,
                                   DReader.Rows(rowIndex)!IsMultCheck, DReader.Rows(rowIndex).Item("UseY").ToString,
                                   DReader.Rows(rowIndex).Item("Userid").ToString, DReader.Rows(rowIndex).Item("Intime").ToString,
                                   DReader.Rows(rowIndex).Item("PartLengthSettingFlag").ToString(),
                                   DReader.Rows(rowIndex).Item("Canceluser").ToString(),
                                   DReader.Rows(rowIndex).Item("Canceltime").ToString())
                ElseIf DGrid Is Me.dgM_SnRuleD_t Then
                    DGrid.Rows.Add(DReader.Rows(rowIndex).Item("F_orderid").ToString, DReader.Rows(rowIndex).Item("F_codeID").ToString, _
                                   DReader.Rows(rowIndex).Item("F_codename").ToString, DReader.Rows(rowIndex).Item("F_codelen").ToString, _
                                   DReader.Rows(rowIndex).Item("DResource").ToString, DReader.Rows(rowIndex).Item("UnitID").ToString, _
                                   DReader.Rows(rowIndex).Item("SnSet").ToString, DReader.Rows(rowIndex).Item("BarArea").ToString, _
                                   DReader.Rows(rowIndex).Item("F_codestart").ToString, DReader.Rows(rowIndex).Item("SplitChar").ToString, _
                                   DReader.Rows(rowIndex).Item("IsStyle").ToString, _
                                   DReader.Rows(rowIndex).Item("IsPrintStyle").ToString, _
                                   DReader.Rows(rowIndex).Item("IsBoxQty").ToString, _
                                   DReader.Rows(rowIndex).Item("Userid").ToString, DReader.Rows(rowIndex).Item("intime").ToString)
                ElseIf DGrid Is Me.dgm_SnPFormat_t Then
                    DGrid.Rows.Add(DReader.Rows(rowIndex).Item("PFormatID").ToString, DReader.Rows(rowIndex).Item("TemplatePath").ToString,
                                   DReader.Rows(rowIndex).Item("CusName").ToString, DReader.Rows(rowIndex).Item("CodeID").ToString,
                                   DReader.Rows(rowIndex).Item("FontID").ToString, DReader.Rows(rowIndex).Item("ColNum").ToString,
                                   DReader.Rows(rowIndex).Item("RowNum").ToString, DReader.Rows(rowIndex).Item("KLabelid").ToString,
                                   DReader.Rows(rowIndex).Item("PrinterID").ToString, DReader.Rows(rowIndex).Item("PTemperature").ToString,
                                   DReader.Rows(rowIndex).Item("CarbonTape").ToString, DReader.Rows(rowIndex).Item("PaperMaterial").ToString,
                                   DReader.Rows(rowIndex).Item("PaperSize").ToString, DReader.Rows(rowIndex).Item("Descripe").ToString,
                                   DReader.Rows(rowIndex).Item("UseY").ToString, DReader.Rows(rowIndex).Item("Userid").ToString,
                                   DReader.Rows(rowIndex).Item("Intime").ToString, DReader.Rows(rowIndex).Item("PrintStyle").ToString)
    ...
    Private Sub LoadDataToCombox(ByVal sql As String, ByVal cmb As ComboBox)
        ...
        Try
            cmb.Items.Clear()
            DReader = DbOperateUtils.GetDataTable(sql)
            For rowIndex As Integer = 0 To DReader.Rows.Count - 1
                cmb.Items.Add(DReader.Rows(rowIndex).Item(0).ToString)
            Next
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Sub ToolInPortTxt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolInPortTxt.Click
        If OpenFileDialog1.ShowDialog() = Windows.Forms.DialogResult.OK Then
            ...
            If Me.cmbColNum.Text.Trim = "" Then
                ...
                MessageUtils.ShowInformation("請選擇每行打印列數！")
...
#Region "表格數據刷新"
    ...
    Private Sub RushDataRuleM()
        ...
        'modify by paul 20150310 增加制程条码 K
        Sqlstr.Append(" select a.CodeRuleID, a.Fstyle, a.Fdest, " &
                     " case a.LabelType when 'S' then 'S-產品條碼' when 'C' then 'C-外箱條碼' when 'P' then 'P-棧板條碼' " &
                     " when 'N' then 'N-無流水號標籤' when 'A' then N'A-附属條碼' when 'K' then N'K-制程条码' when 'L' then N'L-批次条码' end as LabelType,  " &
                     " a.Flen, a.Gflen, a.BarCodeQty, a.TextQty,isnull(a.IsMultCode,'N') IsMultCode,isnull(a.IsMultCheck,'N') IsMultCheck, " &
                     " case a.Usey when 'Y' then 'Y-有效' when 'N' then 'N-無效' end as Usey, a.Remark, b.username as Userid, a.Intime, " &
                     " c.username as Canceluser, a.Canceltime,a.PartLengthSettingFlag " &
        ...
        If Me.txtFlen.Text.Trim <> "" Then
            ...
            Flag = True
        End If
        'BUG修复 不存在Glen字段,应该是Gflen字段 Modified By KyLinQiu20180322
        If Me.txtGlen.Text.Trim <> "" Then
            If Flag = True Then
                'Sqlstr.Append(" and a.Glen=" & CInt(Me.txtGlen.Text.Trim))
                Sqlstr.Append(" and a.Gflen=" & CInt(Me.txtGlen.Text.Trim))
            Else
                'Sqlstr.Append(" where a.Glen=" & CInt(Me.txtGlen.Text.Trim))
                Sqlstr.Append(" where a.Gflen=" & CInt(Me.txtGlen.Text.Trim))
            End If
            Flag = True
        End If
    ...
    Private Sub FillDGFormatCbo(ByVal CodeRuleID As String, ByVal ColNum As Int16)
        ...
        Rtable = DbOperateUtils.GetDataTable("select distinct BarArea from dbo.m_SnRuleD_t where coderuleid='" & CodeRuleID & "' order by BarArea")
        Areaid.DataSource = Rtable
        Areaid.DisplayMember = "BarArea"
        Areaid.ValueMember = "BarArea"
        '所在列
        Style.DataPropertyName = "Style"
        Rtable = DbOperateUtils.GetDataTable("Create table #temp(a varchar(5)) declare @a varchar(5),@i int set @i=1 set @a=" & ColNum _
    ...
    Private Sub GetSnRuleDRecord(ByVal CodeRuleID As String)
        Try
            Dim strSQL As New StringBuilder
            strSQL.AppendLine(" SELECT   CodeRuleID, F_codeID, F_codename, F_orderid, F_codestart, F_codelen, F_codeSN, UnitID, BarArea, SplitChar, DResource, IsStyle, ")
            strSQL.AppendLine(" IsPrintStyle, IsDate, IsBoxQty, Userid, Intime, RecordTime, RecordUserId, CASE RecordStatus WHEN '1' THEN N'新增' WHEN '2' THEN N'修改' ELSE N'作废' END AS RecordStatus ")
            strSQL.AppendLine(" FROM m_SnRuleDRecord_t WHERE CodeRuleID='" & CodeRuleID & "' ORDER BY RecordTime DESC")

            Me.dgvRecord.DataSource = DbOperateUtils.GetDataTable(strSQL.ToString)
        Catch ex As Exception
            MsgBox("加载历史记录异常！", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "系統提示")
        End Try
    End Sub
#End Region
#Region "保存前的數據檢驗"
    ...
    Private Function CheckDataRuleD() As Boolean
        ...
        If Me.cmbType.Text.Trim.Split("-")(0) = "0" AndAlso (Me.txtF_codeID.Text.Trim = "Y" OrElse Me.txtF_codeID.Text.Trim = "DW" OrElse Me.txtF_codeID.Text.Trim = "W" OrElse Me.txtF_codeID.Text.Trim = "D" OrElse Me.txtF_codeID.Text.Trim = "M" OrElse Me.txtF_codeID.Text.Trim = "Q" OrElse Me.txtF_codeID.Text.Trim = "S" OrElse Me.txtF_codeID.Text.Trim = "XC" OrElse Me.txtF_codeID.Text.Trim = "DY" OrElse Me.txtF_codeID.Text.Trim = "A") Then
            MsgBox("輸入碼元代碼不能為[Y/M/W/D/S/Q/DW/XC/DY/A]，请重新输入码元代码!", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "系統提示")
            Me.ActiveControl = Me.txtF_codeID
            Return False
        End If
        ...
        End If
        'If update Then
        '    Dim sql As String = "SELECT COUNT(*) from dbo.m_SnRuleD_t where coderuleid='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' AND F_codeID='" & txtF_codeID.Text.Trim & "'"
        '    Dim dt As DataTable = DbOperateUtils.GetDataTable(sql)
        '    If dt.Rows(0)(0) > 0 Then
        '        MsgBox("码元代码" & txtF_codeID.Text.Trim & "已存在!", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "系統提示")
        '        Me.ActiveControl = Me.txtF_codeID
...
#Region "數據編輯處理"
    ...
    Private Sub RuleMEdit()
        Try
            ...
            Me.cmbLabelType.SelectedIndex = Me.cmbLabelType.FindString(Me.dgM_SnRuleM_t.Item("LabelType",
                                                                       Me.dgM_SnRuleM_t.CurrentRow.Index).Value.ToString.Trim.Split("-")(0))
            Me.chkPartLengthSettingFlag.Checked = SqlClassM.StringToBoolean(Me.dgM_SnRuleM_t.Item("PartLengthSettingFlag",
                                                                            Me.dgM_SnRuleM_t.CurrentRow.Index).Value.ToString())
            Me.ChkIsMulltCode.Checked = IIf(Me.dgM_SnRuleM_t.Item("ColMuCode", Me.dgM_SnRuleM_t.CurrentRow.Index).Value.ToString = "Y", True, False)
            Me.ChkMultCheck.Checked = IIf(Me.dgM_SnRuleM_t.Item("ColMchek", Me.dgM_SnRuleM_t.CurrentRow.Index).Value.ToString = "Y", True, False)
            opFlag = 2
            SetStatus(opFlag)
    ...
    Private Sub RuleDEdit()
        ...
        Me.ChkPackQty.Checked = IIf(Me.dgM_SnRuleD_t.Item("IsBoxQty", Me.dgM_SnRuleD_t.CurrentRow.Index).Value.ToString.Trim.ToUpper = "Y", True, False)
        Me.CobIsPrintStyle.SelectedIndex = Me.CobIsPrintStyle.FindString(Me.dgM_SnRuleD_t.Item("IsPrintStyle",
                                                                         Me.dgM_SnRuleD_t.CurrentRow.Index).Value.ToString.Trim.Split("-")(0))
        '依據碼元類型決定控件顯示
    ...
    Private Sub PformatEdit()
        ...
        FillDGFormatCbo(Me.CboRuleID2.Text.Trim.Split("-")(0), CInt(dgm_SnPFormat_t.Item("ColNum",
                        Me.dgm_SnPFormat_t.CurrentRow.Index).Value.ToString.Trim) * CInt(dgm_SnPFormat_t.Item("RowNum",
                        Me.dgm_SnPFormat_t.CurrentRow.Index).Value.ToString.Trim))
    End Sub
#End Region
...
#Region "數據作廢處理"
    ...
    Private Sub RuleMDel()
        ...
        Try
            DbOperateUtils.ExecSQL("update dbo.m_SnRuleM_t set usey='N',canceluser='" & SysMessageClass.UseId.Trim.ToLower & "',canceltime=getdate() where CodeRuleID='" & Me.dgM_SnRuleM_t.Item("CodeRuleID", Me.dgM_SnRuleM_t.CurrentRow.Index).Value.ToString & "'")
            Me.txtCodeRuleID.Text = Me.dgM_SnRuleM_t.Item("CodeRuleID", Me.dgM_SnRuleM_t.CurrentRow.Index).Value.ToString
    ...
    Private Sub RuleDDel()
        ...
        Try
            '不推荐这种带*的写法,很危险,后期如果数据库加字段就报错 Modified By KyLinQiu20180322
            'Sqlstr = " INSERT INTO m_SnRuleDRecord_t SELECT *, GETDATE(), '" & VbCommClass.VbCommClass.UseId & "', '3' FROM M_SnRuleD_t WHERE CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid='" & Me.dgM_SnRuleD_t.Item("F_orderid", Me.dgM_SnRuleD_t.CurrentRow.Index).Value.ToString.Trim & "' "
            Sqlstr = "INSERT INTO m_SnRuleDRecord_t(CodeRuleID,F_codeID,F_codename,F_orderid,F_codestart,F_codelen,F_codeSN,UnitID,BarArea," &
                     " SplitChar,DResource,IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid,Intime,RecordTime,RecordUserId,RecordStatus) " &
                     " SELECT CodeRuleID,F_codeID,F_codename,F_orderid,F_codestart,F_codelen,F_codeSN,UnitID,BarArea,SplitChar,DResource, " &
                     " IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid,Intime,GETDATE() AS RecordTime,'" & VbCommClass.VbCommClass.UseId & "' AS RecordUserId,3 AS RecordStatus FROM M_SnRuleD_t " &
                     " WHERE CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid='" & Me.dgM_SnRuleD_t.Item("F_orderid", Me.dgM_SnRuleD_t.CurrentRow.Index).Value.ToString.Trim & "'"
            Sqlstr = Sqlstr & " delete from M_SnRuleD_t where CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid=" & Me.dgM_SnRuleD_t.Item("F_orderid", Me.dgM_SnRuleD_t.CurrentRow.Index).Value.ToString.Trim
            Sqlstr = Sqlstr & " create table #a(CodeRuleID varchar(8), " _
                     & " F_codeID  varchar(2),F_codename  varchar(50),F_orderid  smallint, " _
                     & " F_codestart  smallint,F_codelen  smallint, F_codeSN nvarchar(64),UnitID varchar(50),BarArea  varchar(50), " _
                     & " SplitChar  varchar(10),DResource  varchar(50),IsStyle  varchar(1),Userid varchar(10),Intime  datetime, IsPrintStyle  varchar(1), [IsDate] varchar(1), IsBoxQty varchar(1)) "
            Sqlstr = Sqlstr & " insert #a(CodeRuleID, F_codeID, F_codename, F_orderid, F_codestart, F_codelen, F_codeSN, " _
                    & " UnitID, BarArea, SplitChar, DResource, IsStyle, Userid, Intime, IsPrintStyle, [IsDate], IsBoxQty) " _
                    & " select CodeRuleID, F_codeID, F_codename, ROW_NUMBER() OVER(ORDER BY BarArea,F_codestart) as F_orderid, " _
                    & " F_codestart, F_codelen, F_codeSN, UnitID, BarArea, SplitChar, DResource, IsStyle, Userid, Intime, IsPrintStyle, [IsDate], IsBoxQty " _
                    ...
                    & " insert dbo.m_SnRuleD_t(CodeRuleID, F_codeID, F_codename, F_orderid, F_codestart, F_codelen, F_codeSN," _
                    & " UnitID, BarArea, SplitChar, DResource, IsStyle, Userid, Intime, IsPrintStyle, [IsDate], IsBoxQty) select * from #a drop table #a"
            ...
            LoadDataToDgrid("select  a.F_orderid, a.F_codeID, a.F_codename,a.F_codelen,a.DResource,a.UnitID,c.ShortName as Snset,BarArea,F_codestart, SplitChar,case IsStyle when 'Y' " _
                    & " then 'Y-掃描檢測' when 'N' then 'N-掃描不檢測' end as IsStyle,case IsPrintStyle when 'Y' then 'Y-打印檢測' when 'N' then 'N-打印不檢測' end as IsPrintStyle,a.IsBoxQty,b.username as Userid,a.Intime " _
            ...
            GetSnRuleDRecord(Me.CboRuleID.Text.Trim.Split("-")(0))
        Catch ex As Exception
    ...
    Private Sub PformatDel()
        ...
        Try
            DbOperateUtils.ExecSQL("update dbo.M_SnPFormat_t set usey='N' where PFormatID='" & Me.dgm_SnPFormat_t.Item("PFormatID", Me.dgm_SnPFormat_t.CurrentRow.Index).Value.ToString & "'")
...
#Region "數據保存"
    ...
    Private Sub RuleMSave()
        Dim Rtable As New DataTable
        Dim SqlStr As New StringBuilder
        Dim MultCode As String = IIf(ChkIsMulltCode.Checked = True, "Y", "N")
        Dim MultCheck As String = IIf(ChkMultCheck.Checked = True, "Y", "N")
        '數據檢驗
        ...
        Try
            If opFlag = 1 Then
                '獲取編碼原則代碼
                'modify by paul 20150310 增加制程条码类型 K
                'modify by hgd 20191104  编码原则ID支持5位
                SqlStr.Append("declare @CodeRuleid varchar(8) declare @CNo varchar(10)" _
                         & " select @CodeRuleid=max(Coderuleid) from m_SnRuleM_t where LabelType='" & Me.cmbLabelType.Text.Trim.Split("-")(0) _
                         & " ' and left(Coderuleid,1)=case '" & Me.cmbLabelType.Text.Trim.Split("-")(0) & "' when 'S' then 'B' when 'P' then 'P' when 'C' then 'C' when 'N' then 'N' when 'A' then 'A' when 'K' then 'K' when 'L' then 'L' end " _
                         & " if @CodeRuleid is NULL begin " _
                         & " Set @CodeRuleid=case '" & cmbLabelType.Text.Trim.Split("-")(0) & "' when 'S' then 'B' when 'P' then 'P' when 'C' then 'C' when 'N' then 'N' when 'A' then 'A' when 'K' then 'K' when 'L' then 'L' end + '001' " _
                         & " End Else begin Set  @CNo=SUBSTRING(@CodeRuleid,2,10) if  CAST(@CNo AS int) <999 begin " _
                         & " Set @CodeRuleid=left(@CodeRuleid,1) + right('000'+ cast(cast(right(@Coderuleid,3) as int)+1 as varchar),3) end" _
                         & " else begin Set @CodeRuleid=left(@CodeRuleid,1) + right('0000'+ cast(CAST(@CNo AS int)+1 as varchar),4) end End ")

                'insert編碼原則
                SqlStr.Append("insert M_SnRuleM_t(CodeRuleID, LabelType, Flen, Gflen,BarCodeQty, TextQty,Usey,Fstyle,Fdest,Remark, PartLengthSettingFlag, userid, intime,IsMultCode,IsMultCheck) values(@CodeRuleid" _
                            ...
                            & "',N'" & Me.txtRemark.Text.ToString.Trim & "','" & SqlClassM.BooleanToString(Me.chkPartLengthSettingFlag.Checked) & "','" & SysMessageClass.UseId.Trim.ToLower & "',getdate(),'" & MultCode & "','" & MultCheck & "')")
                '返回編碼原則代碼
                SqlStr.Append(" select @CodeRuleid as CodeRuleid ")
            ElseIf opFlag = 2 Then
                'update編碼原則
                SqlStr.Append("update dbo.m_SnRuleM_t set Fstyle='" & Me.txtFstyle.Text.ToString.Trim & "',IsMultCode='" & MultCode & "',IsMultCheck='" & MultCheck & "',Fdest=N'" & Me.txtFdest.Text.ToString.Trim _
                      & "', Flen=" & Me.txtFlen.Text.ToString.Trim & ", Gflen=" & Me.txtGlen.Text.Trim & ", BarCodeQty=" & Me.txtBarCodeQty.Text.Trim _
                      & ", TextQty=" & Me.txtTextQty.Text.Trim & ",Remark=N'" & Me.txtRemark.Text.Trim & "', Userid='" & SysMessageClass.UseId.ToLower.Trim _
                      & "',Intime=getdate(), PartLengthSettingFlag= '" & SqlClassM.BooleanToString(Me.chkPartLengthSettingFlag.Checked) _
            ...
            SetStatus(opFlag)
            ClearRuleM()
            Rtable = DbOperateUtils.GetDataTable(SqlStr.ToString.Trim)
    ...
    Private Sub RuleDSave()
        ...
        Try
            If opFlag = 4 Then '新增
                ...
                SqlStr.Append(" insert M_SnRuleD_t(CodeRuleID, F_codeID, F_codename, F_orderid, F_codestart, F_codelen, F_codeSN, UnitID, BarArea, SplitChar, DResource, IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid, Intime) " _
                         ...
                         "," & txtF_codelen.Text.ToString.Trim & ", N'" & Me.TxtSnSet.Text.ToString & "','" & Me.cmbUnitID.Text.Trim.Split("-")(0) & "','" & cmbBarArea.Text.ToString.Split("-")(0) & "','" & _
                ...
                '增加记录    2016-12-08     马锋
                '不推荐这种带*的写法,很危险,后期如果数据库加字段就报错 Modified By KyLinQiu20180322
                'SqlStr.Append(" INSERT INTO m_SnRuleDRecord_t SELECT *, GETDATE(), '" & VbCommClass.VbCommClass.UseId & "', '1' FROM M_SnRuleD_t WHERE CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid=@maxorderid ")
                SqlStr.Append("INSERT INTO m_SnRuleDRecord_t(CodeRuleID,F_codeID,F_codename,F_orderid,F_codestart,F_codelen,F_codeSN,UnitID,BarArea," &
                     " SplitChar,DResource,IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid,Intime,RecordTime,RecordUserId,RecordStatus) " &
                     " SELECT CodeRuleID,F_codeID,F_codename,F_orderid,F_codestart,F_codelen,F_codeSN,UnitID,BarArea,SplitChar,DResource, " &
                     " IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid,Intime,GETDATE() AS RecordTime,'" & VbCommClass.VbCommClass.UseId & "' AS RecordUserId,1 AS RecordStatus FROM M_SnRuleD_t " &
                     " WHERE CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid=@maxorderid ")
                If UCase(txtF_codeID.Text.Trim) = "S" And Me.ChkSnIsSplit.Checked = True Then
                    ...
                    For i As Byte = 0 To CInt(TxtSplitQty.Text) - 2
                        ...
                        SqlStr.Append(vbNewLine & " insert M_SnRuleD_t(CodeRuleID, F_codeID, F_codename, F_orderid, F_codestart, F_codelen, F_codeSN, UnitID, BarArea, SplitChar, DResource, IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid, Intime) " _
                         & " values('" & Me.CboRuleID.Text.Trim.Split("-")(0) & "','" & txtF_codeID.Text.ToString.Trim _
                        ...
                        '增加记录    2016-12-08     马锋
                        '不推荐这种带*的写法,很危险,后期如果数据库加字段就报错 Modified By KyLinQiu20180322
                        'SqlStr.Append(" INSERT INTO m_SnRuleDRecord_t SELECT *, GETDATE(), '" & VbCommClass.VbCommClass.UseId & "', '1' FROM M_SnRuleD_t WHERE CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid=@maxorderid +1+ " & i & " ")
                        SqlStr.Append("INSERT INTO m_SnRuleDRecord_t(CodeRuleID,F_codeID,F_codename,F_orderid,F_codestart,F_codelen,F_codeSN,UnitID,BarArea," &
                     " SplitChar,DResource,IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid,Intime,RecordTime,RecordUserId,RecordStatus) " &
                     " SELECT CodeRuleID,F_codeID,F_codename,F_orderid,F_codestart,F_codelen,F_codeSN,UnitID,BarArea,SplitChar,DResource, " &
                     " IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid,Intime,GETDATE() AS RecordTime,'" & VbCommClass.VbCommClass.UseId & "' AS RecordUserId,1 AS RecordStatus FROM M_SnRuleD_t " &
                     " WHERE CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid=@maxorderid +1+ " & i & "")
                    Next
                End If
            ...
            ElseIf opFlag = 5 Then '修改
                '增加记录    2016-12-08     马锋
                '不推荐这种带*的写法,很危险,后期如果数据库加字段就报错 Modified By KyLinQiu20180322
                'SqlStr.Append(" INSERT INTO m_SnRuleDRecord_t SELECT *, GETDATE(), '" & VbCommClass.VbCommClass.UseId & "', '2' FROM M_SnRuleD_t WHERE CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid= " & dgM_SnRuleD_t.Item("F_orderid", dgM_SnRuleD_t.CurrentRow.Index).Value.ToString & " ")
                SqlStr.Append("INSERT INTO m_SnRuleDRecord_t(CodeRuleID,F_codeID,F_codename,F_orderid,F_codestart,F_codelen,F_codeSN,UnitID,BarArea," &
                     " SplitChar,DResource,IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid,Intime,RecordTime,RecordUserId,RecordStatus) " &
                     " SELECT CodeRuleID,F_codeID,F_codename,F_orderid,F_codestart,F_codelen,F_codeSN,UnitID,BarArea,SplitChar,DResource, " &
                     " IsStyle,IsPrintStyle,IsDate,IsBoxQty,Userid,Intime,GETDATE() AS RecordTime,'" & VbCommClass.VbCommClass.UseId &
                     "' AS RecordUserId,2 AS RecordStatus FROM M_SnRuleD_t " &
                     " WHERE CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid=" &
                     dgM_SnRuleD_t.Item("F_orderid", dgM_SnRuleD_t.CurrentRow.Index).Value.ToString & " ")
                SqlStr.Append(" update M_SnRuleD_t set F_codeID='" & txtF_codeID.Text.ToString.Trim & "', F_codename=N'" &
                         txtF_codename.Text.ToString.Trim & "', F_codestart=" & txtF_codestart.Text.ToString.Trim &
                         ", F_codelen=" & txtF_codelen.Text.ToString.Trim & ", UnitID= '" &
                         IIf(cmbUnitID.Text.ToString.Trim <> "", cmbUnitID.Text.Trim.Split("-")(0), "") &
                          "', BarArea='" & cmbBarArea.Text.ToString.Split("-")(0) & "', SplitChar='" & TxtSplitChar.Text.ToString &
                          "', DResource=N'" & IIf(cmbDResource.Text.ToString.Trim <> "", cmbDResource.Text.Trim.Split("-")(0), "") & "', IsStyle='" &
                          cmbIsStyle.Text.Trim.Split("-")(0) &
                          "',IsPrintStyle='" & CobIsPrintStyle.Text.Trim.Split("-")(0) & "',IsDate='" & IIf(ChkDate.Checked, "Y", "N") & "',IsBoxQty='" &
                          IIf(ChkPackQty.Checked, "Y", "N") & "', F_codeSN = N'" & Me.TxtSnSet.Text.ToString & "', userid='" & SysMessageClass.UseId.Trim.ToLower &
                          "', intime=getdate() where CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_orderid=" &
                          dgM_SnRuleD_t.Item("F_orderid", dgM_SnRuleD_t.CurrentRow.Index).Value.ToString)
            End If
            '添加SnSet固定碼元值
            SqlStr.Append(" delete  from m_SnSet_t where CodeRuleID='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "' and F_codeID='" &
                             txtF_codeID.Text.ToString.Trim & "'")
            If Me.TxtSnSet.Enabled = True Then
                SqlStr.Append(" insert m_SnSet_t(CodeRuleID, F_codeID, ShortName, FullName, Userid, Intime) values('" &
                              Me.CboRuleID.Text.Trim.Split("-")(0) & "',N'" & txtF_codeID.Text.ToString.Trim & "'" _
                              & ",N'" & Me.TxtSnSet.Text.ToString & "','','" & SysMessageClass.UseId.ToLower.Trim & "',getdate())")
            End If

            '重新排序作業     2015-07-07      马锋    取消重新排序作业,修正lable大于十BUG
            ...
            '            & " BarArea [varchar](20),SplitChar [varchar](10),DResource [varchar](50),IsStyle [varchar](1),IsPrintStyle [varchar](1),IsDate [varchar](1),IsBoxQty [varchar](1), " _
            ...
            SetStatus(opFlag)
            ClearRuleD()
            LoadDataToDgrid(" select  a.F_orderid, a.F_codeID, a.F_codename,a.F_codelen,a.DResource,a.UnitID,c.ShortName as Snset,BarArea,F_codestart," &
                            " SplitChar,case IsStyle when 'Y' then N'Y-扫描檢測' when 'N' then N'N-扫描不檢測' end as IsStyle, " &
                            " case IsPrintStyle when 'Y' then N'Y-打印檢測' when 'N' then N'N-打印不檢測' end as IsPrintStyle,a.IsBoxQty,IsDate," &
                            " b.username as Userid,a.Intime  from M_SnRuleD_t as a left join dbo.m_Users_t as b on a.userid=b.userid " &
                            " left join dbo.m_SnSet_t as c on a.f_codeid=c.f_codeid and a.coderuleid=c.coderuleid " &
                            " where a.coderuleid='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "'", dgM_SnRuleD_t)
            Me.LblMsg.Text = "保存成功..."
            GetSnRuleDRecord(Me.CboRuleID.Text.Trim.Split("-")(0))
        Catch ex As Exception
    ...
    Private Sub PformatSave()
        ...
        Try
            If opFlag = 7 Then  '新建
                ...
                SqlStr.Append("declare @PFormatID varchar(10)  Declare @NumLen varchar(10) select top 1 @PFormatID=PFormatID from dbo.m_SnPFormat_t where CodeRuleID='" & Me.CboRuleID2.Text.Trim.Split("-")(0) & "' Order by cast(replace(PFormatID,left(PFormatID,6),'') as int) desc " _
                         & " if @PFormatID is NULL begin Set @PFormatID='" & Me.CboRuleID2.Text.Trim.Split("-")(0) & "' + '-F' + '01' End Else begin " _
                         & " set @NumLen= cast(cast(replace(@PFormatID,left(@PFormatID,6),'') as int )+1 as varchar(10)) if len(@NumLen)=1 begin " _
                         & "Set @PFormatID='" & Me.CboRuleID2.Text.Trim.Split("-")(0) & "' + '-F0' +@NumLen END ELSE BEGIN " _
                         & " Set @PFormatID='" & Me.CboRuleID2.Text.Trim.Split("-")(0) & "' + '-F' +@NumLen END End select @PFormatID as PFormatID")
                Dim Mreader As DataTable = DbOperateUtils.GetDataTable(SqlStr.ToString)
                If Mreader.Rows.Count > 0 Then
                    PFormatID = Mreader.Rows(0)!PFormatID.ToString
                End If
                ...
                cmbPaperMaterial.Text.Trim & "',N'" & txtPaperSize.Text.Trim & "','" & cmbColNum.Text.Trim & "','" & Me.TxtRow.Text.Trim & "',N'" & _
                 txtKLabelid.Text.Trim & "',N'" & txtPrinterID.Text.Trim & "',N'" & txtPTemperature.Text.Trim & "',N'" & txtCarbonTape.Text.Trim & "'," & _
                  "N'" & txtDescripe.Text.Trim & "','Y','" & SysMessageClass.UseId.ToLower.Trim & "',getdate(),'" & PrintStyle & "')")
                '保存格式命令
            ...
            ElseIf opFlag = 8 Then '修改
                '保存格式主表
                SqlStr.Append("update M_SnPFormat_t set CusName=N'" & txtCusName.Text.Trim & "',CodeID=N'" & cmbCodeID.Text.Trim & _
                "',FontID=N'" & cmbFontID.Text.Trim & "',PaperMaterial=N'" & cmbPaperMaterial.Text.Trim & "',PaperSize=N'" & _
                ...
                "',Descripe=N'" & txtDescripe.Text.Trim & "',Userid='" & SysMessageClass.UseId.ToLower.Trim & "',Intime=getdate(),PrintStyle='" & PrintStyle & "'  where PFormatID='" & _
...
#Region "Txt/Cbo/Dg/TabControl2"
    ...
    Private Sub cmbType_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbType.SelectedIndexChanged
        ...
        Select Case cmbType.Text.Trim.Split("-")(0)
            Case "0" '自定義
                ...
                Me.cmbUnitID.Enabled = False
                Me.ActiveControl = Me.txtF_codeID
            Case "Y", "M", "D", "W", "DW", "S", "XC", "DY", "A" 'Y-年份,M-月份,D-日期,W-周別,DW-星期幾,S-流水號,XC-校驗位,A-自增流水碼
                Me.txtF_codeID.Enabled = False
                Me.txtF_codename.Enabled = False
    ...
    Private Sub cmbUnitID_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbUnitID.SelectedIndexChanged
        ...
        RData = DbOperateUtils.GetDataTable("select Charqty from M_SnUnitM_t where UnitID='" & Me.cmbUnitID.Text.Trim.Split("-")(0) & "'")
    ...
    Private Sub CboRuleID_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CboRuleID.SelectedIndexChanged
        ...
        LoadDataToDgrid("select  a.F_orderid, a.F_codeID, a.F_codename,a.F_codelen,a.DResource,a.UnitID,c.ShortName as Snset,BarArea,F_codestart, SplitChar,case IsStyle when 'Y' " _
                    ...
                    & " from M_SnRuleD_t as a left join dbo.m_Users_t as b on a.userid=b.userid left join dbo.m_SnSet_t as c on a.f_codeid=c.f_codeid and a.coderuleid=c.coderuleid " _
                    & " where a.coderuleid='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "'", dgM_SnRuleD_t)
        Rtable = DbOperateUtils.GetDataTable("select BarCodeQty,TextQty,Usey from dbo.m_SnRuleM_t where coderuleid='" & Me.CboRuleID.Text.Trim.Split("-")(0) & "'")
        '自動生成打印字串
    ...
    Private Sub CboRuleID2_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CboRuleID2.SelectedIndexChanged
        Dim Rtable As New DataTable
        Rtable = DbOperateUtils.GetDataTable("select BarCodeQty,TextQty,Usey from dbo.m_SnRuleM_t where coderuleid='" & Me.CboRuleID2.Text.Trim.Split("-")(0) & "'")
        If Rtable.Rows.Count = 0 Then Exit Sub
        ToolNew.Enabled = IIf(Rtable.Rows(0)(2).ToString.Trim = "Y", True, False)
        ToolEdit.Enabled = IIf(Rtable.Rows(0)(2).ToString.Trim = "Y", True, False)
        ToolCancel.Enabled = IIf(Rtable.Rows(0)(2).ToString.Trim = "Y", True, False)
        ...
        LoadDataToDgrid("select a.PFormatID,f.TemplatePath,a.CusName, a.CodeID, a.FontID, a.ColNum,a.RowNum,a.KLabelid,a.PrinterID,a.PTemperature,a.CarbonTape,a.PaperMaterial, a.PaperSize,a.Descripe, case a.UseY when 'Y' then 'Y-有效' when 'N' " _
                    ...
                    & " left join dbo.m_Users_t as b on a.userid=b.userid where CodeRuleID='" & Me.CboRuleID2.Text.Trim.Split("-")(0) & "'  order by cast(replace(a.PFormatID,left(a.PFormatID,6),'') as int) ", dgm_SnPFormat_t)
        Me.dgm_SnFormat_t.Rows.Clear()
