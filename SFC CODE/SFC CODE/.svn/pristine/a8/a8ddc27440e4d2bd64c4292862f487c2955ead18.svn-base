...
#Region "打印裁线卡"
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Dim o_strCutCardVersion As String = "", o_strErpVer As String = ""
        Try
            ...
            For Each CutCard As RCardComBusiness.stcCutcard In Me._cutCardIDList
                ...
                If Not String.IsNullOrEmpty(CutCard.CutCardPartPN) AndAlso CutCard.CutCardPartPN.Substring(0, 1) <> "L" Then
                    o_strErpVer = CutCardBusiness.GetVerFromErp(CutCard.CutCardPartPN).ToUpper
                    If (Not String.IsNullOrEmpty(o_strErpVer)) AndAlso CutCard.CutCardVersion.ToUpper.Trim <> o_strErpVer.Trim Then
                        MessageUtils.ShowError(String.Format("裁线卡[{0}]版本[{1}]跟ERP[{2}]不一致,请确认！",
                                                        CutCard.CutCardPartPN, CutCard.CutCardVersion.ToUpper, o_strErpVer.ToUpper))
                        Exit Sub
                    End If
                End If
...
#Region "裁线卡审核"
            ...
            For Each row As DataGridViewRow In dgvCutCardHeader.Rows
                If Not row.Cells(CutCardBusiness.HeaderGrid.CheckBox).EditedFormattedValue Is Nothing AndAlso
                    ...
                    If o_strTempPartID.Substring(0, 1) = "9" Then
                        ...
                        o_strErpVer = CutCardBusiness.GetVerFromErp(o_strTempPartID).ToUpper
                        If (Not String.IsNullOrEmpty(o_strErpVer)) AndAlso row.Cells(CutCardBusiness.NewHeaderGrid.DrawingVer).Value.ToString.ToUpper.Trim <> o_strErpVer.Trim Then
                            MessageUtils.ShowError(String.Format("流程卡[{0}]版本[{1}]跟ERP[{2}]不一致,请确认！",
                                                                  ...
                                                                  row.Cells(CutCardBusiness.NewHeaderGrid.DrawingVer).Value.ToString.ToUpper.Trim, o_strErpVer.ToUpper))
                            Exit Sub
                        End If
                    Else
                        'By pass 
                    ...
                    Else
                        ...
                        'cq 20170228
                        sql &= CutCardBusiness.GetUpateCutCardMAndSaveBOMSQL(Me.IsQueryOldVersion, partID, RCardVersion)
    ...
    Private Function CheckAutiStatus(ByVal row As DataGridViewRow) As Boolean
        Try
            ...
            dt = CutCardBusiness.GetAutiStatus(Me.IsQueryOldVersion, row.Cells(CutCardBusiness.HeaderGrid.PartId.ToString).Value.ToString)
...
#Region "绑定 CutCard 头部"
    ...
    Private Sub LoadCutCardHeader(ByVal sqlWhere As String, Optional ByVal isQquery As Boolean = False, _
        ...
        Try
            Dim dt As DataTable = CutCardBusiness.GetCutCardHeader(isQueryOldVersion, sqlWhere)
            ...
            For Each row As DataGridViewRow In dgvCutCardHeader.Rows
                If Not row.Cells(CutCardBusiness.HeaderGrid.CheckBox).EditedFormattedValue Is Nothing AndAlso
                    ...
                    Else
                        ...
                        sql &= CutCardBusiness.GetRejectStatusDeleteLogSQL(partID, o_strCutCardVer)
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                If CInt(dt.Rows(0)(0)) = 2 Then
                    MessageUtils.ShowInformation(row.Cells(CutCardBusiness.HeaderGrid.PartId.ToString).Value.ToString & "该料件的裁线卡还在审核中，不允许取消生效。")
                    Return False
                ElseIf CInt(dt.Rows(0)(0)) = 0 Then
                    MessageUtils.ShowInformation(row.Cells(CutCardBusiness.HeaderGrid.PartId.ToString).Value.ToString & "该料件的裁线卡还在制作中，不允许取消生效。")
                    Return False
                End If
            End If
...
#Region "导出"
    ...
    Private Sub tsmiHeaderExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsmiHeaderExport.Click
        ...
        Try
            ...
            Dim frmStation As New FrmCutCardImportStation(Me.CutCardPN, "Export", Me.IsQueryOldVersion)
    ...
    Private Sub tsmiHeaderSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsmiHeaderSearch.Click
        Try
            ...
            If frmQuery.ShowDialog() = Windows.Forms.DialogResult.OK Then
                ...
                LoadCutCardHeader(frmQuery.SqlWhere, True, frmQuery.IsQueryOldVersion, False) 'cq20151026, Add flag
    ...
    Private Sub tsmiHeaderPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsmiHeaderPrint.Click
        ...
        Try
            ...
            For Each o_strRunCard As RCardComBusiness.stcCutcard In _cutCardIDList
                ...
                o_strErpVer = CutCardBusiness.GetVerFromErp(o_Cutcard.CutCardPartPN).ToUpper
                If (Not String.IsNullOrEmpty(o_strErpVer)) AndAlso o_Cutcard.CutCardVersion.ToUpper().Trim <> o_strErpVer.Trim Then
                    MessageUtils.ShowError("裁线卡[ " & o_Cutcard.CutCardPartPN & "]版本跟Erp不一致！")
                    Exit Sub
                End If
            Next
...
#Region "打印裁线卡头部"
    ...
    Private Sub PrintRunCardHeader(Optional ByVal pn As String = "")
        ...
        Try
            ...
            Dim frmStation As New FrmRunCardImportStation(Me.CutCardPN, "Export", Me.IsQueryOldVersion) 'cq20150615,(Me.RunCardId, Me.RunCardPn, "Export", Me.IsQueryOldVersion)
...
#Region "绑定 CutCard 身体"
    Private Sub LoadCutCardBody(ByVal cutCardPartId As String, Optional ByVal strVersion As String = "")
        ...
        Dim ds As DataSet = CutCardBusiness.GetCutCardBody(Me.IsQueryOldVersion, cutCardPartId, strVersion)
        Try
        ...
        Try
            If e.RowIndex > -1 And e.ColumnIndex > -1 Then
                ...
                If e.RowIndex <> dgvCutCardBody.Rows.Count - 1 Then
                    ...
                    Dim o_strCutCardVersion As String = CStr(Me.dgvCutCardHeader.CurrentRow.Cells(CutCardBusiness.HeaderGrid.DrawingVer.ToString).Value.ToString)

                    '加载料件信息
                    Me.LoadPartGrid(CardPN, Me.CutCardStationId, o_strCutCardVersion)
                    '显示料件面板
    ...
    Private Sub tsmiBodyAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsmiBodyAdd.Click
        Try
            ...
            If Me.CutCardStatus = 2 Or Me.CutCardStatus = 1 Then
                ...
                Exit Sub
            End If
            Dim frmEdit As New FrmCutCardBodyEdit(Me.CutCardPN, "add", Nothing, Me, Me.Version, Me.IsQueryOldVersion)
            frmEdit.ShowDialog()
            '刷新
            LoadCutCardBody(Me.CutCardPN)
...
#Region "插入 body"
    ...
    Private Sub tsmiBodyInsert_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsmiBodyInsert.Click
        ...
        Try
            If IsNothing(Me.dgvCutCardBody.CurrentRow) Then Exit Sub
            ...
            '审核状态不可修改
            If Me.CutCardStatus = 2 Or Me.CutCardStatus = 1 Then
                MessageUtils.ShowError("料件:""" & Me.CutCardPN & """ 待审核或是已生效状态,不允许插入 O_O")
                Exit Sub
            End If

            If IsDBNull(Me.dgvCutCardBody.CurrentRow.Cells(0).Value) OrElse String.IsNullOrEmpty(CStr(Me.dgvCutCardBody.CurrentRow.Cells(0).Value)) Then
                i_MaxStationSQ = CInt(Me.dgvCutCardBody.Rows(Me.dgvCutCardBody.CurrentRow.Index - 1).Cells(CutCardBusiness.BodyGrid.StationSQ.ToString).Value) + 1
            End If

            Dim frmEdit As New FrmCutCardBodyEdit(Me.CutCardPN, "insert", Me.dgvCutCardBody.CurrentRow, Me, Me.Version, Me.IsQueryOldVersion, i_MaxStationSQ)
            frmEdit.ShowDialog()
            '刷新
            LoadCutCardBody(Me.CutCardPN)
            If Me.CutCardStatus = 1 Then
            ...
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmRunCardCut", "tsmiBodyInsert_Click", "RCard")
        End Try
    End Sub
#End Region
...
#Region "修改 body"
    Private Sub tsmiBodyModify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsmiBodyModify.Click
        Try
            ...
            Dim frmDialog As New FrmCutCardBodyEdit(Me.CutCardPN, "modify", CType(Me.tsmiBodyModify.Tag, DataGridViewRow), Me, Me.Version, Me.IsQueryOldVersion)
            If frmDialog.ShowDialog() = Windows.Forms.DialogResult.OK Then
                '刷新
                LoadCutCardBody(Me.CutCardPN)
                If Me.CutCardStatus = 1 Then
...
#Region "删除 body"
    Private Sub tsmiBodyDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsmiBodyDelete.Click
        Try
            ...
            Dim stationNo As String = row.Cells(CutCardBusiness.BodyGrid.StationID.ToString).Value.ToString
            Dim stationName As String = row.Cells(CutCardBusiness.BodyGrid.StationName.ToString).Value.ToString
            '审核中：不可删除
            If Me.CutCardStatus = 2 Or Me.CutCardStatus = 1 Then
            ...
            If MessageUtils.ShowConfirm("确认删除所选工站:""" & stationNo & "-" & stationName & """ ?", MessageBoxButtons.OKCancel) = Windows.Forms.DialogResult.OK Then
                CutCardBusiness.DeleteCutBody(Me.IsQueryOldVersion, Me.CutCardPN, Me.CutCardStationId)
    ...
    Private Sub RToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RToolStripMenuItem.Click
        Try
            ...
            If Me.CutCardStatus = 2 Or Me.CutCardStatus = 1 Then
                ...
                Exit Sub
            End If
            Dim frmDialog As New FrmCutCardBodyEdit(Me.CutCardPN, "reset", CType(Me.RToolStripMenuItem.Tag, DataGridViewRow), Me, Me.Version, Me.IsQueryOldVersion)
            If frmDialog.ShowDialog() = Windows.Forms.DialogResult.OK Then
                '刷新
                LoadCutCardBody(Me.CutCardPN)
                If Me.CutCardStatus = 1 Then
            ...
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmRunCardCut", "RToolStripMenuItem_Click", "RCard")
        End Try
    End Sub
#End Region
...
#Region "确认body"
    Private Sub tsmiBodyConfirm_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsmiBodyConfirm.Click
        ...
        Try
            ...
            For Each row As DataGridViewRow In dgvCutCardHeader.Rows
                If Not row.Cells(CutCardBusiness.HeaderGrid.CheckBox).EditedFormattedValue Is Nothing AndAlso
                    ...
                    If strTempPartId.Substring(0, 1) = "9" Then
                        ...
                        strErpVer = CutCardBusiness.GetVerFromErp(strTempPartId).ToUpper
                        If (Not String.IsNullOrEmpty(strErpVer)) AndAlso row.Cells(CutCardBusiness.HeaderGrid.DrawingVer.ToString).Value.ToString.ToUpper.Trim <> strErpVer.Trim Then
                            MessageUtils.ShowInformation(" " & strTempPartId & " 的版本和Erp版本不一致,请检查! ")
                            Exit For
                        End If
                    Else
                        'bypass check
                    ...
                    If CutCardBusiness.CheckIsPartLoss(strTempPartId, strTempStationNameList) Then
                    ...
                    Else
                        ...
                        sql &= CutCardBusiness.GetBodyConfirmSQL(Me.IsQueryOldVersion, strTempPartId)


                        Dim o_tempsql As String = ""
                        Call UpdateTVPNShow(strTempPartId, o_tempsql)
                        If Not String.IsNullOrEmpty(o_tempsql) Then
                            sql &= o_tempsql
                        End If

                        Dim o_UpdateLeftSQL As String = ""
                        Call UpdateLeftProcessStd(strTempPartId, o_UpdateLeftSQL)
                        If Not String.IsNullOrEmpty(o_UpdateLeftSQL) Then
                            sql &= o_UpdateLeftSQL
                        End If

                        Dim o_UpdateRightSQL As String = ""
                        Call UpdateRightProcessStd(strTempPartId, o_UpdateRightSQL)
                        If Not String.IsNullOrEmpty(o_UpdateRightSQL) Then
                            sql &= o_UpdateRightSQL
                        End If
            ...
            If MessageUtils.ShowConfirm("确定要确认选中的裁线卡信息", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                RCardComBusiness.ExecSQL(sql)
                ...
                'SideBar1.ExpandedPanel = sbPanelAudit
                '提示
                MessageUtils.ShowInformation("料件:" & Me.CutCardPN & "确认成功！")

                For Each row As DataGridViewRow In dgvCutCardHeader.Rows
                    If Not row.Cells(CutCardBusiness.HeaderGrid.CheckBox).EditedFormattedValue Is Nothing AndAlso
                           row.Cells(CutCardBusiness.HeaderGrid.CheckBox).EditedFormattedValue.ToString.ToUpper = "TRUE" Then
                        row.Cells(CutCardBusiness.NewHeaderGrid.ZStatus).Value = "待审核"
                        row.Cells(CutCardBusiness.NewHeaderGrid.Status).Value = "2"
                    End If
                Next
    ...
    Private Sub UpdateTVPNShow(ByVal cutPartID As String, ByRef o_strSQL As String)
        ...
        lsSQL = "SELECT A.StationID,B.StationName from m_RCardCutD_t A LEFT JOIN dbo.m_Rstation_t AS B ON  A.StationID =B.Stationid WHERE A.partid ='" & cutPartID & "'"

        Using o_dt As DataTable = sConn.GetDataTable(lsSQL)
            If Not IsNothing(o_dt) AndAlso o_dt.Rows.Count > 0 Then
                For Each dr As DataRow In o_dt.Rows()
                    Call ExistLCLCurrentStation(cutPartID, CStr(DBNullToStr(dr.Item(0))), CStr(DBNullToStr(dr.Item(1))), o_strSQL11)
    ...
    Private Sub ExistLCLCurrentStation(ByVal strPartID As String, ByVal strStationID As String, ByVal strStationName As String, ByRef strTempSQL As StringBuilder)
        Dim sConn As New MainFrame.SysDataHandle.SysDataBaseClass
        ...
        Using o_dt As DataTable = sConn.GetDataTable(lssql)
            If Not IsNothing(o_dt) AndAlso o_dt.Rows.Count > 0 Then
                '
                ExistLDPart(strPartID, strStationID, strStationName, CStr(DBNullToStr(o_dt.Rows(0).Item(0))), strLflag)
    ...
    Private Sub ExistLDPart(ByVal strPartID As String, ByVal strStationID As String, ByVal strStationName As String, ByVal strLeftTVPN As String, _
        ...
        Using o_dt As DataTable = DbOperateUtils.GetDataTable(lsSQL)
            If Not IsNothing(o_dt) AndAlso o_dt.Rows.Count > 0 Then
                '裁线/脱皮/左端压接/右端镀锡   ，既有左端字眼，又有右端字眼。cq20200304
                If strStationName.IndexOf("右端") >= 0 AndAlso strStationName.IndexOf("左端") < 0 Then
                    strLflag = "NL"
                Else
                    strLflag = "EL"
                End If
    ...
    Private Function CheckStatus(ByVal row As DataGridViewRow) As Boolean
        Try
            Dim dt As DataTable = CutCardBusiness.GetAutiStatus(Me.IsQueryOldVersion, row.Cells(CutCardBusiness.HeaderGrid.PartId.ToString).Value.ToString)
    ...
    Private Function CheckRCardStatus(ByVal strPartID As String) As Boolean
        Try
            Dim dt As DataTable = CutCardBusiness.GetAutiStatus(Me.IsQueryOldVersion, strPartID)
...
#Region "复制工站"
    Private Sub ToolCopyBodyStation_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolCopyBodyStation.Click
        Try
            ...
            Dim frmStationCopy As New FrmCutCardStationCopy(Me.CutCardPN, Me.CutCardStationId, "copy", CType(Me.RToolStripMenuItem.Tag, DataGridViewRow), Me.IsQueryOldVersion)
            ...
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmRunCardCut", "ToolCopyBodyStation_Click", "RCard")
        End Try
    End Sub
#End Region
    ...
    Private Sub btn_Click(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            If currentColumnName = CutCardBusiness.BodyGrid.StationName.ToString Then '工站名称
                frmModify = New FrmCutCardBodyModify(Me.CutCardPN, FrmCutCardBodyModify.ActionType.ModifyStation, dgvCutCardBody.CurrentRow, Me.IsQueryOldVersion, Me.Version)
            ElseIf currentColumnName = CutCardBusiness.BodyGrid.ProcessStandard.ToString Then '工艺标准
                frmModify = New FrmCutCardBodyModify(Me.CutCardPN, FrmCutCardBodyModify.ActionType.ModifyProcessParameter, dgvCutCardBody.CurrentRow, Me.IsQueryOldVersion, Me.Version)
            End If
...
#Region "绑定 CutCard 料件"
    Private Sub LoadPartGrid(cutCardPartId As String, stationID As String, Optional strCutCardVer As String = "")
        Try
            Dim dt As DataTable = CutCardBusiness.GetCutCardPart(Me.IsQueryOldVersion, cutCardPartId, stationID, strCutCardVer)
...
#Region "增加料件信息"
    Private Sub menuAddPart_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsmiPartAdd.Click
        ...
        Try
            ...
            Dim frmDialog As New FrmCutCardPartDetail(Me.CutCardPN, Me.CutCardStationId, o_strCutCardVersion, Me.IsQueryOldVersion)
            If frmDialog.ShowDialog() = Windows.Forms.DialogResult.OK Then
                ''刷新
        ...
        Try
            ...
            For Each row As DataGridViewRow In dgvPartNumber.Rows
                If Not row.Cells(0).EditedFormattedValue Is Nothing AndAlso
                    ...
                    sql &= CutCardBusiness.DeleteDPartID(Me.IsQueryOldVersion, Me.CutCardPN, partID, Me.CutCardStationId)
                End If
                index += 1
                If index = 200 Then
                    Exit For
        ...
        Try
            If Not IsNothing(o_strCutcard) Then
                For Each o_strTempCutcard In o_strCutcard
                    ...
                    Dim frmStation As New FrmCutCardImportStation(o_strTempCutcard.CutCardPartPN, "Export", Me.IsQueryOldVersion)

                    frmStation.cutCardPartId = o_strTempCutcard.CutCardPartPN

                    Using dt As DataTable = RCardComBusiness.GetDataTable(FrmCutCardImportStation.GetExportSql(o_strTempCutcard.CutCardPartPN))
                        If dt.Rows.Count > 0 Then
                            dt.TableName = "RunCard"
            ...
            Else
                ...
                Using dt As DataTable = RCardComBusiness.GetDataTable(FrmCutCardImportStation.GetExportSql(Me.CutCardPN))
                    If dt.Rows.Count > 0 Then
                        dt.TableName = "RunCard"
            ...
            If dgvCutCardBody.RowCount > 0 Then
                If dgvCutCardBody.IsCurrentCellInEditMode Or isClose Then
                    If Not dgvCutCardBody.CurrentCell.EditedFormattedValue.ToString Is Nothing AndAlso dgvCutCardBody.CurrentCell.EditedFormattedValue.ToString <> m_currentBodyValue Then
                        ...
                        Dim strSQL As String = CutCardBusiness.GetBodyUpdateSQL(Me.IsQueryOldVersion, currentColumnName, editValue, runCardId, stationId, m_currentBodyValue)
    ...
    Private Sub UpdateStdTime(ByVal parCountWorkHoursValue As String)
        Try
            If dgvCutCardBody.RowCount > 0 Then
                ...
                Dim strSQL As String = CutCardBusiness.GetBodyUpdateSQL(Me.IsQueryOldVersion, CutCardBusiness.BodyGrid.WorkingHours.ToString, parCountWorkHoursValue, runCardId, stationId, "")
...
#End Region
            ...
            If dgvCutCardHeader.RowCount > 0 Then
                If dgvCutCardHeader.IsCurrentCellInEditMode Or isClose Then
                    If Not dgvCutCardHeader.CurrentCell.EditedFormattedValue.ToString Is Nothing AndAlso dgvCutCardHeader.CurrentCell.EditedFormattedValue.ToString <> currentValue Then
                        ...
                        strSQL = CutCardBusiness.GetHeaderUpdateSQL(Me.IsQueryOldVersion, parcurrentColumnName, editValue, runCardId)
    ...
    Private Sub btnExportHeader_Click(sender As Object, e As EventArgs) Handles btnExportHeader.Click
        Try
            ...
            Dim frmStation As New FrmRunCardImportStation(Me.CutCardPN, "Export", Me.IsQueryOldVersion)
            frmStation.SelectExportPath("RunCard", strPath)

            Call DoExport(strPath)
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmRunCardCut", "btnExportHeader_Click", "RCard")
        End Try
    End Sub
    ...
    Private Sub DoExport(ByVal strPath As String)
        ...
        Try
            ...
            filePath = VbCommClass.VbCommClass.SopFilePath.Substring(0, VbCommClass.VbCommClass.SopFilePath.Length - 8) + "RunCard\RunCardHeaderTemplate.xlsx"
            ...
            Using dt As DataTable = m_dtRuncardHeader
                If dt.Rows.Count > 0 Then
                    dt.TableName = "RunCard"
                    If SysDataBaseClass.Import2ExcelByAs(filePath, o_outputFile, dt, frmStation.GetNoVariables(dt), errorMsg) Then
                    Else
                        MessageUtils.ShowError(errorMsg)
                    End If
                Else
        ...
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmRunCardCut", "DoExport", "RCard")
    ...
    Private Sub toolExportChangeLog_Click(sender As Object, e As EventArgs) Handles toolExportChangeLog.Click
        Try
            Dim frmlog As New FrmCutCardLog
            frmlog.ShowDialog()

        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmRunCardCut", "toolExportChangeLog_Click", "RCard")
        End Try
    End Sub
End Class