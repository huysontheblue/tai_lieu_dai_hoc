...
Public Class FrmRunCardStationCopy
    ...
    Sub New(ByVal runCardId As String, ByVal runCardOldStation As String, ByVal action As String, ByVal dataGridViewRow As DataGridViewRow, ByVal isQueryOldVersion As Boolean)

        ' 此调用是 Windows 窗体设计器所必需的。
        InitializeComponent()

        Me._runCardPn = runCardId
    ...
    Private Sub FillCombox(ByVal ColComboBox As ComboBox)
        Try
            If ColComboBox.Name = "cboNewStation" Then
                ...
                Dim strSQL As String = "select Stationid,StationName from m_Rstation_t where usey ='Y' and Stationtype='" & RCardType & "' order by  case when substring(StationName,1,1) =N'" &
...
#Region "初始化控件"
    ...
    Private Function GetStationIDByName(ByVal parNewStationID As String) As String
        Dim strSQL As String = " select StationName from m_Rstation_t(nolock) where usey =1 and stationName =N'" & parNewStationID.Trim() & "' and Stationtype='" & RCardType & "'"
        Dim dt As DataTable = RCardComBusiness.GetDataTable(strSQL)
        If dt.Rows.Count > 0 Then
    ...
    Private Function GetRCardDTable(sationID As String) As DataTable
        Dim strSQL As String = "SELECT (SELECT COUNT(1) FROM m_RCardD_t WHERE  PartID ='" & _runCardPn & "'  )NUM,* FROM m_RCardD_t WHERE  PartID ='" &
            Me._runCardPn & "' AND StationID= N'" & sationID & "' " & RCardComBusiness.GetFatoryProfitcenter()
        Dim dt As DataTable = RCardComBusiness.GetDataTable(strSQL)
