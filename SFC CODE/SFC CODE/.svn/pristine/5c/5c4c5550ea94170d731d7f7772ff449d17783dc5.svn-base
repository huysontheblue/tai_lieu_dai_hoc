...
Public Class FrmFinishedProductMaster
...
#Region "加载事件"
    ...
    Private Sub FrmFinishedProductMaster_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
...
#Region "控件事件"
    ...
    Private Sub btnDeleteRow_Click(sender As Object, e As EventArgs) Handles btnDeleteRow.Click
        ...
        Try
            ...
            strMaterialNO = Me.dgvFinishedProductItem.Rows(Me.dgvFinishedProductItem.CurrentRow.Index).Cells("MATERIALNO").Value.ToString
            strTransactionType = Me.dgvFinishedProductItem.Rows(Me.dgvFinishedProductItem.CurrentRow.Index).Cells("TransactionType").Value.ToString
            ...
            If (String.IsNullOrEmpty(TransactionId)) Then
                ...
                Me.dgvFinishedProductItem.Update()
            Else
                ...
                Try
                    ...
                    Me.dgvFinishedProductItem.DataSource = GetMesData.GetInvoiceTransactionItem(VbCommClass.VbCommClass.Factory, VbCommClass.VbCommClass.profitcenter, TransactionId)
                Catch ex As Exception
                    If (Conn.PubConnection.State = ConnectionState.Open) Then
                        Conn.PubConnection.Close()
                    End If
    ...
    Private Sub btnBarcodeInput_Click(sender As Object, e As EventArgs) Handles btnBarcodeInput.Click
        Try
            ...
            End If
            Dim strMaterialNo As String = ""
            If Not Me.dgvFinishedProductItem.CurrentRow Is Nothing Then
                strMaterialNo = Me.dgvFinishedProductItem.Rows(Me.dgvFinishedProductItem.CurrentRow.Index).Cells("MATERIALNO").Value
            End If
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Try
            If (String.IsNullOrEmpty(TransactionId)) Then
                ...
                strSQL.AppendLine(" SELECT @InvoicePrefix = InvoicePrefix FROM m_InvoiceDefinitions_t WHERE InvoiceDefinitionsID='8' ")
                ...
                strSQL.AppendLine(" VALUES( '1', '0','" & VbCommClass.VbCommClass.Factory & "','" & VbCommClass.VbCommClass.profitcenter & "', @TransactionID,'" & Me.cboWarehouse.SelectedValue & "', N'" & Me.cboWarehouse.Text.Trim & "', '" & Me.cboDept.SelectedValue & "', ")
                strSQL.AppendLine("  N'" & Me.cboDept.Text.Trim & "', 0,N'" & Me.mtxtDelivery.Text.Trim & "', NULL, N'" & Me.mtxtAttentionName.Text.Trim & "','" & Me.txtERPTranscationID.Text.Trim & "', N'" & Me.txtRemark.Text.Trim.Replace("'", "''") & "',")
                ...
                For I As Int16 = 0 To Me.dgvFinishedProductItem.Rows.Count - 1
                    strMaterialNO = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("MATERIALNO").Value), "", Me.dgvFinishedProductItem.Rows(I).Cells("MATERIALNO").Value.ToString())
                    strDescription = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("DESCRIPTION").Value), "", Me.dgvFinishedProductItem.Rows(I).Cells("DESCRIPTION").Value.ToString())
                    strSpecification = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("SPECIFICATION").Value), "", Me.dgvFinishedProductItem.Rows(I).Cells("SPECIFICATION").Value.ToString())
                    strUint = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("UOM_NAME").Value), "", Me.dgvFinishedProductItem.Rows(I).Cells("UOM_NAME").Value.ToString())
                    strQuantity = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("QUANTITY").Value), "0", Me.dgvFinishedProductItem.Rows(I).Cells("QUANTITY").Value.ToString())
                    strUnitPrice = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("UNITPRICE").Value), "", Me.dgvFinishedProductItem.Rows(I).Cells("UNITPRICE").Value.ToString())
                    strTotalAmount = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("TotalAmount").Value), "0", Me.dgvFinishedProductItem.Rows(I).Cells("TotalAmount").Value.ToString())
                    strRemark = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("REMARK").Value), "", Me.dgvFinishedProductItem.Rows(I).Cells("REMARK").Value.ToString())
                    strTransactionType = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("TransactionType").Value), "", Me.dgvFinishedProductItem.Rows(I).Cells("TransactionType").Value.ToString())
                    strPackingSame = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("PackingSame").Value), "0", Me.dgvFinishedProductItem.Rows(I).Cells("PackingSame").Value.ToString())
                    strOrderNumber = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(I).Cells("OrderNumber").Value), "", Me.dgvFinishedProductItem.Rows(I).Cells("OrderNumber").Value.ToString())
                ...
                If (ScanBarcode) Then
                    ...
                    'strSQL.AppendLine(" SELECT BarCode FROM m_ScanBacodeTransaction_t WHERE ScanBarcodeTransactionId='" & ScanTransactionId & "' AND DeleteFlag='0' AND PackingSame='0' AND ProductType='1' ) ")

                    strSQL.AppendLine(" UPDATE W_REELS_T SET REMAINING_QUANTITY = 0, STATUS='3' ")
                    strSQL.AppendLine(" FROM m_ScanBacodeTransaction_t ")
                    ...
                    strSQL.AppendLine(" UPDATE m_Cartonsn_t SET Status = '1' ")
                    strSQL.AppendLine(" FROM m_ScanBacodeTransaction_t ")
                    ...
                    strSQL.AppendLine(" FROM m_ScanBacodeTransaction_t WHERE W_REELS_T.REEL_BARCODE = m_ScanBacodeTransaction_t.BARCODE AND ScanBarcodeTransactionId='" & ScanTransactionId & "' AND m_ScanBacodeTransaction_t.DeleteFlag='0' AND m_ScanBacodeTransaction_t.PackingSame='1' AND m_ScanBacodeTransaction_t.ProductType='1'  ")

                    '更新产品出货状态
                    strSQL.AppendLine(" UPDATE m_Cartonsn_t SET Status='1' ")
                    strSQL.AppendLine(" FROM m_ScanBacodeTransaction_t ")
                    strSQL.AppendLine(" WHERE  m_Cartonsn_t.ppid=m_ScanBacodeTransaction_t.BARCODE AND ScanBarcodeTransactionId='" & ScanTransactionId & "' AND m_ScanBacodeTransaction_t.DeleteFlag='0' AND m_ScanBacodeTransaction_t.ProductType='0' ")

                    '更新产品对应箱在库数量
                    strSQL.AppendLine(" UPDATE W_REELS_T SET REMAINING_QUANTITY = REMAINING_QUANTITY - m_ScanBacodeTransaction_t.Quantity ")
                    strSQL.AppendLine(" FROM m_ScanBacodeTransaction_t WHERE W_REELS_T.REEL_BARCODE = m_ScanBacodeTransaction_t.PackingCartonId AND ")
                    strSQL.AppendLine(" ScanBarcodeTransactionId='" & ScanTransactionId & "' AND m_ScanBacodeTransaction_t.DeleteFlag='0' AND m_ScanBacodeTransaction_t.ProductType='0' ")
...
#Region "函数"
    ...
    Private Sub LoadControlData()
        ...
        Try
            ...
            If (String.IsNullOrEmpty(_strTransactionId)) Then
                ...
                Me.dgvFinishedProductItem.Rows.Add(7)
            ...
            Else
                ...
                If (Not dtInvoice Is Nothing And dtInvoice.Rows.Count > 0) Then
                    ...
                    Me.cboDept.SelectedIndex = Me.cboDept.FindString(dtInvoice.Rows(0).Item("DeptName").ToString)
                ...
                If (Not dtInvoiceItem Is Nothing And dtInvoiceItem.Rows.Count > 0) Then
                    Me.dgvFinishedProductItem.DataSource = dtInvoiceItem
                End If
            End If
            InitDataGrid_afterBindData()
        Catch ex As Exception
    ...
    Private Function CheckSave() As Boolean
        ...
        Try
            ...
            If (Me.dgvFinishedProductItem.Rows.Count <= 0) Then
                rtValue = False
            ...
            Else
                ...
                For i As Int16 = 0 To Me.dgvFinishedProductItem.Rows.Count - 1
                    strMaterialNO = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(i).Cells("MaterialNO").Value), "", Me.dgvFinishedProductItem.Rows(i).Cells("MaterialNO").Value)
                    Qty = IIf(IsDBNull(Me.dgvFinishedProductItem.Rows(i).Cells("QUANTITY").Value), "", Me.dgvFinishedProductItem.Rows(i).Cells("QUANTITY").Value)
    ...
    Private Sub InitDataGrid_afterBindData()
        If Me.dgvFinishedProductItem.RowCount <= 0 Then Return
        For i As Integer = 0 To Me.dgvFinishedProductItem.RowCount - 1
            If (String.IsNullOrEmpty(Me.dgvFinishedProductItem.Rows(i).Cells("TransactionType").Value) Or Me.dgvFinishedProductItem.Rows(i).Cells("TransactionType").Value = "1") Then
                Me.dgvFinishedProductItem.Rows(i).Cells("QUANTITY").ReadOnly = True                               '设置行只读  
                Me.dgvFinishedProductItem.Rows(i).Cells("QUANTITY").Style.BackColor = Color.Lavender              '背景设置灰色只读  
            Else
                Me.dgvFinishedProductItem.Rows(i).Cells("QUANTITY").ReadOnly = False                               '设置行只读  
                Me.dgvFinishedProductItem.Rows(i).Cells("QUANTITY").Style.BackColor = Color.White              '背景设置灰色只读
            End If
        Next
    End Sub
#End Region
