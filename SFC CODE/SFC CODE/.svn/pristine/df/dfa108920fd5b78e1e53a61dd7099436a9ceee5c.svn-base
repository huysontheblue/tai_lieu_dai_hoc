...
Public Class FrmReportMain
    ...
    Private _SqlUsey As String = "N"
    Public Property SqlUsey() As String
        Get
            Return _SqlUsey
        End Get
    ...
    Public Property ChartUsey() As String
        ...
        End Set
    End Property
    Private _LocalDbUsey As String
    Public Property LocalDbUsey() As String
        Get
            Return _LocalDbUsey
        End Get
        Set(ByVal value As String)
    ...
    Public Property ChartTitle() As String
        ...
        End Set
    End Property
    Private _SummaryTitle As String
    ...
    Private Sub FrmReportMain_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        sql.Append("select a.Tkey,Tparent,Ttext,Enname,TreeTag,ReportUsey=isnull(ReportUsey,'N') from (select * from  m_Logtree_t where Tparent='m09_' or Tkey='m09_' or Tparent in (  select Tkey from m_Logtree_t where Tparent='m09_' )) a ")
        sql.Append(" join m_UserRight_t b on a.Tkey=b.Tkey where  b.userid='" & SysCheckData.SysMessageClass.UseId & "' and ReportUsey='Y' order by Tparent,OrderBy")
        sql.Append(" ")
        Dim dt As DataTable = DbOperateReportUtils.GetDataTable(sql.ToString())
        '  Dim dt As DataTable = DbOperateReportUtils.GetDataTable("select a.Tkey,Tparent,Ttext,Enname,TreeTag,ReportUsey=isnull(ReportUsey,'N') from m_Logtree_t a join m_UserRight_t b on a.Tkey=b.Tkey where b.userid='" & SysCheckData.SysMessageClass.UseId & "' and ReportUsey='Y' order by  a.Tparent,a.OrderBy")
    ...
    Private Sub AdvTree1_Click(sender As Object, e As EventArgs) Handles AdvTree1.Click
        ...
        Dim sql As String = String.Format(" select a.Ttext,b.SqlScript,b.SqlUsey,DetScript,DetUsey,ChartScript,ChartTitle,SummaryTitle,ChartType,ChartUsey,LocalDbUsey from m_Logtree_t a left join m_Logtree_Det_t b on a.Tkey=b.Tkey  where a.Tkey='{0}'  select * from m_Logtree_Qry_t where  Tkey='{0}' order by QRYSEQ  ", key)
    ...
    Public Sub CreateControlsList()
        ...
        If ds.Tables.Count > 0 Then
            ...
            SqlUsey = dt_mstr.Rows(0)("SqlUsey").ToString
            ...
            SummaryTitle = dt_mstr.Rows(0)("SummaryTitle").ToString
                    ...
                    Select Case PARA_TYPE
                        Case "D"
                            ...
                            If DEFAULT_VALUE = "" Then
                                tb.Value = Now().ToString("yyyy-MM-dd")
                            Else
    ...
    Public Sub BindDropDownList(ByVal obj As ComboBox, ByVal StrSql As String, ByVal ISNUL As Boolean, ByVal DEFAULT_VALUE As String, Optional ByVal ValueField As String = "", Optional ByVal TextField As String = "")
        ...
        StrSql = ReplaceNoCase(StrSql, "@@factoryid", "'" + SysMessageClass.FactoryId + "'")
        ...
        If ds.Tables.Count > 0 Then
            ...
            If dt_mstr.Rows.Count > 0 Then
                ...
                If SqlUsey <> "Y" Then
                    Exit Sub
                End If
                LocalDbUsey = dt_mstr.Rows(0)("LocalDbUsey").ToString
                DetUsey = dt_mstr.Rows(0)("DetUsey").ToString
                isSelectSQL = True
                If sql <> "" Then
                ...
                sql = ReplaceNoCase(sql, "@@factoryid", "'" + SysMessageClass.FactoryId + "'")
                ...
                Try
                    If dt_det.Rows.Count > 0 And sql <> "" Then
                        ...
                        If (isSelectSQL) Then
                            If AllowQry Then
                                pageData.IsLoaclQuery = IIf(LocalDbUsey = "Y", True, False)
    ...
    Private Sub btnReport_Click(sender As Object, e As EventArgs) Handles btnReport.Click
        If ChartUsey = "Y" Then
            ...
            ' Dim cout As Integer = sql.Split("{").Length - 1
            For i = 0 To DataGridViewX1.CurrentRow.Cells.Count - 1
                sql = sql.Replace("{" & i.ToString & "}", DataGridViewX1.CurrentRow.Cells(i).Value.ToString())
                ChartTitle = ChartTitle.Replace("{" & i.ToString & "}", DataGridViewX1.CurrentRow.Cells(i).Value.ToString())
                SummaryTitle = SummaryTitle.Replace("{" & i.ToString & "}", DataGridViewX1.CurrentRow.Cells(i).Value.ToString())
            Next
            sql = ReplaceNoCase(sql, "@@userid", "'" + SysMessageClass.UseId + "'")
            sql = ReplaceNoCase(sql, "@@factoryid", "'" + SysMessageClass.FactoryId + "'")
            ...
            ReportChar.ReportSQL = sql

            ReportChar.CharTitle = ChartTitle ' GroupBox1.Text
            ReportChar.SummaryTitle = SummaryTitle
            If ChartType = 0 Then
                ReportChar.ChartViewType = DevExpress.XtraCharts.ViewType.Bar
            ElseIf ChartType = 1 Then
                ReportChar.ChartViewType = DevExpress.XtraCharts.ViewType.Line
            ElseIf ChartType = 2 Then
                ReportChar.ChartViewType = DevExpress.XtraCharts.ViewType.Bar3D
            End If
            ReportChar.PubParam = PubParam
