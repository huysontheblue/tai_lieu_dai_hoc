...
Public Class FrmCutCardHeaderEdit
...
#Region "FrmCutCardHeaderEdit_Load"
    Private Sub FrmCutCardHeaderEdit_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim lsSQL As String = ""
        Try
            lblMsg.Text = ""
            lblId.Text = ""
        ...
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmCutCardCutHeaderEdit", "FrmCutCardHeaderEdit_Load", "RCard")
        End Try
    End Sub
...
#Region "btnCheckBom_Click"
    ...
    Private Sub btnCheckBom_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCheckBom.Click
        ...
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmCutCardHeaderEdit", " btnCheckBom_Click", "RCard")
        End Try
    End Sub
...
#Region "btnOK_Click"
    Private Sub btnOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOK.Click
        Try
            ...
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmCutCardHeaderEdit", "btnOK_Click", "RCard")
        End Try
    End Sub
#End Region
...
#Region "检查料件是否已经建档"
    Private Function RunCardExists(ByVal pn As String) As Boolean
        ...
        If Action = "add" Then
            strSql = "SELECT 1 FROM m_RCardCutM_t(nolock) where PartID in(select TAvcPart from m_PartContrast_t(nolock) where TAvcPart=N'" & pn & "' and UseY='Y') " &
    ...
    Private Function DownloadBom(ByVal pn As String, ByRef msg As String, Optional ByVal HasUI As Boolean = False) As Boolean
        ...
        Try
            ...
            Else
                ...
                dt = RCardComBusiness.GetDataTableOracle(strSQL)
            End If
            ...
            Return True
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmCutCardHeaderEdit", "DownloadBom", "RCard")
            Throw ex
        End Try
    End Function
...
#Region "保存数据"
    Private Sub SaveData()
        ...
        Try
            ...
            If Action = "add" Then
                '写入DB
                StrSql = " IF NOT EXISTS(SELECT TOP 1 1 FROM m_RCardCutM_t WHERE PARTID='" & PartNumber & "' AND Factoryid='" & Factoryid & "' AND Profitcenter='" & Profitcenter & "') " & _
                         " Begin " & _
                         "INSERT INTO m_RCardCutM_t(PartID,DrawingVer,DrawingFilePath,Shape,FinishSize,Status,UserID,InTime,Factoryid,Profitcenter,StdLabors) VALUES(" &
                         "'{0}',N'{1}',N'{2}',N'{3}',{4},0,'{5}',getdate(),'{6}','{7}','{8}' )" & _
                         " End"
                StrSql = String.Format(StrSql, PartNumber, DrawingVer, ServerFile, Shape, FinishSize, UserID, Factoryid, Profitcenter, StdLabors)
            Else
                If Not String.IsNullOrEmpty(Me.Version) AndAlso Me.Version.ToUpper <> txtDrawingVer.Text.Trim.ToUpper Then
                    StrSql = (CutCardBusiness.GetCutCardEditSQLOfValue(PartNumber, Me.Version, Factoryid, Profitcenter))

                    StrSql = StrSql + RunCardBusiness.GetSaveOldBOMSQL(PartNumber, Me.Version.ToUpper, Factoryid, Profitcenter)
                ...
                If Not o_blnOnlyUpdateBOM Then
                    ...
                    StrSql += " UPDATE m_RCardCutM_t SET MODIFYTIME=GETDATE() ,DrawingVer=N'" & DrawingVer & "',STATUS=0,DrawingFilePath=N'" & ServerFile &
                                "',Shape=N'" & Shape & "',FinishSize = '" & FinishSize & "', StdLabors='" & StdLabors & "',InTime=getdate()," & _
                                " userid ='" & VbCommClass.VbCommClass.UseId & "'  WHERE PartID='" & PartNumber & "'"
                    'Modify by cq 20160705,remove update intime
                    StrSql += " UPDATE m_RCardCutD_t SET DrawingVer=N'" & DrawingVer & "' WHERE PartID='" & PartNumber & "'"
                    StrSql += " UPDATE m_CutCardDPart_t SET InTime=GETDATE() ,DrawingVer=N'" & DrawingVer & "' WHERE PartID='" & PartNumber & "'"
                Else
                    StrSql += " UPDATE m_RCardCutM_t SET MODIFYTIME=GETDATE() ,Shape=N'" & Shape & "',StdLabors='" & StdLabors & "' WHERE PartID='" & PartNumber & "'"
                End If
            End If
            ...
            If Not String.IsNullOrEmpty(StrSql) Then
                ...
                DbOperateUtils.ExecSQL(StrSql, al)
            End If
            If result <> "" Then
                Throw New Exception(result)
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmCutCardHeaderEdit", "SaveData", "RCard")
        End Try
    End Sub
#End Region
