...
Public Class FrmMaintainBom
    ...
    Private Function GetSqlString(ByVal sqlWhere As String) As String
        ...
        sql = "SELECT TOP 180 ROW_NUMBER() OVER(ORDER BY B.PARTNUMBER,C.PARTNUMBER) 序号,B.PARTNUMBER 父料件编号,C.PARTNUMBER 子料件编号,FORMAT 规格," & _
              ...
              "CASE A.EXTENSIBLE WHEN 'Y' THEN N'Y.可展开' ELSE N'N.不展开' END 可展开否,QTY 数量," & _
              "EFFECTIVEDATE 有效开始日期, EXPIRATIONDATE 失效日期," & _
              "A.USERID 维护人员,A.INTIME 维护日期 FROM M_RUNCARDBOMINFO_T A,M_RUNCARDPARTNUMBER_T B,M_RUNCARDPARTNUMBER_T  C" & _
              " WHERE A.PARENTPARTID =B.ID " & _
              " AND A.PARTID=C.ID "
        If sqlWhere <> "" Then
            sql += sqlWhere
    ...
    Private Function GetErpFilterSql() As String
        txtGroupCode.Text = ""
        Dim querySql As String = Nothing
        querySql = "SELECT ROW_NUMBER() OVER(ORDER BY A.BMB01) ID,A.BMB01,A.BMB03,B.IMA05,B.IMA021,C.IMA021,B.IMA02,C.IMA02," & _
        "A.BMB04,A.BMB05,DECODE(A.BMB19,3,'Y','N'),DECODE(A.BMB19,3,'Y.可展開','N.不展開') EXTENSIABLE,A.BMB06 " & _
        " FROM(" & _
        "SELECT A.BMB01,A.BMB03,A.BMB19,A.BMB06,TO_CHAR(A.BMB04,'YYYY/MM/DD') BMB04,TO_CHAR(A.BMB05,'YYYY/MM/DD') BMB05 " & _
        "FROM " & VbCommClass.VbCommClass.Factory & " .BMB_FILE A " & _
        "CONNECT BY prior A.BMB03=A.BMB01 AND LEVEL<3" & _
        "START WITH A.BMB01='" & txtErpPartID.Text.Trim & "') A," & VbCommClass.VbCommClass.Factory & ".IMA_FILE B," & VbCommClass.VbCommClass.Factory & ".IMA_FILE C " & _
        "WHERE A.BMB01=B.IMA01 " & _
        "AND A.BMB03=C.IMA01 " & _
        "AND A.BMB04<=TO_CHAR(SYSDATE,'YYYY/MM/DD') AND (A.BMB05 IS NULL OR A.BMB05>=TO_CHAR(SYSDATE,'YYYY/MM/DD'))"
        Dim codes As String = ""
    ...
    Private Function PnExistsInBom() As Boolean
        Dim sqlConn As New SysDataBaseClass
        Dim sql = "SELECT A.PARTNUMBER FROM M_RUNCARDPARTNUMBER_T A,M_RUNCARDBOMINFO_T B" & _
                  " WHERE A.PARTNUMBER='" & txtErpPartID.Text.Trim & "' AND " & _
                  " EXISTS (SELECT 1 FROM M_RUNCARDBOMINFO_T WHERE PARENTPARTID=A.ID)"
        Return sqlConn.GetRowsCount(sql) > 0
    End Function
    ...
    Private Sub SaveBomInfoToDb(ByVal isPnExists As Boolean)
        ...
        Try
            If dgvBomInfo.Rows.Count = 0 Then
                ...
                Exit Sub
            End If
            sql.Append(" DELETE FROM M_RUNCARDBOMINFO_T WHERE PARENTPARTID IN(SELECT A.PARTID FROM M_RUNCARDBOMINFO_T A,M_RUNCARDBOMINFO_T B,M_RUNCARDPARTNUMBER_T C WHERE A.PARTID=B.PARENTPARTID AND A.PARENTPARTID=C.ID AND C.PARTNUMBER='" & txtErpPartID.Text.Trim & "')").Append(vbNewLine)
            sql.Append(" DELETE FROM M_RUNCARDBOMINFO_T WHERE PARENTPARTID IN (SELECT ID FROM M_RUNCARDPARTNUMBER_T WHERE PARTNUMBER='" & txtErpPartID.Text.Trim & "')").Append(vbNewLine)
            For Each Row As DataGridViewRow In dgvBomInfo.Rows
                If Row.Index = dgvBomInfo.Rows.Count Then Exit For
                sql.Append(" IF NOT EXISTS(SELECT 1 FROM M_RUNCARDPARTNUMBER_T WHERE PARTNUMBER='" & Row.Cells(BomInfo.ParentPartId).Value & "') begin").Append(vbNewLine)
                sql.Append(" INSERT INTO M_RUNCARDPARTNUMBER_T(PARTNUMBER,DESCRIPTION,STATUS,USERID,INTIME,DESCRIPTION1,TYPE) VALUES('" & Row.Cells(BomInfo.ParentPartId).Value & "',").Append(vbNewLine)
                sql.Append(" N'" & Row.Cells(BomInfo.ParentDescription).Value.ToString.Replace("'", "''") & "','1','" & VbCommClass.VbCommClass.UseId & "',getdate(),N'" & Row.Cells(BomInfo.PFormat).Value.ToString.Replace("'", "''") & "','R') end ").Append(vbNewLine)
                sql.Append(" ELSE BEGIN UPDATE M_RUNCARDPARTNUMBER_T SET DESCRIPTION=N'" & Row.Cells(BomInfo.ParentDescription).Value.ToString.Replace("'", "''") & "',DESCRIPTION1=N'" & Row.Cells(BomInfo.PFormat).Value.ToString.Replace("'", "''") & "' WHERE PARTNUMBER='" & Row.Cells(BomInfo.ParentPartId).Value & "' END").Append(vbNewLine)
                sql.Append(" IF NOT EXISTS(SELECT 1 FROM M_RUNCARDPARTNUMBER_T WHERE PARTNUMBER='" & Row.Cells(BomInfo.ChildPartId).Value & "') begin").Append(vbNewLine)
                sql.Append(" INSERT INTO M_RUNCARDPARTNUMBER_T(PARTNUMBER,DESCRIPTION,STATUS,USERID,INTIME,DESCRIPTION1,TYPE) VALUES('" & Row.Cells(BomInfo.ChildPartId).Value & "',").Append(vbNewLine)
                sql.Append(" N'" & Row.Cells(BomInfo.ChildDescription).Value.ToString.Replace("'", "''") & "','1','" & VbCommClass.VbCommClass.UseId & "',getdate(),N'" & Row.Cells(BomInfo.CFormat).Value.ToString.Replace("'", "''") & "','R') end").Append(vbNewLine)
                sql.Append(" ELSE BEGIN UPDATE M_RUNCARDPARTNUMBER_T SET DESCRIPTION=N'" & Row.Cells(BomInfo.ChildDescription).Value.ToString.Replace("'", "''") & "',DESCRIPTION1=N'" & Row.Cells(BomInfo.CFormat).Value.ToString.Replace("'", "''") & "' WHERE PARTNUMBER='" & Row.Cells(BomInfo.ChildPartId).Value & "' END").Append(vbNewLine)
                sql.Append(" IF NOT EXISTS(SELECT 1 FROM M_RUNCARDBOMINFO_T WHERE PARENTPARTID=(SELECT TOP 1 ID FROM M_RUNCARDPARTNUMBER_T WHERE PARTNUMBER='" & Row.Cells(BomInfo.ParentPartId).Value & "') AND PARTID=(SELECT TOP 1 ID FROM M_RUNCARDPARTNUMBER_T WHERE PARTNUMBER='" & Row.Cells(BomInfo.ChildPartId).Value & "')) begin").Append(vbNewLine)
                sql.Append(" INSERT INTO M_RUNCARDBOMINFO_T(PARENTPARTID,PARTID,STATUS,USERID,INTIME,EXTENSIBLE,QTY,EFFECTIVEDATE,EXPIRATIONDATE,VERSION,FORMAT,PARENTPARTNUMBER,PARTNUMBER,FACTORY) ").Append(vbNewLine)
                sql.Append(" VALUES((SELECT TOP 1 ID FROM M_RUNCARDPARTNUMBER_T WHERE PARTNUMBER='" & Row.Cells(BomInfo.ParentPartId).Value & "'),").Append(vbNewLine)
                sql.Append("(SELECT TOP 1 ID FROM M_RUNCARDPARTNUMBER_T WHERE PARTNUMBER='" & Row.Cells(BomInfo.ChildPartId).Value & "'),'1','" & VbCommClass.VbCommClass.UseId & "',getdate(),'" & Row.Cells(BomInfo.Extensible).Value & "','" & Row.Cells(BomInfo.Qty).Value & "'").Append(vbNewLine)
                sql.Append(",'" & Row.Cells(BomInfo.EffectiveDate).Value & "','" & Row.Cells(BomInfo.ExpirationDate).Value & "','" & Row.Cells(BomInfo.Version).Value & "',N'" & Row.Cells(BomInfo.PFormat).Value & "','" & Row.Cells(BomInfo.ParentPartId).Value & "','" & Row.Cells(BomInfo.ChildPartId).Value & "','" & VbCommClass.VbCommClass.Factory & "') end").Append(vbNewLine)
                sql.Append(" ELSE BEGIN UPDATE M_RUNCARDBOMINFO_T SET QTY='" & Row.Cells(BomInfo.Qty).Value & "',EXTENSIBLE='" & Row.Cells(BomInfo.Extensible).Value & "'").Append(vbNewLine)
                sql.Append(",EFFECTIVEDATE='" & Row.Cells(BomInfo.EffectiveDate).Value & "',EXPIRATIONDATE='" & Row.Cells(BomInfo.ExpirationDate).Value & "'").Append(vbNewLine)
                sql.Append(",VERSION='" & Row.Cells(BomInfo.Version).Value & "',USERID='" & VbCommClass.VbCommClass.UseId & "',INTIME=getdate(),FORMAT=N'" & Row.Cells(BomInfo.PFormat).Value & "'").Append(vbNewLine)
                sql.Append(" WHERE PARENTPARTID=(SELECT TOP 1 ID FROM M_RUNCARDPARTNUMBER_T WHERE PARTNUMBER='" & Row.Cells(BomInfo.ParentPartId).Value & "') ").Append(vbNewLine)
                sql.Append(" AND PARTID=(SELECT TOP 1 ID FROM M_RUNCARDPARTNUMBER_T WHERE PARTNUMBER='" & Row.Cells(BomInfo.ChildPartId).Value & "') end").Append(vbNewLine)
            Next
            sqlConn.ExecSql(sql.ToString)
            MessageBox.Show("资料保存成功", "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information)
            lblMsg.Text = "资料保存成功"
