...
Public Class FrmAssetMaintenanceType
...
#Region "方法"
    ...
    Private Function SaveData() As Boolean
        ...
        Try
            ...
            If Me.ActivaStatus = FormBtnonType.Add.ToString Then
                o_strSql.Append("INSERT INTO m_AssetMaintenanceType_t (AssetName,MaintenanceProject,IsMpDay,IsMpMonth,IsMpQuarter,CreateTime,CreateUserID)")
                o_strSql.Append(String.Format(" VALUES(N'{0}',N'{1}',{2},{3},{4},GETDATE(),N'{5}')", Me.cbAssetName.SelectedValue.ToString, txtMaintenanceProject.Text.Trim,
                                       IsMpDay, IsMpQuarter, IsMpQuarter, UserID))
                DbOperateUtils.ExecSQL(o_strSql.ToString)
            Else

                Dim _Id As String
                _Id = Me.dgvMaintenanceType.CurrentRow.Cells(GridView.Id).Value.ToString
                o_strSql.Append(String.Format("UPDATE m_AssetMaintenanceType_t SET AssetName=N'{0}',IsMpDay={1},IsMpMonth={2},IsMpQuarter={3} ,MaintenanceProject=N'{4}',CreateUserID=N'{5}',CreateTime=GETDATE() WHERE ID='{6}'",
                                              Me.cbAssetName.SelectedValue.ToString, IsMpDay, IsMpMonth, IsMpQuarter, Me.txtMaintenanceProject.Text.Trim, UserID, _Id))
                DbOperateUtils.ExecSQL(o_strSql.ToString)
            End If
        Catch ex As Exception
          
            SysMessageClass.WriteErrLog(ex, "FrmAssetMaintenanceType", "SaveData()", "sys")
        End Try
        Return True
    End Function
    ...
    Private Function DeleteData() As Boolean
        ...
        Try
            ...
            o_strSql.Append(String.Format(" DELETE m_AssetMaintenanceType_t WHERE ID={0} ", Id))
            DbOperateUtils.ExecSQL(o_strSql.ToString)
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmAssetMaintenanceType", "DeleteData()", "sys")
        End Try
    ...
    Private Function ExistMpCheck() As Boolean
        ...
        Try
            ...
            Dim Id As String = Nothing
            o_strSql.Append(String.Format("SELECT * FROM m_AssetMaintenanceType_t WHERE AssetName=N'{0}' AND MaintenanceProject=N'{1}'",
            ...
            End If
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmAssetMaintenanceType", "ExistMpCheck()", "sys")
        End Try
        Return True
    End Function
    ...
    Private Sub BindData()
        Try
            ...
            sb.Append(" ,b.UserName ,CreateTime,Id  FROM dbo.m_AssetMaintenanceType_t a left join m_Users_t b on b.UserID=a.CreateUserID where 1=1")
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub LoadAssetName()
        Try
            Dim strSQL As String = "SELECT AssetName as value,AssetName as text  from m_Asset_t   GROUP BY AssetName "
            ...
            For Each drr As DataRow In dt.Rows
                sList.Add(drr("text").ToString())
            Next

            cbAssetName.DataSource = dt
            cbAssetName.DisplayMember = "text"
            cbAssetName.ValueMember = "value"
            cbAssetName.AutoCompleteCustomSource.AddRange(sList.ToArray())
            cbAssetName.AutoCompleteSource = AutoCompleteSource.ListItems
            cbAssetName.AutoCompleteMode = AutoCompleteMode.SuggestAppend
        Catch ex As Exception
            SysMessageClass.WriteErrLog(ex, "FrmAssetMaintenanceType", "LoadAssetName()", "sys")
        End Try
