...
Public Class FrmCutCardBodyEdit
...
#Region "构造函数"
    Sub New(ByVal cutCardPn As String, ByVal action As String, ByVal dataGridViewRow As DataGridViewRow, ByVal isQueryOldVersion As Boolean)
        ...
        End Try
    End Sub
    Private frm As FrmRunCardCut
    Sub New(ByVal cutCardPn As String, ByVal action As String, _
            ByVal dataGridViewRow As DataGridViewRow, ByVal form As FrmRunCardCut, _
...
#Region "获取标准工艺"
    Private Sub btnGetProcessStandard_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGetProcessStandard.Click
        Try
            ...
            Using frm As New FrmCutCardBodyModify(CutCardPN, FrmCutCardBodyModify.ActionType.GetProcessParameter, Me.txtStationid.Text.Trim.Split(CChar("-"))(0), False, me.drawingVer )
                frm.ShowDialog()
                processStandardSql = frm.outputSql
                txtProcessStandard.Text = frm.outputProcessStand
                Me.txtLeftProcessStandard.Text = frm.outputLeftProcessStand
                Me.txtRightProcessStandard.Text = frm.outputRightProcessStand
                Me.txtLCLVale.Text = frm.outputLCLValue
                m_iFinishSize = CInt(Val(GetFinishSizeByPartID(CutCardPN)))
                ...
                Me.txtProcessStandard.Focus()

                If Me.txtProcessStandard.Text <> String.Empty OrElse m_iFinishSize > 0 Then
                    Me.iiWoringHours.Text = GetStdTime(Me.txtProcessStandard.Text).ToString
        ...
        Catch ex As Exception
            ...
            SysMessageClass.WriteErrLog(ex, "FrmCutCardBodyEdit", "btnGetProcessStandard_Click", "RCard")
        Finally
...
#Region "保存"
    Public Function SaveData() As Boolean
        Try
            ...
            Dim WorkingHours As Double = Val(Me.iiWoringHours.Text)
            ...
            Dim LCLValue As String = Me.txtLCLVale.Text.Trim
            ...
            Select Case Action
                ...
                Case "insert"
                    ...
                    '123 ==> 1 a, 2+1,3+1
                    StrSql =
                    "UPDATE {0} SET ModifyTime=GETDATE() WHERE PartID='" & Me.CutCardPN & "'" & RCardComBusiness.GetFatoryProfitcenter() & _
                    " UPDATE {1} SET StationSQ=StationSQ+1 WHERE PartID='" & Me.CutCardPN & "' AND STATIONSQ >= " & Val(Me.stationSeq) & "" & RCardComBusiness.GetFatoryProfitcenter() & _
                    " INSERT INTO {1}(PartID,StationID,DrawingVer,StationSQ,WorkingHours,Equipment,ProcessStandard,Remark,Status,UserID,InTime," &
                    " Shape,NewProcessStandard, VariableName1,VariableValue1,VariableName2,VariableValue2, Factoryid,Profitcenter) VALUES(" &
                    "'" & Me.CutCardPN & "','" & StationID & "','" & drawingVer & "'," & Val(Me.stationSeq) & ",'" & WorkingHours & "',N'" & Equipment & "',N'" &
                    ProcessStandard & "',N'" & remark & "',1,'" & UserID & "',getdate(),'" & Shape & "',N'" & NewProcessStandard & "'" &
                    " ,N'" & m_strVarName1 & "'," & m_iVarValue1 & ", '" & m_strVarName2 & "'," & m_iVarValue2 & "," &
                    " '" & factoryID & "','" & profitCenter & "','" & LCLValue & "') "
                    StrSql &= GetInsertCutcardPartSQL(Me.CutCardPN, StationID, Me.drawingVer)
                ...
                Case "modify"
                    ...
                    StrSql = " UPDATE {0} SET ModifyTime=GETDATE() WHERE PartID='" & Me.CutCardPN & "' " & RCardComBusiness.GetFatoryProfitcenter() &
                              ...
                              " WHERE PartID='" & Me.CutCardPN & "' AND StationID='" & Me.stationId & "'" &
                ...
                Case Else
                    StrSql = " UPDATE {0} SET ModifyTime=GETDATE() WHERE PartID='" & Me.CutCardPN & "' " & RCardComBusiness.GetFatoryProfitcenter &
                          " UPDATE {1} SET StationSQ=StationSQ+1 WHERE PartID='" & Me.CutCardPN & "' AND StationSQ>=" & iiStationSQ.Value & RCardComBusiness.GetFatoryProfitcenter &
                          " UPDATE {1} SET StationSQ=" & iiStationSQ.Value & " WHERE PartID='" & Me.CutCardPN & "' AND StationID='" & Me.stationId & "' " & RCardComBusiness.GetFatoryProfitcenter &
            ...
            '触发器保存修改记录
            If _isQueryOldVersion = False Then
                result = DbOperateUtils.ExecSQL(StrSql & vbNewLine & "UPDATE m_RCardCutM_t SET Status=0 WHERE PartID='" & Me.CutCardPN & "'" & RCardComBusiness.GetFatoryProfitcenter)
            Else
                result = DbOperateUtils.ExecSQL(StrSql & vbNewLine & "UPDATE m_CutCardMOldVer_t SET Status=0 WHERE PartID='" & Me.CutCardPN & "'" & RCardComBusiness.GetFatoryProfitcenter)
            End If
