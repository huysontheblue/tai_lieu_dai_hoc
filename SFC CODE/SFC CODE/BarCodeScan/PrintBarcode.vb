        ...
        Try
            ...
            Dim strSql As String = String.Format(
                  " declare @CodeRuleID varchar(20)" &
                    ...
                    " select TemplatePath,PrinterName,pp.Qty,ISNULL(PARTSET.F_codeID,'') AS F_codeID ,ISNULL(PARTSET.DValues,'') AS DValues  from m_PartPack_t PP " &
...
#Region "外部调用"
    ...
    Public Function PrintFullCartonWeight(CartonBarcode As String, LabelNum As String, al As ArrayList) As Boolean
        ...
        Try
            ...
            Dim sqlstr As String = ""
            If CheckWiFi6Print(al(0).ToString) Then
                Dim sTableName As String = ""
                Dim sKeyFileld As String = ""
                Dim lssql As String = "SELECT PARAMETER_VALUES,APFormName FROM m_SystemSetting_t WHERE PARAMETER_CODE='BarcodeSnPrintMac' AND PARAMETER_NAME='" & al(0).ToString & "'"
                Dim o_dt As DataTable = DbOperateUtils.GetDataTable(lssql)
                sTableName = o_dt.Rows(0)!PARAMETER_VALUES.ToString
                sKeyFileld = o_dt.Rows(0)!APFormName.ToString
                sqlstr = " select distinct b.MAC AS ppid  from m_Cartonsn_t a left join " & sTableName & " b on a.ppid =b." & sKeyFileld & " where  Cartonid='" & CartonBarcode & "' "
            Else
                sqlstr = " select distinct a.ppid  from m_Cartonsn_t a left join m_BarRecordValue_t b on a.ppid =b.barcodeSNID where  Cartonid='" & CartonBarcode & "' "
            End If
    ...
    '在线打印彩盒，机器标签(WIFI6)20220901
    Public Function PrintOnlineWifiList(CartonBarcode As String, partId As String, ByVal StationId As String) As Boolean
        ...
        Try
            ...
            '20200921
            Dim o_dtResource As DataTable = GetLabelDResourceValue(strCodeRuleID, partId)
            If Not IsNothing(o_dtResource) AndAlso o_dtResource.Rows.Count > 0 Then
                Dim col As Integer = 9
                For Each dr1 As DataRow In o_dtResource.Rows
                    col = col + 1
                    Select Case dr1.Item(0).ToString.ToUpper
                        Case "PartSet".ToUpper
                            Dim sValue As String = GetDValue(partId, dr1.Item(2).ToString.ToUpper)
                            TxtFileStrLabelValue.Append("""" & sValue & """" & ",")
                            titlestr.Append("""lable" & col & """,")
                        Case "Array28".ToUpper
                            Dim sMac As String = GetMacValue(partId, dt.Rows(0)!ppid.ToString)
                            TxtFileStrLabelValue.Append("""" & sMac & """" & ",")
                            titlestr.Append("""lable" & col & """,")
                        Case "".ToUpper
                            If dr1.Item(2).ToString.ToUpper = "M" Then
                                Dim sM As String = GetUnitCodeUnit(dr1.Item(1).ToString.ToUpper, Date.Now.ToString("MM"))
                                TxtFileStrLabelValue.Append("""" & sM & """" & ",")
                                titlestr.Append("""lable" & col & """,")
                            End If
                            If dr1.Item(2).ToString.ToUpper = "Y" Then
                                Dim sY As String = GetUnitCodeUnit(dr1.Item(1).ToString.ToUpper, Date.Now.ToString("yyyy"))
                                TxtFileStrLabelValue.Append("""" & sY & """" & ",")
                                titlestr.Append("""lable" & col & """,")
                            End If
                            If dr1.Item(2).ToString.ToUpper = "D" Then
                                Dim sD As String = GetUnitCodeUnit(dr1.Item(1).ToString.ToUpper, Date.Now.ToString("dd"))
                                TxtFileStrLabelValue.Append("""" & sD & """" & ",")
                                titlestr.Append("""lable" & col & """,")
                            End If
                    End Select
                Next
    ...
    End Function
    '20220921
    Private Function GetUnitCodeUnit(ByVal Unitid As String, ByVal shortdate As String)
        Try
            Dim Sqlstr As String = ""
            Dim fulltime As String = ""
            Dim RecTable As New DataTable
            Sqlstr = "SELECT TOP 1 CodeUnit FROM m_SnUnitD_t WHERE UnitID='" & Unitid & "' AND CodeUnit='" & shortdate & "' ORDER BY codesort DESC "
            RecTable = DbOperateUtils.GetDataTable(Sqlstr)
            fulltime = RecTable.Rows(0)("CodeUnit").ToString
            RecTable.Dispose()
            Return fulltime
        Catch ex As Exception
            Throw ex
    ...
    End Function
    '20220921
    Private Function GetLabelDResourceValue(ByVal strCodeID As String, ByVal strPartID As String) As DataTable
        Dim lssql As String = String.Empty
        GetLabelDResourceValue = Nothing
        lssql = " SELECT  DResource,unitid,F_codeID " & _
                " FROM dbo.m_SnRuleD_t " & _
                " WHERE CodeRuleID ='" & strCodeID & "'" & _
                " AND BarArea LIKE 'LABEL%'" & _
                " ORDER BY F_orderid"

        Using o_dt As DataTable = DbOperateUtils.GetDataTable(lssql)
            If Not IsNothing(o_dt) AndAlso o_dt.Rows.Count > 0 Then
                GetLabelDResourceValue = o_dt
            Else
                GetLabelDResourceValue = Nothing
            End If
        End Using
    ...
    End Function
    '20220921
    Private Function GetMacValue(ByVal strPartID As String, ByVal strBarcode As String) As String
        '获取TABLE内的MAC
        Dim lssql As String = String.Empty
        Dim sTableName As String = ""
        Dim sKeyFileld As String = ""
        ...
        lssql = "SELECT PARAMETER_VALUES,APFormName FROM m_SystemSetting_t WHERE PARAMETER_CODE='BarcodeSnPrintMac' AND PARAMETER_NAME='" & strPartID & "'"
        Dim o_dt As DataTable = DbOperateUtils.GetDataTable(lssql)
        If o_dt.Rows.Count > 0 Then
            sTableName = o_dt.Rows(0)!PARAMETER_VALUES.ToString
            sKeyFileld = o_dt.Rows(0)!APFormName.ToString
            sb.Append(" SELECT MAC FROM " & sTableName & " WHERE " & sKeyFileld & " = '" & strBarcode & "'")
            Dim dt As DataTable = DbOperateUtils.GetDataTable(sb.ToString)
            If dt.Rows.Count > 0 Then
                sMac = dt.Rows(0)!MAC.ToString
            End If
        End If 
        Return sMac
    End Function
    '20220921
    Private Function GetDValue(ByVal strPartID As String, ByVal strFcode As String) As String 
        Dim lssql As String = String.Empty
        Dim sFcode As String = ""
        Dim sb As New StringBuilder
        lssql = "SELECT DValues FROM m_SnPartSet_t WHERE Partid='" & strPartID & "' AND F_codeID='" & strFcode & "' AND PACKID IN ('C', 'N')  AND   Usey = 'Y'"
        Dim o_dt As DataTable = DbOperateUtils.GetDataTable(lssql)
        If o_dt.Rows.Count > 0 Then  
            sFcode = o_dt.Rows(0)!DValues.ToString
        End If
        Return sFcode
    End Function
    '20220921
    Public Function CheckWiFi6Print(strPartID As String) As Boolean
        Dim lssql As String = String.Empty
        Dim sb As New StringBuilder
        lssql = "SELECT PARAMETER_VALUES FROM m_SystemSetting_t WHERE PARAMETER_CODE='BarcodeSnPrintMac' AND PARAMETER_NAME='" & strPartID & "'"
        Dim o_dt As DataTable = DbOperateUtils.GetDataTable(lssql)
        If o_dt.Rows.Count > 0 Then
            Return True
        End If
