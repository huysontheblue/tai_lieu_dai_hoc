...
Public Class FrmReportAdm
    ...
    Private Sub loadTreeData()
        ...
        Dim sql As String = "select * from  m_Logtree_t where Tparent='m09_' or Tkey='m09_' or Tparent in (  select Tkey from m_Logtree_t where Tparent='m09_' ) order by Tparent,OrderBy"
    ...
    Private Sub AdvTree1_Click(sender As Object, e As EventArgs) Handles AdvTree1.Click
        ...
        End If
        If dt.Rows(0)("LocalDbUsey").ToString = "Y" Then
            Me.chkLocalDbUsey.Checked = True
        Else
            Me.chkLocalDbUsey.Checked = False
        End If
        Me.txtChartScript.Text = dt.Rows(0)("ChartScript").ToString
        Me.txtChartTitle.Text = dt.Rows(0)("ChartTitle").ToString
        Me.txtSummaryTitle.Text = dt.Rows(0)("SummaryTitle").ToString
        Me.txtChartType.Text = dt.Rows(0)("ChartType").ToString
        txtorderby.Value = dt.Rows(0)("OrderBy").ToString
    ...
    Private Function GetlogtreeDt(ByVal tkey As String) As DataTable
        Dim sql As String = String.Format("select a.Tkey,a.Tparent,a.Ttext,a.Enname,a.TreeTag,b.SqlScript,b.SqlUsey,DetScript,DetUsey,ChartScript,ChartTitle,SummaryTitle,ChartType,ChartUsey,LocalDbUsey,OrderBy=isnull(OrderBy,100) from m_Logtree_t a left join m_Logtree_Det_t b on a.Tkey=b.Tkey where a.Tkey='{0}'", tkey)
    ...
    Private Sub SaveLogtreeData()
        Try
            ...
            Dim parameters() As SqlParameter = {New SqlParameter("@SQLSCRIPT", SqlDbType.NText), New SqlParameter("@DetScript", SqlDbType.NText), New SqlParameter("@ChartScript", SqlDbType.NText), New SqlParameter("@ChartTitle", SqlDbType.NVarChar, 100), New SqlParameter("@SummaryTitle", SqlDbType.NVarChar, 100), New SqlParameter("@ChartType", SqlDbType.VarChar, 1)}
            ...
            parameters(4).Value = txtSummaryTitle.Text.Trim
            parameters(5).Value = txtChartType.Text.Trim.Split("-")(0)
            If opFlag = 1 Then
                sqlstr.Append(" declare @keyid varchar(20) select @keyid=[dbo].fun_GetReportMaxTkey() ")
                sqlstr.Append(" insert m_Logtree_t(Treeid,Tkey,Tparent,Ttext,Enname,TreeTag,Rightid,Formid,ButtonID,Timage,Tsimage,TADimage,TADsimage,TADUsey,listy,Usey,Remark,OrderBy) values(	@keyid	,	@keyid+'_'	,	'" & cmbTparent.SelectedNode.Name & "'	,	N'" & txtTtext.Text.Trim & "'	,	N'" & txtEnname.Text.Trim & "'	,	'" & txtTreeTag.Text.Trim & "'	,	'mes00'	,	'apm09005'	,	NULL	,	1	,	1	,	1	,	1	,	'N'	,	'Y'	,	'Y'	,	NULL,'" & txtorderby.Text.Trim & "'	)")
                If chkSqlUsey.Checked Then
                    sqlstr.Append(" insert m_Logtree_Det_t(Tkey,SqlScript,SqlUsey,DetScript,DetUsey,ChartScript,ChartTitle,SummaryTitle,ChartType,ChartUsey,LocalDbUsey) ")
                    sqlstr.Append(" values(@keyid+'_',@SQLSCRIPT,'" & IIf(chkSqlUsey.Checked, "Y", "N") & "',@DetScript,'" & IIf(chkDetUsey.Checked, "Y", "N") & "',@ChartScript,@ChartTitle,@SummaryTitle,@ChartType,'" & IIf(chkChartUsey.Checked, "Y", "N") & "','" & IIf(chkLocalDbUsey.Checked, "Y", "N") & "') ")
            ...
            If opFlag = 2 Then
                If txtTkey.Text.Trim.ToLower <> cmbTparent.SelectedNode.Name.Trim.ToLower Then
                    ...
                    sqlstr.Append(" update m_Logtree_t set Ttext=N'" & txtTtext.Text.Trim & "',Enname=N'" & txtEnname.Text.Trim & "',TreeTag=N'" & txtTreeTag.Text.Trim & "', Tparent='" & cmbTparent.SelectedNode.Name & "',OrderBy=N'" & txtorderby.Text.Trim & "'	 where Tkey=@keyid ")
                    If chkSqlUsey.Checked Then
                        sqlstr.Append(" if not exists(select * from m_Logtree_Det_t where Tkey=@keyid) ")
                        sqlstr.Append(" insert m_Logtree_Det_t(Tkey,SqlScript,SqlUsey,DetScript,DetUsey,ChartScript,ChartTitle,SummaryTitle,ChartType,ChartUsey,LocalDbUsey) ")
                        sqlstr.Append(" values(@keyid,@SQLSCRIPT,'" & IIf(chkSqlUsey.Checked, "Y", "N") & "',@DetScript,'" & IIf(chkDetUsey.Checked, "Y", "N") & "',@ChartScript,@ChartTitle,@SummaryTitle,@ChartType,'" & IIf(chkChartUsey.Checked, "Y", "N") & "','" & IIf(chkLocalDbUsey.Checked, "Y", "N") & "') ")
                        sqlstr.Append(" else ")
                        sqlstr.Append(" update m_Logtree_Det_t set SqlScript=@SQLSCRIPT,SqlUsey='" & IIf(chkSqlUsey.Checked, "Y", "N") & "',DetScript=@DetScript,DetUsey='" & IIf(chkDetUsey.Checked, "Y", "N") & "',ChartScript=@ChartScript,ChartTitle=@ChartTitle,SummaryTitle=@SummaryTitle,ChartType=@ChartType,ChartUsey='" & IIf(chkChartUsey.Checked, "Y", "N") & "',LocalDbUsey='" & IIf(chkLocalDbUsey.Checked, "Y", "N") & "' where Tkey=@keyid ")
                        sqlstr.Append(" declare @ReportUsey varchar(1)='Y'")
                    Else
                        sqlstr.Append(" update m_Logtree_Det_t set SqlScript=@SQLSCRIPT,SqlUsey='N',DetScript=@DetScript,DetUsey='" & IIf(chkDetUsey.Checked, "Y", "N") & "',ChartScript=@ChartScript,ChartTitle=@ChartTitle,SummaryTitle=@SummaryTitle,ChartType=@ChartType,ChartUsey='" & IIf(chkChartUsey.Checked, "Y", "N") & "',LocalDbUsey='" & IIf(chkLocalDbUsey.Checked, "Y", "N") & "' where Tkey=@keyid ")
            ...
            If Not AdvTree1.FindNodeByDataKey(Me.txtTkey.Text) Is Nothing Then
