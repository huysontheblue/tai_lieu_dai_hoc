    ...
    Private Function DataAnalysis(ByVal MsnId As String) As Boolean
        ...
        Try
            ...
            If (CobLaserStyle.SelectedIndex = 1) Then
                mRead = conn.GetDataReader("select 1 from m_ppidlink_t  where exppid='" & str1 & "'")
                If mRead.HasRows = False Then
                    mRead.Close()
                    LblReadCount.Text = "该序号打印记录中不存在或未进行关联扫描..."
                    Me.txtInput.Clear()
                    IsSendOkFlag = "false"
                    clearMem()
                    Return False
            ...
            If RdataSave.Count > 1 Then
                Call showResult("治具连接器读取的数量，超过系统预设的1次!", False)
                LblReadCount.Text = "治具连接器读取的数量，超过系统预设的1次..."
                IsSendOkFlag = "false"
                Return False
                Exit Function
            End If
...
#End Region
    ...
    Private Sub txtInput_PreviewKeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.PreviewKeyDownEventArgs) Handles txtInput.PreviewKeyDown
        ...
        If e.KeyValue = 13 Then
            ...
            If ChkScanStyle.Checked = False Then
            ...
            Else
                ...
                txtInput.Text = ""
                If CobLaserStyle.SelectedIndex = 1 Then
                    printChrLength = TableLayoutPanel1.Controls.Item("lbl12").Text.Trim.Length
                End If
                If chkAuto.Checked = True And TableLayoutPanel1.Controls.Item("lbl11").Text.Trim <> "" And TableLayoutPanel1.Controls.Item("lbl12").Text.Trim.Length = printChrLength Then
