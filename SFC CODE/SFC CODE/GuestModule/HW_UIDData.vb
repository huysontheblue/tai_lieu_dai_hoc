...
Public Class HW_UIDData
    ...
    Private Sub HW_UIDData_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.DtStar.Value = Date.Now
        Me.DtEnd.Value = Date.Now
        'QueryData() ''load不初始化查询
    End Sub
    ...
    Private Sub btnUnLock_Click(sender As Object, e As EventArgs) Handles btnUnLock.Click
        ...
        If DialogResult = Windows.Forms.DialogResult.Yes Then
            Dim sql As String = "update MESDataCenter.dbo.m_HW_C314OTAUID_t set Usey='N',LockBy=null,LockTime=null,UsedBy=null,UsedTime=null,Remark=isnull((case when len(Remark)>100 then '' else Remark end),'')+N'解除锁定By'+N'" & UserID & "'+','+convert(varchar(25),getdate() ,21)+';' where ID='" & id & "'"
            Dim returnval As String = ""

            Try
                Dim sqlL As String = "SELECT Usey,Partid FROM MESDataCenter.dbo.m_HW_C314OTAUID_t WHERE  id='" + id + "'"
                Dim Data As DataTable = conn.GetDataTable(sqlL)
                If Data.Rows(0)(0).ToString() <> "N" Then
                    sqlL = "UPDATE MESDataCenter.dbo.m_HW_C314UIDUse  SET Alreadyused = Alreadyused-1  WHERE Partid ='" + Data.Rows(0)(1) + "'"
    ...
    Private Sub btnClear_Click(sender As Object, e As EventArgs) Handles btnClear.Click
        ...
        If DialogResult = Windows.Forms.DialogResult.Yes Then
            ...
            Dim sql As String = "update MESDataCenter.dbo.m_HW_C314OTAUID_t set Usey='N',LockBy=null,LockTime=null,UsedBy=null,UsedTime=null,Remark=isnull((case when len(Remark)>100 then '' else Remark end),'')+N'清除使用记录By'+N'" & UserID & "'+','+convert(varchar(25),getdate() ,21)+';' where ID='" & id & "'"
            Dim returnval As String = ""
            'Dim conn As New MainFrame.SysDataHandle.SysDataBaseClass()
            Try
                Dim sqlL As String = "SELECT Usey,Partid FROM MESDataCenter.dbo.m_HW_C314OTAUID_t WHERE  id='" + id + "'"
    ...
    Private Sub QueryData()
        ...
        strSql.Append("select id,PartID,FileName,UID=N'...',Path,Intime,UserId,Usey,LockBy,LockTime,UsedBy,UsedTime,Remark from MESDataCenter.dbo.m_HW_C314OTAUID_t ")
